...
public class FindTextTests extends FreeMindTestBase {
  ...
  public void testDirectReplace() throws Exception {
    assertEquals(
        ...
        HtmlTools.getInstance()
            .getReplaceResult(
                Pattern.compile("test"), "blabla", "<html><strong>test</strong></html>"));
    assertEquals(
    ...
    assertEquals(
        ...
        HtmlTools.getInstance()
            .getReplaceResult(
                Pattern.compile("strong"), "strang", "<html><strong>blabla</strong></html>"));
  }
  ...
  public void testReplaceNodeText() throws Exception {
    ...
    TimeList.IReplaceInputInformation info =
        new TestReplaceInputInfo(
            "<html><strong>test</strong></html>", "<html><strong>blabla</strong></html>");
    TimeList.replace(info, "test", "blabla");
    // but tags not:
    info =
        new TestReplaceInputInfo(
            "<html><strong>test</strong></html>", "<html><strong>test</strong></html>");
    TimeList.replace(info, "strong", "strang");
    // and replaces <tag>tes</tag>t by <tag>text</tag>:
    info =
        new TestReplaceInputInfo(
            "<html><strong>tes</strong>t</html>", "<html><strong>blabla</strong></html>");
    TimeList.replace(info, "test", "blabla");
    // and replaces tex<tag>t</tag> by text<tag></tag>:
    info =
        new TestReplaceInputInfo(
            "<html>tes<strong>t</strong></html>", "<html>blabla<strong></strong></html>");
    TimeList.replace(info, "test", "blabla");
    ...
    info =
        new TestReplaceInputInfo(
            "<html><strong>tes</strong>t</html>", "<html><strong>tes</strong>t</html>" /*
