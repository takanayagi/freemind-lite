 ...
 * @author foltin
 * 
 *         Exports the map using an XSLT script. The parameterization is described in the
...
public class ExportWithXSLT extends ExportHook {
		...
		if (Tools.safeEquals(getResourceString("file_type"), "user")) {
			...
			try {
				...
				if (transformErrors != null) {
					JOptionPane.showMessageDialog(null, getResourceString(transformErrors),
	...
	public String transform(File saveFile) throws Exception {
		...
		boolean create_image =
				Tools.safeEquals(getResourceString("create_html_linked_image"), "true");
	...
	private void createImageFromMap(String directoryName) {
		...
		try {
			FileOutputStream out =
					new FileOutputStream(directoryName + File.separator + "image.png");
	...
	/**
	 */
	private void copyFilesFromResourcesToDirectory(String directoryName, String files,
			String filePrefix) {
	...
	public boolean transform(Source xmlSource, InputStream xsltStream, File resultFile,
		...
		try {
			...
			trans.setParameter("destination_dir", Tools.fileToRelativeUrlString(
