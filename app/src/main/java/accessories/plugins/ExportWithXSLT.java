...
/**
 * @author foltin
 *     <p>Exports the map using an XSLT script. The parameterization is described in the
 *     corresponding Export... .xml-file.
 */
public class ExportWithXSLT extends ExportHook {
  ...
  public void startupMapHook() {
    ...
    if (Tools.safeEquals(getResourceString("file_type"), "user")) {
      if (model == null) return; // there may be no map open
      ...
      try {
        ...
        if (transformErrors != null) {
          JOptionPane.showMessageDialog(
              null, getResourceString(transformErrors), "Freemind", JOptionPane.ERROR_MESSAGE);
  ...
  /** */
  private void copyFilesFromResourcesToDirectory(
      String directoryName, String files, String filePrefix) {
   ...
   * @throws IOException
   */
  private boolean transformMapWithXslt(String xsltFileName, File saveFile, String areaCode)
      throws IOException {
  ...
  private String getAreaCode(boolean create_image) {
    ...
    if (create_image) {
      ...
      ClickableImageCreator creator =
          new ClickableImageCreator(
              root, getController(), getResourceString("link_replacement_format_string"));
  ...
  public boolean transform(
      Source xmlSource, InputStream xsltStream, File resultFile, String areaCode)
    ...
    try {
      ...
      trans.setParameter(
          "destination_dir",
          Tools.fileToRelativeUrlString(
                  new File(resultFile.getAbsolutePath() + "_files/"), resultFile)
              + "/");
