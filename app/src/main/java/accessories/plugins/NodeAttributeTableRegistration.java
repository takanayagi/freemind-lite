...
public class NodeAttributeTableRegistration
    implements HookRegistration, MenuItemSelectedListener, DocumentListener {
  private final class AttributeManager implements NodeSelectionListener, NodeLifetimeListener {
    ...
    public void onSaveNode(MindMapNode pNode) {
      if (mAttributeTable.isEditing()) {
        // This will dispose editor and call setValueAt() of your model as normally
        // happens
    ...
    public boolean areModelAndNodeAttributesEqual() {
      ...
      if (mCurrentNode.getAttributeTableLength() == mAttributeTableModel.mData.size()) {
        ...
        for (AttributeHolder holder : mAttributeTableModel.mData) {
          ...
          if (Tools.safeEquals(holder.mKey, attribute.getName())
              && Tools.safeEquals(holder.mValue, attribute.getValue())) {
            // ok
          } else {
            equal = false;
            break;
  ...
  private static final String DELETE_ROW_TEXT_ID =
  ...
  public final class AttributeTableModel extends AbstractTableModel {
    /** */
    private final String[] COLUMNS = new String[] {KEY_COLUMN_TEXT, VALUE_COLUMN_TEXT};
  ...
  public boolean shouldShowSplitPane() {
    return "true".equals(controller.getProperty(FreeMind.RESOURCES_SHOW_ATTRIBUTE_PANE));
  }
  ...
  public void register() {
    ...
    RowSorter<TableModel> sorter = new TableRowSorter<TableModel>(mAttributeTableModel);
    String marshalled = controller.getProperty(ATTRIBUTE_TABLE_PROPERTIES);
    AttributeTableProperties props =
        (AttributeTableProperties) XmlBindingTools.getInstance().unMarshall(marshalled);
    Vector<SortKey> keys = new Vector<RowSorter.SortKey>();
    for (Iterator<TableColumnOrder> it = props.getListTableColumnOrderList().iterator();
        it.hasNext(); ) {
      TableColumnOrder setting = it.next();
      keys.add(
          new SortKey(setting.getColumnIndex(), SortOrder.valueOf(setting.getColumnSorting())));
    }
    ...
    menuItem.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent e) {
            ...
            mAttributeTableModel.removeAttributeHolder(
                table.convertRowIndexToModel(table.getSelectedRow()));
          }
        });
    mAttributeTable.addMouseListener(
        new MouseAdapter() {
  ...
  public void checkForUpdate() {
    ...
    if (mAttributeTable.getCellEditor() != null) makeMapDirty();
  }
}
