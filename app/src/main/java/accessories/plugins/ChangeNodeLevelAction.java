/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2000-2006 by Christian
 * Foltin See COPYING for Details
 *
...
public class ChangeNodeLevelAction extends MindMapNodeHookAdapter {
	...
	public void invoke(MindMapNode rootNode) {
		...
		boolean upwards =
				Tools.safeEquals("left", getResourceString("action_type")) != selectedNode.isLeft();
		...
		if (upwards) {
			...
			if (parentPosition == grandParent.getChildCount() - 1) {
				...
				getMindMapController().paste(copy,
						(MindMapNode) grandParent.getChildAt(parentPosition + 1), true, isLeft);
			}
			select(selectedNodeId, selectedNodesId);
	...
	private void select(String selectedNodeId, List<String> selectedNodesIds) {
		...
		MindMapNode newInstanceOfSelectedNode =
				getMindMapController().getNodeFromID(selectedNodeId);
