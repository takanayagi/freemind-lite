...
public class ExportDialog extends JFrame {
	private static final String ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_TARGET =
			"accessories.plugins.util.xslt.ExportDialog.store.target"; //$NON-NLS-1$
	private static final String ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_XSLT =
			"accessories.plugins.util.xslt.ExportDialog.store.xslt"; //$NON-NLS-1$
	...
	public ExportDialog(File nxmlFile, ModeController pController) {
		...
		/*
		 * List list = new List(); for (int i = 0; i < 20; ++i) { list.add("This is item " + i); }
		 * gbc = makegbc(0, 0, 1, 3); gbc.weightx = 100; gbc.weighty = 100; gbc.fill =
		 * GridBagConstraints.BOTH; gbl.setConstraints(list, gbc); getContentPane().add(list);
		 */

		// get last value from preferences:
		String lastXsltFileName = mController.getFrame()
				.getProperty(ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_XSLT);
		String lastTargetFileName = mController.getFrame()
				.getProperty(ACCESSORIES_PLUGINS_UTIL_XSLT_EXPORT_DIALOG_STORE_TARGET);
		...
		xslbutton.addActionListener(
				new FileChooseListener(0, fieldXsltFileName, xslbutton, xmlFile));
		...
		exportbutton.addActionListener(
				new FileChooseListener(1, fieldTargetFileName, exportbutton, xmlFile));
	...
	class FileChooseListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (kind == 0) {
				filter = new ExampleFileFilter("xsl", //$NON-NLS-1$
						getResourceString("ExportDialog.13")); //$NON-NLS-1$
			}
					...
					if (kind == 0) {
						if (!new File(chooser.getSelectedFile().getAbsolutePath()).exists()) {
							...
							JOptionPane.showMessageDialog(null, Message,
									getResourceString("ExportDialog.15"), //$NON-NLS-1$
					...
					if (kind == 1) {
						if (!new File(chooser.getSelectedFile().getAbsolutePath()).exists()) {
							...
							int i = JOptionPane.showConfirmDialog(null,
									getResourceString("ExportDialog.16"), //$NON-NLS-1$
									getResourceString("ExportDialog.17"), //$NON-NLS-1$
