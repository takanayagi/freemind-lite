 ...
 * 
 *         Initial Comment: The "New Parent Node" action creates a node as a parent of one or more
 *         selected nodes. If more than one node is selected, the selected nodes must all have the
 *         same parent -- this restriction is imposed to make the action easier to understand and to
 *         undo manually, and could potentially be removed when we get automated undo.
 * 
 *         The root node must not be one of the selected nodes. I find this action useful when I
 *         need to add an extra level of grouping in the middle of an existing hierarchy. It is
 *         quicker than adding a new node at the same level and then cutting-and-pasting the child
 *         nodes. The code simply performs these actions in sequence, after validating the selected
 *         nodes.
 */
public class NewParentNode extends MindMapNodeHookAdapter {
	...
	public void invoke(MindMapNode rootNode) {
		...
		if (focussed.isRoot()) {
			if (selecteds.size() == 1) {
				...
				/*
				 * look for the new node as the difference between former children and new children.
				 */
			...
			getMindMapController().getController()
					.errorMessage(getResourceString("cannot_add_parent_to_root"));
			return;
		}

		MindMapNode newNode = moveToNewParent(rootNode, selectedNode, selectedNodes);
		if (newNode == null) {
			return;
		}
	...
	private MindMapNode moveToNewParent(MindMapNode rootNode, MindMapNode selectedNode,
		...
		MindMapNode newNode = getMindMapController().addNewNode(selectedParent, childPosition,
				selectedNode.isLeft());
		return moveToOtherNode(rootNode, selectedNodes, selectedParent, newNode);
	}

	private MindMapNode moveToOtherNode(MindMapNode rootNode, List<MindMapNode> nodesToBeMoved,
		...
		for (MindMapNode node : nodesToBeMoved) {
			if (node.getParentNode() != selectedParent) {
				getMindMapController().getController()
						.errorMessage(getResourceString("cannot_add_parent_diff_parents"));
				return null;
			}
			if (node == rootNode) {
				getMindMapController().getController()
		...
		getMindMapController().select(selectedParent,
				Tools.getVectorWithSingleElement(selectedParent));
