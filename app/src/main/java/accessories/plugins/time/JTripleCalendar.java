/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2007 Christian Foltin
...
/**
 * Formerly, it has only three calendar widgets at once. Now, it has 9 in total, but we keep the
 * naming.
 */
public class JTripleCalendar extends JPanel implements PropertyChangeListener {
	 ...
	 */
	private HashMap<JSwitchableCalendar, Integer> mInfoPanels = new HashMap<>();
		...
		try {
			for (int column = 0; column < AMOUNT_OF_COLUMNS; ++column) {
				...
				if (column < AMOUNT_OF_COLUMNS - 1) {
					GridBagConstraints constraints = new GridBagConstraints(2 * column + 1, 0, 1,
							AMOUNT_OF_ROWS, 0, 0, GridBagConstraints.CENTER,
							GridBagConstraints.VERTICAL, new Insets(0, 0, 0, 10), 0, 0);
	...
	protected GridBagConstraints getConstraints(int row, int column) {
		return new GridBagConstraints(2 * column, row, 1, 1, 1, 1, GridBagConstraints.CENTER,
		...
		protected JDayChooser createJDayChooser(boolean pWeekOfYearVisible) {
			return new JDayChooser(weekOfYearVisible) {
				...
				protected void init() {
					 ...
					 * the user presses. Thus, without this statement, no property change event is
					 * issued.
					 */
	...
	private JSwitchableCalendar createInfoPanel() {
		return new JSwitchableCalendar();
	}
	...
	protected void setCalendarInternally(Calendar gregorianCalendar) {
		...
		// System.out.println("Comparing new date " + gregorianCalendar.getTime() + "
		// with active panel time " + mCurrentDate.getTime());
		int dist = mCurrentDate.get(Calendar.MONTH) - gregorianCalendar.get(Calendar.MONTH)
				+ 12 * (mCurrentDate.get(Calendar.YEAR) - gregorianCalendar.get(Calendar.YEAR));
		if (dist == 0) {
	...
	public void exchangeCalendars(JSwitchableCalendar infoPanel, Calendar gregorianCalendar) {
