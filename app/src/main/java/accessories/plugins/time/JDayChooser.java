/*
 * JDayChooser.java - A bean for choosing a day Copyright (C) 2004 Kai Toedter kai@toedter.com
 * www.toedter.com
 *
...
public class JDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener {
  ...
  public static class ChangeAwareButton extends ScalableJButton {
    ...
    public void setText(String pText) {
      if (getText().equals(pText)) return;
      super.setText(pText);
    }
    ...
    public void setForeground(Color pFg) {
      if (!Tools.safeEquals(getForeground(), pFg)) super.setForeground(pFg);
    }
    ...
    public void setBackground(Color pBg) {
      if (!Tools.safeEquals(getBackground(), pBg)) super.setBackground(pBg);
    }
  }
  ...
  public JDayChooser(boolean weekOfYearVisible) {
    if (mCalendarMarkingEvaluator == null) {
      ...
      try {
        String marking =
            Resources.getInstance().getProperty(FreeMindCommon.TIME_MANAGEMENT_MARKING_XML);
        if (!marking.isEmpty()) {
      ...
      Controller.addPropertyChangeListener(
          (pPropertyName, pNewValue, pOldValue) -> {
            if (FreeMindCommon.TIME_MANAGEMENT_MARKING_XML.equals(pPropertyName)) {
              CalendarMarkings markings1 =
                  (CalendarMarkings) XmlBindingTools.getInstance().unMarshall(pNewValue);
  ...
  protected void drawWeeks() {
    ...
    for (int i = 1; i < 7; i++) {
      ...
      if (!currentWeek.getText().equals(buttonText)) currentWeek.setText(buttonText);
  ...
  protected void drawDays() {
    ...
    while (day.before(firstDayInNextMonth)) {
      ...
      if (mCalendarMarkingEvaluator != null) {
        CalendarMarking marked = mCalendarMarkingEvaluator.isMarked(tmpCalendar);
        if (marked != null) {
      ...
      currentDay.setEnabled(!tmpCalendar.before(minCal) && !tmpCalendar.after(maxCal));
    ...
    for (int k = n + i + 7; k < 49; k++) {
      ...
      currentDay.setText("");
      /*
       * this was a try to display the days of the adjacent months, too, but some points are
       * missing: - only the last week should be filled
       *
       */
   ...
   */
  /*
   * Code below commented out by Mark Brown on 24 Aug 2004. This code breaks the JDateChooser
   * code by triggering the actionPerformed method on the next day button. This causes the
   * date chosen to always be incremented by one day.
   */
  ...
  /**
   * JDayChooser is the KeyListener for all day buttons. (Added by Thomas Schaefer and modified by
   * Austin Moore)
   *
  ...
  public GregorianCalendar getTemporaryCalendar() {
    return new GregorianCalendar(yearChooser.getYear(), monthChooser.getMonth(), getDay());
  }
  ...
  /**
   * In some Countries it is often useful to know in which week of the year a date is.
   *
  ...
  /**
   * Sets the maximum number of characters per day in the day bar. Valid values are 0-4. If set to
   * 0, dateFormatSymbols.getShortWeekdays() will be used, otherwise these strings will be reduced
