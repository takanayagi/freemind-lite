...
public class JDayChooser extends JPanel implements ActionListener, KeyListener, FocusListener {
	...
	public JDayChooser(boolean weekOfYearVisible) {
		if (mCalendarMarkingEvaluator == null) {
			...
			try {
				String marking = Resources.getInstance()
						.getProperty(FreeMindCommon.TIME_MANAGEMENT_MARKING_XML);
				if (!marking.isEmpty()) {
	...
	protected void drawDays() {
		...
		while (day.before(firstDayInNextMonth)) {
			...
			if (mCalendarMarkingEvaluator != null) {
				...
				if (marked != null) {
					currentBorder =
							BorderFactory.createLineBorder(Tools.xmlToColor(marked.getColor()), 2);
	...
	/**
	 * The decoration background is the background color of the day titles and the weeks of the
	 * year.
	 * 
	...
	public void updateUI() {
		...
		int fontSize =
				Resources.getInstance().getIntProperty(FreeMind.RESOURCES_CALENDAR_FONT_SIZE, 0);
	 ...
	 * 0, dateFormatSymbols.getShortWeekdays() will be used, otherwise these strings will be
	 * reduced to the maximum number of characters.
	 * 
