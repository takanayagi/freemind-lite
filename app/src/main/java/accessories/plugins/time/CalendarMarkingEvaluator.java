/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2000-2015 Christian Foltin,
...
public class CalendarMarkingEvaluator implements ICalendarMarkingEvaluator {
	...
	private abstract static class BasicRepetitionHandler implements RepetitionHandler {
		...
		public Calendar shiftToBeAfterStartDate(Calendar pFirstDate, CalendarMarking pMarking,
				Calendar pUserStartDate) {
			while (pFirstDate != null && pUserStartDate.compareTo(pFirstDate) > 0) {
				...
				if (millies >= pFirstDate.getTimeInMillis()) {
					throw new IllegalArgumentException(
							"Next doesn't work: " + pFirstDate.getTime());
				}
			}
			return pFirstDate;
		}
	...
	public CalendarMarkingEvaluator(CalendarMarkings pCalendarMarkings) {
		...
		if (sHandlerMap == null) {
			...
			sHandlerMap.put(CalendarMarking.MONTHLY_EVERY_NTH_WEEK,
					new MonthlyEveryNthWeekHandler());
			...
			sHandlerMap.put(CalendarMarking.YEARLY_EVERY_NTH_MONTH,
					new YearlyEveryNthMonthHandler());
		}
	}
	...
	public Set<Calendar> getAtLeastTheFirstNEntries(int n) {
		...
		for (int i = 0; i < mCalendarMarkings.sizeCalendarMarkingList(); i++) {
			...
			RepetitionHandler handler = sHandlerMap.get(repeatType);
	...
	private boolean equal(Calendar pCalendar, Calendar pOtherDay) {
		return pCalendar.get(Calendar.YEAR) == pOtherDay.get(Calendar.YEAR)
				&& pCalendar.get(Calendar.MONTH) == pOtherDay.get(Calendar.MONTH)
				&& pCalendar.get(Calendar.DAY_OF_MONTH) == pOtherDay.get(Calendar.DAY_OF_MONTH);
	}
