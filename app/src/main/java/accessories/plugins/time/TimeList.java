/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2000-2005 Joerg Mueller,
...
public class TimeList extends MindMapHookAdapter implements MapModuleChangeObserver {
	...
	public void startupMapHook() {
		...
		mSorter.setColumnComparator(Date.class, TableSorter.COMPARABLE_COMPARATOR);
		mSorter.setColumnComparator(NodeHolder.class, TableSorter.LEXICAL_COMPARATOR);
		mSorter.setColumnComparator(NotesHolder.class, TableSorter.LEXICAL_COMPARATOR);
		mSorter.setColumnComparator(IconsHolder.class, TableSorter.COMPARABLE_COMPARATOR);
		...
		contentPane.add(new JScrollPane(mTreeLabel), new GridBagConstraints(0, 5, 1, 2, 1.0, 4.0,
				GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		mStatisticsLabel = new JLabel();
		contentPane.add(new JScrollPane(mStatisticsLabel),
				new GridBagConstraints(0, 7, 1, 2, 1.0, 4.0, GridBagConstraints.WEST,
						GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
		// button bar
		...
		final JMenuItem selectMenuItem =
				addAccelerator(menuHolder.addAction(selectAction, "main/actions/select"),
		...
		addAccelerator(menuHolder.addAction(replaceAllAction, "main/actions/replaceAll"),
				"keystroke_plugins/TimeList_replaceAll");

		final JMenuItem exportMenuItem =
				addAccelerator(menuHolder.addAction(exportAction, "main/actions/export"),
						"keystroke_plugins/TimeList_export");
		addAccelerator(menuHolder.addAction(disposeAction, "main/actions/dispose"),
	...
	protected void decorateButtonAndAction(String stringProperty, final AbstractAction selectAction,
	...
	protected void exportSelectedRowsAndClose() {
		...
		// create new map:
		MindMapController newMindMapController =
				(MindMapController) getMindMapController().newMap();
	 ...
	 * @date 25.04.2012
	 */
	private final class ToggleViewFoldedNodesAction extends AbstractAction
	...
	public static void replace(IReplaceInputInformation info, String searchString,
		...
		for (int i = 0; i < length; i++) {
			...
			String replaceResult = HtmlTools.getInstance().getReplaceResult(p, replaceString, text);
	...
	private class ReplaceAllInfo implements IReplaceInputInformation {
		...
		public NodeHolder getNodeHolderAt(int i) {
			return (NodeHolder) mFlatNodeTableFilterModel.getValueAt(i, NODE_TEXT_COLUMN);
		}
	...
	private class ReplaceSelectedInfo implements IReplaceInputInformation {
		...
		public NodeHolder getNodeHolderAt(int i) {
			return (NodeHolder) mSorter.getValueAt(mTimeTable.getSelectedRows()[i],
					NODE_TEXT_COLUMN);
		}
	...
	private MindMapNode getMindMapNode(int focussedRow) {
		return ((NodeHolder) mTimeTable.getModel().getValueAt(focussedRow, NODE_TEXT_COLUMN)).node;
	}
	...
	private DefaultTableModel updateModel() {
		...
		mFlatNodeTableFilterModel =
				new FlatNodeTableFilterModel(mTimeTableModel, NODE_TEXT_COLUMN, NODE_NOTES_COLUMN);
		if (mSorter == null) {
	...
	protected TimeWindowConfigurationStorage getTableConfiguration() {
		...
		for (int i = 0; i < mTimeTable.getColumnCount(); i++) {
			...
			setting.setColumnWidth(mTimeTable.getColumnModel().getColumn(i).getWidth());
	...
	/**
	 * Removes all regular expression stuff with exception of "*", which is replaced by ".*".
	 */
	...
	static class NodeRenderer extends DefaultTableCellRenderer {
		...
		public void setValue(Object value) {
			...
			setText((value == null) ? "" : holder.getUntaggedNodeText());
		}
	}
	...
	public static class NodeHolder implements Comparable<NodeHolder> {
		...
		/**
		 * Holds the original node content to cache the untaggedNodeText and to see whether or not
		 * the cache is dirty.
		 */
	...
	public static class NotesHolder implements Comparable<NotesHolder> {
		...
		public String getUntaggedNotesText() {
			...
			if (untaggedNotesText == null
				...
				// remove tags:
				untaggedNotesText =
						HtmlTools.removeHtmlTagsFromString(notesText).replaceAll("\\s+", " ");
			}
	...
	public void updateStatistics() {
		...
		mStatisticsLabel
				.setText(Resources.getInstance().format("timelist_statistics", new Integer[] {count,
						rowCount, mTimeTableModel.getRowCount(), ((mViewFoldedNodes) ? 1 : 0)}));
	}
}
