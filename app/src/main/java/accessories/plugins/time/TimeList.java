...
/**
 * @author foltin
 *     <p>TODO: - Extract HTML from nodes and notes.
 */
public class TimeList extends MindMapHookAdapter implements MapModuleChangeObserver {
  ...
  public void startupMapHook() {
    ...
    contentPane.add(
        new JScrollPane(mStatisticsLabel),
        new GridBagConstraints(
            0,
            7,
            1,
  ...
  protected void setTableConfiguration(WindowConfigurationStorage storage) {
    ...
    for (TimeWindowColumnSetting setting : timeStorage.getListTimeWindowColumnSettingList()) {
      mTimeTable.getColumnModel().getColumn(column).setPreferredWidth(setting.getColumnWidth());
  ...
  private class ReplaceSelectedInfo implements IReplaceInputInformation {
    ...
    public NodeHolder getNodeHolderAt(int i) {
      return (NodeHolder) mSorter.getValueAt(mTimeTable.getSelectedRows()[i], NODE_TEXT_COLUMN);
    }
  ...
  private final class FlatNodeTable extends ScalableJTable {
    public TableCellRenderer getCellRenderer(int row, int column) {
      ...
      if (object instanceof Date) return mDateRenderer;
      if (object instanceof NodeHolder) return mNodeRenderer;
      if (object instanceof NotesHolder) return mNotesRenderer;
      if (object instanceof IconsHolder) return mIconsRenderer;
  ...
  public static class NodeHolder implements Comparable<NodeHolder> {
    ...
    /**
     * Holds the original node content to cache the untaggedNodeText and to see whether or not the
     * cache is dirty.
     */
  ...
  public void updateStatistics() {
    ...
    mStatisticsLabel.setText(
        Resources.getInstance()
            .format(
                "timelist_statistics",
                new Integer[] {
                  count, rowCount, mTimeTableModel.getRowCount(), ((mViewFoldedNodes) ? 1 : 0)
                }));
  }
}
