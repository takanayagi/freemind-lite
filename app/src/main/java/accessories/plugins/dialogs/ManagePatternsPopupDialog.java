...
public class ManagePatternsPopupDialog extends JDialog implements TextTranslator, KeyListener {
	...
	private final class PatternListSelectionListener implements ListSelectionListener {
	...
	private static final String WINDOW_PREFERENCE_STORAGE_PROPERTY =
			"accessories.plugins.dialogs.ManagePatternsPopupDialog/window_positions";
	...
	public ManagePatternsPopupDialog(JFrame caller, MindMapController controller) {
		...
		if (logger == null) {
			logger = mController.getFrame().getLogger(this.getClass().getName());
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(this, getDialogTitle(),
	...
	private void initialize(List<Pattern> patternList) {
		...
		this.pack();
		ManageStyleEditorWindowConfigurationStorage decorateDialog =
				(ManageStyleEditorWindowConfigurationStorage) mController.decorateDialog(this,
	...
	private void close() {
		ManageStyleEditorWindowConfigurationStorage storage =
				new ManageStyleEditorWindowConfigurationStorage();
		...
		if (jContentPane == null) {
			...
			mList.setCellRenderer(new DefaultListCellRenderer() {
				...
				public Component getListCellRendererComponent(JList<?> list, Object value,
					...
					return super.getListCellRendererComponent(list, ((Pattern) value).getName(),
			...
			JMenuItem menuItemApplyPattern =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.apply"));
			menuItemApplyPattern.addActionListener(applyActionListener);
			menuHolder.addMenuItem(menuItemApplyPattern, "main/actions/apply");
			JMenuItem menuItemAddPattern =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.add"));
			menuItemAddPattern.addActionListener(addPatternActionListener);
			menuHolder.addMenuItem(menuItemAddPattern, "main/actions/add");
			JMenuItem menuItemPatternFromNodes =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.from_nodes"));
			...
			JMenuItem menuItemDuplicate =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.duplicate"));
			menuHolder.addMenuItem(menuItemDuplicate, "popup/duplicate");
			menuItemDuplicate.addActionListener(this::duplicatePattern);
			JMenuItem menuItemFromNodes =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.from_nodes"));
			...
			JMenuItem menuItemRemove =
					new JMenuItem(mController.getText("ManagePatternsPopupDialog.remove"));
			...
			jContentPane.add(mSplitPane,
					new GridBagConstraints(0, 0, 2, 1, 1.0, 8.0, GridBagConstraints.WEST,
							GridBagConstraints.BOTH, new Insets(0, 0, 0, 0), 0, 0));
			jContentPane.add(
					new ButtonBarBuilder().addGlue().addButton(getJCancelButton())
							.addButton(getJOKButton()).build(),
	...
	private void insertPatternFromNode(ActionEvent actionEvent) {
		writePatternBackToModel();
		setLastSelectedPattern(null);
		Pattern newPattern = StylePatternFactory
				.createPatternFromSelected(mController.getSelected(), mController.getSelecteds());
		newPattern.setName(searchForNameForNewPattern());
		int selectedIndex = mList.getSelectedIndex();
		if (selectedIndex < 0) {
			selectedIndex = mList.getModel().getSize();
