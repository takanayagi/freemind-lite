...
public class NodeNoteRegistration implements HookRegistration, MenuItemSelectedListener {
  public static final class SimplyHtmlResources implements UIResources {
    public String getString(String pKey) {
      ...
      resourceString = Resources.getInstance().getResourceString(pKey, null);
    ...
    public Icon getIcon(String pKey) {
      return Optional.ofNullable(Tools.removeTranslateComment(getString(pKey)))
          .map(iconPathString -> UIResources.class.getResource(iconPathString))
          .map(ImageIcon::new)
          .orElse(null);
    }
  }
  ...
  private final class NotesManager implements NodeSelectionListener, NodeLifetimeListener {
    ...
    public void onLostFocusNode(NodeView node) {
      getDocument().removeDocumentListener(mNoteDocumentListener);
      // store its content:
    ...
    public void onSaveNode(MindMapNode node) {
      ...
      editorContentEmpty =
          documentText.equals(NodeNote.EMPTY_EDITOR_STRING)
    ...
    public void onUpdateNodeHook(MindMapNode node) {
      ...
      if (node == controller.getSelected()
          && (HtmlTools.compareHtmlBodies(newText, getHtmlEditorPanel().getDocumentText()) != 0)) {
  ...
  /**
   * Indicates, whether or not the main panel has to be refreshed with new content. The typical
   * content will be empty, so this state is saved here.
   */
    ...
    if (noteIcon == null) {
      noteIcon =
          freemind.view.ImageFactory.getInstance()
              .createUnscaledIcon(Resources.getInstance().getResource("images/knotes.png"));
    }
    boolean showIcon = enabled;
    if (Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_DON_T_SHOW_NOTE_ICONS)) {
      showIcon = false;
    }
    node.setStateIcon(NodeNoteBase.NODE_NOTE_ICON, (showIcon) ? noteIcon : null);
    // tooltip, first try.
    if (!Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_DON_T_SHOW_NOTE_TOOLTIPS)) {
      controller.setToolTip(node, "nodeNoteText", (enabled) ? node.getNoteText() : null);
    }
  }

  /**
   * @return true, if the split pane is to be shown. E.g. when freemind was closed before, the state
   *     of the split pane was stored and is restored at the next start.
   */
  public boolean shouldShowSplitPane() {
    return "true".equals(controller.getProperty(FreeMind.RESOURCES_SHOW_NOTE_PANE));
  }
  ...
  public void register() {
    ...
    String keystroke =
        controller
            .getFrame()
            .getAdjustableProperty("keystroke_accessories/plugins/NodeNote_jumpto.keystroke.alt_N");
    noteViewerComponent
  ...
  public void showNotesPanel() {
    ...
    if ("true".equals(controller.getProperty(FreeMind.RESOURCES_USE_DEFAULT_FONT_FOR_NOTES_TOO))) {
      ...
      rule += "}\n";
      if ("true".equals(controller.getProperty(FreeMind.RESOURCES_USE_MARGIN_TOP_ZERO_FOR_NOTES))) {
        /*
         * this is used for paragraph spacing. I put it here, too, as the tooltip display
         * uses the same spacing. But it is to be discussed. fc, 23.3.2009.
         */
        rule += "p {";
        rule += "margin-top:0;";
        rule += "}\n";
    ...
    noteViewerComponent.setOpenHyperlinkHandler(
        new ActionListener() {
    ...
    controller
        .getController()
        .insertComponentIntoSplitPane(southPanel, SplitComponentType.NOTE_PANEL);
  ...
  private String normalizeString(String input) {
    if (input == null) input = NodeNote.EMPTY_EDITOR_STRING;
    // return null;
