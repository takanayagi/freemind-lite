...
public class NodeNoteRegistration implements HookRegistration, MenuItemSelectedListener {
	...
	/**
	 * @return true, if the split pane is to be shown. E.g. when freemind was closed before, the
	 *         state of the split pane was stored and is restored at the next start.
	 */
	...
	public void register() {
		...
		String keystroke = controller.getFrame().getAdjustableProperty(
	...
	public void showNotesPanel() {
		...
		if ("true".equals(
				controller.getProperty(FreeMind.RESOURCES_USE_DEFAULT_FONT_FOR_NOTES_TOO))) {
			...
			if (Resources.getInstance()
				...
				defaultFont = Tools.updateFontSize(defaultFont,
						this.getMindMapController().getView().getZoom(), defaultFont.getSize());
			}
			String rule = "BODY {";
			...
			rule += "}\n";
			if ("true".equals(
					controller.getProperty(FreeMind.RESOURCES_USE_MARGIN_TOP_ZERO_FOR_NOTES))) {
				/*
		...
		controller.getController().insertComponentIntoSplitPane(southPanel,
				SplitComponentType.NOTE_PANEL);
