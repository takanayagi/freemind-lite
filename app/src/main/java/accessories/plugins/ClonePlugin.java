...
public class ClonePlugin extends PermanentMindMapNodeHookAdapter
    implements NodeLifetimeListener, ClonePropertiesObserver {
  ...
  /**
   * This is the master list. {@link ClonePlugin#mCloneNodes mCloneNodes} is derived from it. It
   * contains id strings.
   */
  private HashSet<String> mCloneNodeIds;

  /**
   * Includes the original node. This is a cached list with the MindMapNodes belonging to the {@link
   * ClonePlugin#mCloneNodeIds mCloneNodeIds}.
   */
  ...
  private void disablePlugin() {
    ...
    getMindMapController()
        .getController()
        .errorMessage(getMindMapController().getText("clone_plugin_impossible"));
  ...
  protected void removeHook() {
    // first deactivate cloning for this node (otherwise, the deactivation will be
    // cloned, too!)
    deregisterCloning();
  ...
  protected String getCloneItselfValue() {
    return mClonePropertiesHolder.isCloneItself() ? CLONE_ITSELF_TRUE : CLONE_ITSELF_FALSE;
  }
  ...
  public void loadFrom(XMLElement child) {
    ...
    if (values.containsKey(XML_STORAGE_CLONE_ITSELF)) {
      mCloneItself =
          Boolean.valueOf(
              Tools.safeEquals(CLONE_ITSELF_TRUE, values.get(XML_STORAGE_CLONE_ITSELF)));
    } else {
  ...
  private void registerPlugin() {
    if (mDisabled) return;
    /*
     * test for error cases: - orig is child of clone now - if clone is a child of clone, this
     * is here not reachable, as the plugin remains active and is not newly invoked. Hmm, what
     * to do?
     */
    MindMapNode originalNode = getNode();
    HashSet<MindMapNode> cloneNodes = getCloneNodes();
    logger.fine(
        "Invoke shadow class with orig: "
            + printNodeId(originalNode)
            + " and clones "
    ...
    addClone(getNode());
    mClonePropertiesHolder = registration.getCloneProperties(mCloneId);
  ...
  private void checkForChainError(
      MindMapNode originalNode, MindMapNode node, MindMapNode cloneNode) {
  ...
  public void removeClone(MindMapNode pCloneNode) {
    ...
    if (mCloneNodeIds.isEmpty()
        || (mCloneNodeIds.size() == 1 && mCloneNodeIds.contains(getNodeId()))) {
      // remove myself
   ...
   * (non-Javadoc)
   *
   * @see freemind.extensions.PermanentNodeHookAdapter#processUnfinishedLinks()
   */
  public void processUnfinishedLinks() {
    ...
    for (MindMapNode cloneNode : cloneNodes) {
      ...
      if (hook == null && cloneNode != null) {
        ...
        getMindMapController().addHook(cloneNode, selecteds, PLUGIN_LABEL, hookProperties);
      }
    }
  }
