...
public class AutomaticLayout extends PermanentMindMapNodeHookAdapter {
	...
	public static class Registration implements HookRegistration {
		...
		public void register() {
			...
			mAutomaticLayoutPropertyContributor =
					new AutomaticLayoutPropertyContributor(modeController);
	...
	/**
	 * Currently not used. Is useful if you want to make single patterns changeable.
	 */
	public static class StylePatternProperty extends PropertyBean
			implements PropertyControl, ActionListener {
		...
		public StylePatternProperty(String description, String label, TextTranslator pTranslator,
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			ChooseFormatPopupDialog formatDialog = new ChooseFormatPopupDialog(
					mindMapController.getFrame().getJFrame(), mindMapController,
					"accessories/plugins/AutomaticLayout.properties_StyleDialogTitle", pat, null);
	...
	public static class StylePatternListProperty extends PropertyBean
			implements PropertyControl, ListSelectionListener {
		...
		public void setValue(String value) {
			...
			int j = 1;
			StylePropertyTranslator stylePropertyTranslator =
					new StylePropertyTranslator(mindMapController);
			for (Pattern pattern : resultPatterns.getListChoiceList()) {
	...
	private static final class AutomaticLayoutPropertyContributor
		...
		public List<PropertyControl> getControls(TextTranslator pTextTranslator) {
			...
			controls.add(new StylePatternListProperty("level", AUTOMATIC_FORMAT_LEVEL,
					pTextTranslator, modeController));
			return controls;
		}
	}
	...
	private void setStyle(MindMapNode node) {
		logger.finest("updating node id=" + node.getObjectId(getMindMapController()) + " and text:"
				+ node);
		...
		if (depth < patterns.sizeChoiceList())
			myIndex = depth;
		Pattern p = patterns.getChoice(myIndex);
	...
	private void reloadPatterns() {
		if (patterns == null) {
			String property = getMindMapController().getFrame().getProperty(AUTOMATIC_FORMAT_LEVEL);
