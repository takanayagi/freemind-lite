...
abstract public class NodeViewLayoutAdapter implements NodeViewLayout {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (component instanceof NodeView child) {
				if (child.isLeft() == isLeft) {
					...
					if (childShift < 0 || isFirstNodeView) { // FIXME: isFirstNodeView is always false.
						shift += childShift;
	...
	protected void placeRightChildren(int childVerticalShift) {
		...
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (componentC instanceof NodeView component) {
				...
				// logger.info("Place of child " + component.getModel().getText() + ": " +
				// child.getLocation());
		...
		int bottom = getContent().getY() + getContent().getHeight() + getSpaceAround();

		if (child != null) {
			getView().setSize(right, Math.max(bottom,
					child.getY() + child.getHeight() + child.getAdditionalCloudHeigth() / 2));
		} else {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see freemind.view.mindmapview.NodeViewLayout#layoutNodeFoldingComponent(freemind.
	 * view.mindmapview.NodeFoldingListenerView)
	 */
