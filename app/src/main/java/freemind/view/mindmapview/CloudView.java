...
public class CloudView {
	...
	public void paint(Graphics graphics) {
		...
		/*
		 * calculate the distances between two points on the convex hull depending on the
		 * getIterativeLevel().
		 */
			...
			if (length > distanceBetweenPoints) {
				for (int j = 0; j < length / distanceBetweenPoints - 1; ++j) {
					if ((j + 2) * distanceBetweenPoints < length) {
						x3 = x0 + (j + 1) * distanceBetweenPoints
								* dxn; /*
	...
	private void paintClouds(Graphics2D g, Graphics2D gstroke, double x0, double y0, double x1,
			double y1, double distanceToConvexHull) {
	...
	private double getDistanceToConvexHull() {
		return 40.0 / (getIterativeLevel() + 1) * getZoom();
	}
