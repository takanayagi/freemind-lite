...
public class EditNodeTextField extends EditNodeBase {
	...
	public EditNodeTextField(final NodeView node, final String text, final KeyEvent firstEvent,
			ModeController controller, EditControl editControl, JComponent pParent,
	...
	public void show() {
		...
		textfield =
				(getText().length() < 8) ? new JTextField(getText(), 8) : new JTextField(getText());
		...
		int xExtraWidth = 0;
		if (MINIMAL_LEAF_WIDTH > xSize && (model.isFolded() || !model.hasChildren())) {
		...
		textfield.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_Z,
		...
		textfield.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_Y,
		...
		Tools.convertPointToAncestor(nodeView.getMainView(), mPoint, mapView);
		if (xExtraWidth < 0) {
	...
	class TextFieldListener
		...
		public void focusGained(FocusEvent e) {} // focus gained
