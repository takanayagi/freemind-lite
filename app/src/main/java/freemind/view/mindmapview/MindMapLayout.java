...
public class MindMapLayout implements LayoutManager {
  ...
  static final int BORDER = 30; // width of the border around the map.
  ...
  public void layoutContainer(Container c) {
    ...
    mapView.setSize(
        calcXBorderSize * 2 + getRoot(mapView).getWidth(),
  ...
  public Dimension preferredLayoutSize(Container c) {
    ...
    return new Dimension(
        ...
        2 * calcYBorderSize(mapView) + preferredSize.height);
  }
  ...
  private int calcXBorderSize(MapView map) {
    int xBorderSize;
    Dimension visibleSize = map.getViewportSize();
    final int minBorderWidth =
        map.getZoomed(MindMapLayout.BORDER + MindMapLayout.MINIMAL_LEAF_WIDTH);
    if (visibleSize != null) {
