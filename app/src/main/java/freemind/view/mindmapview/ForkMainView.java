...
class ForkMainView extends MainView {
	...
	Point getLeftPoint() {
		int edgeWidth = getEdgeWidth();
		return new Point(0, getHeight() - edgeWidth / 2 - 1);
	}
	...
	protected int getEdgeWidth() {
		...
		switch (edge.getStyleAsInt()) {
			...
			case EdgeAdapter.INT_EDGESTYLE_SHARP_LINEAR:
				...
				edgeWidth = 1;
				for (Iterator<? extends MindMapNode> it = nodeModel.childrenUnfolded(); it.hasNext();) {
	...
	Point getRightPoint() {
		int edgeWidth = getEdgeWidth();
		return new Point(getWidth() - 1, getHeight() - edgeWidth / 2 - 1);
	}
