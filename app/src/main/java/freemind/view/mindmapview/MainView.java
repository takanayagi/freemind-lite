...
public abstract class MainView extends JLabel {
  ...
  public Dimension getPreferredSize() {
    ...
    boolean isEmpty =
        text.isEmpty()
            || (HtmlTools.isHtmlNode(text)
                && !text.contains("<img")
                && HtmlTools.htmlToPlain(text).isEmpty());
    if (isEmpty) {
      setText("!");
    }
  ...
  public void setDraggedOver(Point p) {
    setDraggedOver(
        (dropAsSibling(p.getX())) ? NodeView.DRAGGED_OVER_SIBLING : NodeView.DRAGGED_OVER_SON);
  }
  ...
  /**
   * Determines whether or not the xCoord is in the part p of the node: if node is on the left: part
   * [1-p,1] if node is on the right: part[ 0,p] of the total width.
   */
