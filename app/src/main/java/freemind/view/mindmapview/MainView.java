...
public abstract class MainView extends JLabel {
	...
	public Dimension getPreferredSize() {
		...
		boolean isEmpty = text.isEmpty() || (HtmlTools.isHtmlNode(text)
				&& !text.contains("<img") && HtmlTools.htmlToPlain(text).isEmpty());
		if (isEmpty) {
			setText("!");
		}
		...
		prefSize.width =
				Math.max(getNodeView().getMap().getZoomed(MIN_HOR_NODE_SIZE), prefSize.width);
		if (isEmpty) {
			setText("");
		}
	...
	public void paintDragOver(Graphics2D graphics) {
		if (isDraggedOver == NodeView.DRAGGED_OVER_SON) {
			if (getNodeView().isLeft()) {
				graphics.setPaint(new GradientPaint(getWidth() * 3 / 4F, 0,
						getNodeView().getMap().getBackground(), getWidth() / 4F, 0,
						NodeView.dragColor));
				graphics.fillRect(0, 0, getWidth() * 3 / 4, getHeight() - 1);
			} else {
				graphics.setPaint(
						new GradientPaint(getWidth() / 4F, 0, getNodeView().getMap().getBackground(),
								getWidth() * 3 / 4F, 0, NodeView.dragColor));
				graphics.fillRect(getWidth() / 4, 0, getWidth() - 1, getHeight() - 1);
			}
		}

		if (isDraggedOver == NodeView.DRAGGED_OVER_SIBLING) {
			graphics.setPaint(new GradientPaint(0, getHeight() * 3 / 5F,
					getNodeView().getMap().getBackground(), 0, getHeight() / 5F,
					NodeView.dragColor));
			graphics.fillRect(0, 0, getWidth() - 1, getHeight() - 1);
		}
	}
	...
	/**
	 * Determines whether or not the xCoord is in the part p of the node: if node is on the left:
	 * part [1-p,1] if node is on the right: part[ 0,p] of the total width.
	 */
