...
public class SharpBezierEdgeView extends EdgeView {
	...
	private void update() {
		...
		line2.setCurve(end.x, end.y, two.x, two.y + w2, one.x + deltaX, one.y + deltaY,
	...
	protected void createStart() {
		if (source.isRoot()) {
			...
			final double w = mainView.getWidth() / 2.0;
			...
			if (w2 == x02) {
				...
				final double delta = getMap().getZoom() * (getWidth() / 2.0 + 1);
