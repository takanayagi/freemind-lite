...
public class ArrowLinkView {
  ...
  public Rectangle getBounds() {
    if (arrowLinkCurve == null) return new Rectangle();
  ...
  public void paint(Graphics graphics) {
    ...
    if (p1 != null) {
      p3 = new Point(p1);
      p3.translate(
          ((sourceIsLeft) ? -1 : 1) * getMap().getZoomed(arrowLinkModel.getStartInclination().x),
    ...
    if (p2 != null) {
      p4 = new Point(p2);
      p4.translate(
          ((targetIsLeft) ? -1 : 1) * getMap().getZoomed(arrowLinkModel.getEndInclination().x),
