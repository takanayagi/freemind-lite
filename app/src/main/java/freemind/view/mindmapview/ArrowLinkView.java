...
public class ArrowLinkView {
	...
	public void paint(Graphics graphics) {
		...
		if (arrowLinkModel.getEndInclination() == null
			...
			double dellength =
					isSourceVisible() && isTargetVisible() ? p1.distance(p2) / getZoom() : 30;
		...
		if (p1 != null) {
			...
			p3.translate(
					((sourceIsLeft) ? -1 : 1)
							* getMap().getZoomed(arrowLinkModel.getStartInclination().x),
	...
	private Point calcInclination(NodeView node, double dellength) {
		/*
		 * int w = node.getWidth(); int h = node.getHeight(); double r = Math.sqrt(w*w+h*h); double
		 * wr = dellength * w / r; double hr = dellength * h / r; return new Point((int)wr,
		 * (int)hr);
		 */
	...
	/**
	 * Determines, whether or not a given point p is in an epsilon-neighbourhood for the cubic
	 * curve.
	 */
	public boolean detectCollision(Point p) {
		...
		FlatteningPathIterator pi = new FlatteningPathIterator(arrowLinkCurve.getPathIterator(null),
				MAXIMAL_RECTANGLE_SIZE_FOR_COLLISION_DETECTION / 4.0,
				10/*
					 * =maximal 2^10=1024 points.
					 */);
	...
	protected Rectangle2D getControlPoint(Point2D p) {
		...
		return new Rectangle2D.Double(p.getX() - side / 2.0, p.getY() - side / 2.0, side, side);
	}
