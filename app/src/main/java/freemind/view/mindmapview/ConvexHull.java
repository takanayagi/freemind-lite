...
public class ConvexHull {
  ...
  protected static class thetaComparator implements Comparator<Point> {
    ...
    public int compare(Point p1, Point p2) {
      ...
      return Integer.compare(0, comp2);
    }
  ...
  Vector<Point> doGraham(Vector<Point> p) {
    ...
    // search for minimum:
    for (i = 1; i < p.size(); ++i) {
      if (p.get(i).y < p.get(min).y) min = i;
    }
    ...
    p.set(0, p.get(min));
    p.set(min, t);
    thetaComparator comp = new thetaComparator(p.get(0));
    p.sort(comp);
    p.add(0, new Point(p.get(p.size() - 1))); // the first is the
    // last.
    M = 3;
    for (i = 4; i < p.size(); ++i) {
      while (ccw(p.get(M), p.get(M - 1), p.get(i)) >= 0) {
        M--;
      }
      M++;
      // swap:
  ...
  public Vector<Point> calculateHull(LinkedList<Point> coordinates) {
    Vector<Point> q = new Vector<>(coordinates);
    return doGraham(q);
  }
}
