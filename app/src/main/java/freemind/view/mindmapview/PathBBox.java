...
class PathBBox {
    ...
    for (PathIterator pi = s.getPathIterator(null); !pi.isDone(); pi.next()) {
      switch (pi.currentSegment(coords)) {
        ...
        case PathIterator.SEG_CUBICTO:
          ...
          for (int i = 0; i < num; i++) {
            accumCubic(bounds, coords[i], curx, cury, cpx0, cpy0, cpx1, cpy1, endx, endy);
          }
  ...
  private static int findCubicZeros(
      double[] zeros, double cur, double cp0, double cp1, double end) {
  ...
  private static void accumCubic(
      double[] bounds,
      double t,
      double curx,
      double cury,
    ...
    double u = (1 - t);
    double x =
        curx * u * u * u + 3.0 * cpx0 * t * u * u + 3.0 * cpx1 * t * t * u + endx * t * t * t;
    double y =
        cury * u * u * u + 3.0 * cpy0 * t * u * u + 3.0 * cpy1 * t * t * u + endy * t * t * t;
    accum(bounds, x, y);
  }
}
