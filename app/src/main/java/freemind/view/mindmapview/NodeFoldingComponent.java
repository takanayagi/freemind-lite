...
public class NodeFoldingComponent extends JButton {
	...
	public NodeFoldingComponent(NodeView view) {
		...
		boolean mIsEnabled = Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_DISPLAY_FOLDING_BUTTONS);
		if (mIsEnabled) {
	...
	public void setCorrectedLocation(Point p) {
		...
		setLocation(p.x - xCorrection,
				p.y - zoomedCircleRadius * SIZE_FACTOR_ON_MOUSE_OVER);
	}
