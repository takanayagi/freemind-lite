...
public class NodeFoldingComponent extends JButton {
  ...
  public NodeFoldingComponent(NodeView view) {
    ...
    boolean mIsEnabled =
        Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_DISPLAY_FOLDING_BUTTONS);
    if (mIsEnabled) {
      addMouseListener(
          new MouseListener() {
      ...
      mTimer = new Timer(TIMER_DELAY, taskPerformer);
    }
  }
  ...
  class RoundImageButtonUI extends BasicButtonUI {
    ...
    public void paint(Graphics g, JComponent c) {
      ...
      initShape(c);
      // Border
      Object oldRenderingHint = nodeView.getMap().setEdgesRenderingHint(g2);
      ...
      if (b.mIsEntered) {
        ...
        Color oldColor = g2.getColor();
        if (mColorCounter != 0) {
          int diameter = bounds.width * mColorCounter / COLOR_COUNTER_MAX;
          if (isFolded()) {
          ...
          if (isFolded()) {
            ...
            g2.setColor(nodeView.getMap().getBackground());
            g2.fillOval(
                xmiddle - radius, ymiddle - radius, foldingCircleDiameter, foldingCircleDiameter);
            g2.setColor(lineColor);
            g2.drawOval(
                xmiddle - radius, ymiddle - radius, foldingCircleDiameter, foldingCircleDiameter);
            g2.setColor(oldColor);
  ...
  public void setCorrectedLocation(Point p) {
    ...
    int xCorrection =
        (int) (zoomedCircleRadius * (SIZE_FACTOR_ON_MOUSE_OVER + ((left) ? +1f : -1f)));
    setLocation(p.x - xCorrection, p.y - zoomedCircleRadius * SIZE_FACTOR_ON_MOUSE_OVER);
  }
