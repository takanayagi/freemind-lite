...
public class MapView extends JPanel implements ViewAbstraction, Printable, Autoscroll {
	...
	/**
	 * Currently, this listener does nothing. But it should move the map according to the resize
	 * event, such that the current map's center stays at the same location (seen relative).
	 */
	private final class ResizeListener extends ComponentAdapter {
		...
		public void componentResized(ComponentEvent pE) {
			logger.fine(
					"Component resized " + pE + " old size " + mSize + " new size " + getSize());
	...
	static public class ScrollPane extends JScrollPane {
		...
		protected boolean processKeyBinding(KeyStroke pKs, KeyEvent pE, int pCondition,
				boolean pPressed) {
			/*
			 * the scroll pane eats control page up and down. Moreover, the page up and down itself
			 * is not very useful, as the map hops away too far.
			 */
	...
	public MapView(MindMap model, ViewFeedback pFeedback) {
		...
		if (standardNodeTextColor == null) {
			...
			try {
				String stdtextcolor =
						mFeedback.getProperty(FreeMind.RESOURCES_SELECTED_NODE_RECTANGLE_COLOR);
			...
			try {
				String drawCircle =
						mFeedback.getProperty(FreeMind.RESOURCE_DRAW_RECTANGLE_FOR_SELECTION);
			...
			try {
				String printOnWhite =
						mFeedback.getProperty(FreeMind.RESOURCE_PRINT_ON_WHITE_BACKGROUND);
			...
			propertyChangeListener.propertyChanged(FreeMindCommon.RESOURCE_ANTIALIAS,
					mFeedback.getProperty(FreeMindCommon.RESOURCE_ANTIALIAS), null);
		}
	...
	public void initRoot() {
		...
		rootView =
				NodeViewFactory.getInstance().newNodeView(getModel().getRootNode(), 0, this, this);
		rootView.insert();
		revalidate();
	}
	...
	public int getMaxNodeWidth() {
		if (maxNodeWidth == 0) {
			try {
				...
				maxNodeWidth =
						Integer.parseInt(mFeedback.getProperty("el__max_default_window_width"));
			}
		}
		return maxNodeWidth;
	}
	...
	/**
	 * Problem: Before scrollRectToVisible is called, the node has the location (0,0), ie. the
	 * location first gets calculated after the scrollpane is actually scrolled. Thus, as a
	 * workaround, I simply call scrollRectToVisible twice, the first time the location of the node
	 * is calculated, the second time the scrollPane is actually scrolled.
	 */
	...
	/**
	 * Scroll the viewport of the map to the south-west, i.e. scroll the map itself to the
	 * north-east.
	 */
	...
	protected void setViewPosition(Point currentPoint) {
		if (getParent() instanceof JViewport mapViewport) {
	...
	private NodeView getVisibleLeft(NodeView oldSelected) {
		...
		if (oldSelected.getModel().isRoot()) {
			...
			if (oldSelected.getModel().isFolded()) {
				getViewFeedback().setFolded(oldSelected.getModel(), false);
				return oldSelected;
			}
	...
	private void extendSelectionWithKeyMove(NodeView newlySelectedNodeView, KeyEvent e) {
		if (e.isShiftDown()) {
			...
			if (e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_RIGHT) {
				shiftSelectionOrigin = null;
				NodeView toBeNewSelected =
						newlySelectedNodeView.isParentOf(getSelected()) ? newlySelectedNodeView
								: getSelected();
	...
	/**
	 * Select the node and his descendants. On extend = false clear up the previous selection. if
	 * extend is false, the past selection will be empty. if yes, the selection will extended with
	...
	public void setMoveCursor(boolean isHand) {
		int requiredCursor =
				(isHand && !disableMoveCursor) ? Cursor.MOVE_CURSOR : Cursor.DEFAULT_CURSOR;
	...
	public ArrayList<MindMapNode> getSelectedNodesSortedByY() {
		...
		iteration: for (int i = 0; i < selected.size(); i++) {
			...
			for (MindMapNode parent = node.getParentNode(); parent != null; parent =
	...
	/**
	 * @return an ArrayList of MindMapNode objects. If both ancestor and descandant node are
	 *         selected, only the ancestor ist returned
	 */
	...
	public void paint(Graphics g) {
		...
		final Object newRenderingHintFM =
				getZoom() != 1F ? RenderingHints.VALUE_FRACTIONALMETRICS_ON
		...
		logger.fine("End paint of " + getModel().getRestorable() + " in " + localTime
				+ ". Mean time:" + (mPaintingTime / mPaintingAmount));
	}
	...
	private void paintSelecteds(Graphics2D g) {
		...
		for (NodeView selected : getSelecteds()) {
	...
	private void paintSelected(Graphics2D g, NodeView selected) {
		...
		g.drawRoundRect(contentLocation.x - arcWidth, contentLocation.y - arcWidth,
				content.getWidth() + 2 * arcWidth, content.getHeight() + 2 * arcWidth, 15, 15);
	}
	...
	protected void paintLinks(NodeView source, Graphics2D graphics,
		...
		// paint own labels:
		Vector<MindMapLink> vec = getModel().getLinkRegistry().getAllLinks(source.getModel());
		for (MindMapLink mindMapLink : vec) {
			if (LinkAlreadyVisited.add(mindMapLink)) {
				// determine type of link
				if (mindMapLink instanceof MindMapArrowLink) {
					ArrowLinkView arrowLink = new ArrowLinkView((MindMapArrowLink) mindMapLink,
							getNodeView(mindMapLink.getSource()), getNodeView(mindMapLink.getTarget()));
	...
	public MindMapArrowLink detectCollision(Point p) {
		if (mArrowLinkViews == null)
			return null;
		for (ArrowLinkView mArrowLinkView : mArrowLinkViews) {
			if (mArrowLinkView.detectCollision(p))
				return mArrowLinkView.getModel();
		}
		return null;
	}

	/**
	 * Call preparePrinting() before printing and endPrinting() after printing to minimize
	 * calculation efforts
	 */
	...
	private void repaintSelecteds() {
		for (NodeView next : getSelecteds()) {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex) {
		...
		try {
			userZoomFactor = Double.parseDouble(mFeedback.getProperty("user_zoom"));
		} catch (Exception e) {
			// freemind.main.Resources.getInstance().logException(e);
		}
		...
		try {
			...
			if (fitToPage) {
				...
				double zoomFactorY =
						pageFormat.getImageableHeight() / boundingRectangle.getHeight();
				zoomFactor = Math.min(zoomFactorX, zoomFactorY);
			} else {
				zoomFactor = userZoomFactor;

				int nrPagesInWidth = (int) Math.ceil(
						zoomFactor * boundingRectangle.getWidth() / pageFormat.getImageableWidth());
				int nrPagesInHeight = (int) Math.ceil(zoomFactor * boundingRectangle.getHeight()
						/ pageFormat.getImageableHeight());
				if (pageIndex >= nrPagesInWidth * nrPagesInHeight) {
					return Printable.NO_SUCH_PAGE;
				}
				int yPageCoord = (int) (double) (pageIndex / nrPagesInWidth);
	...
	/**
	 * For nodes, they can ask, whether or not the width must be bigger to prevent the "..." at the
	 * output. (Bug of java).
	 */
	...
	public Rectangle getInnerBounds() {
		...
		for (ArrowLinkView mArrowLinkView : mArrowLinkViews) {
			final CubicCurve2D arrowLinkCurve = mArrowLinkView.arrowLinkCurve;
	...
	public void autoscroll(Point cursorLocn) {
		Rectangle r = new Rectangle((int) cursorLocn.getX() - margin,
				(int) cursorLocn.getY() - margin, 1 + 2 * margin, 1 + 2 * margin);
	...
	public void acceptViewVisitor(MindMapNode pNode, NodeViewVisitor visitor) {
		for (NodeView nodeView : getViewers(pNode)) {
			visitor.visit(nodeView);
		}
