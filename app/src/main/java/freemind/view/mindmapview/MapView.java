...
public class MapView extends JPanel implements ViewAbstraction, Printable, Autoscroll {
  ...
  public static class ScrollPane extends JScrollPane {
    ...
    protected boolean processKeyBinding(
        KeyStroke pKs, KeyEvent pE, int pCondition, boolean pPressed) {
      /*
   ...
   * workaround, I simply call scrollRectToVisible twice, the first time the location of the node is
    ...
    if (e.isShiftDown()) {
      ...
      if (e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_RIGHT) {
        ...
        NodeView toBeNewSelected =
            newlySelectedNodeView.isParentOf(getSelected()) ? newlySelectedNodeView : getSelected();
      ...
      if (e.getKeyCode() == KeyEvent.VK_PAGE_UP) {
        for (; ; ) {
          ...
          if (currentSelectedY > selectionOriginY) deselect(currentSelected);
          else makeTheSelected(currentSelected);
      ...
      boolean enlargingMove =
          (deltaY > 0) && (e.getKeyCode() == KeyEvent.VK_DOWN)
  ...
  public boolean selectContinuous(NodeView newSelected) {
    ...
    while (i.hasNext()) {
      ...
      if ((nodeView.isLeft() == oldPositionLeft || nodeView.isLeft() == newPositionLeft)) {
        if (isSelected(nodeView)) deselect(nodeView);
        else break;
      }
    }
    /* other direction. */
    ...
    while (i.hasNext()) {
      ...
      if ((nodeView.isLeft() == oldPositionLeft || nodeView.isLeft() == newPositionLeft)
          && !isSelected(nodeView)
          && nodeView.isContentVisible()) toggleSelected(nodeView);
      if (nodeView == newSelected || nodeView == oldSelected) {
        break;
      }
    }
  ...
  public NodeView getSelected() {
    if (selected.size() > 0) return selected.get(0);
    else return null;
  }
  ...
  public ArrayList<MindMapNode> getSelectedNodesSortedByY() {
    ...
    for (int i = 0; i < selected.size(); i++) {
      ...
      for (MindMapNode parent = node.getParentNode();
          parent != null;
  ...
  /**
   * @return an ArrayList of MindMapNode objects. If both ancestor and descandant node are selected,
   *     only the ancestor ist returned
   */
  ...
  private void paintSelected(Graphics2D g, NodeView selected) {
    ...
    g.drawRoundRect(
        ...
        contentLocation.y - arcWidth,
        content.getWidth() + 2 * arcWidth,
        content.getHeight() + 2 * arcWidth,
        15,
        15);
  }
  ...
  protected void collectLabels(NodeView source, HashMap<String, NodeView> labels) {
    ...
    if (getModel().getLinkRegistry() == null) return;
    // apply own label:
    String label = getModel().getLinkRegistry().getLabel(source.getModel());
    if (label != null) labels.put(label, source);
    for (NodeView target : source.getChildrenViews()) {
      collectLabels(target, labels);
    }
  }

  protected void paintLinks(
      NodeView source,
      Graphics2D graphics,
      HashMap<String, NodeView> labels,
      HashSet<MindMapLink> LinkAlreadyVisited) {
    // check for existing registry:
    if (getModel().getLinkRegistry() == null) return;
    if (LinkAlreadyVisited == null) LinkAlreadyVisited = new HashSet<>();
    // references first
    ...
    for (MindMapLink mindMapLink : vec) {
      if (LinkAlreadyVisited.add(mindMapLink)) {
        ...
        if (mindMapLink instanceof MindMapArrowLink) {
          ArrowLinkView arrowLink =
              new ArrowLinkView(
                  (MindMapArrowLink) mindMapLink,
                  getNodeView(mindMapLink.getSource()),
                  getNodeView(mindMapLink.getTarget()));
  ...
  public MindMapArrowLink detectCollision(Point p) {
    ...
    for (ArrowLinkView mArrowLinkView : mArrowLinkViews) {
      if (mArrowLinkView.detectCollision(p)) return mArrowLinkView.getModel();
    }
    return null;
  }

  /**
   * Call preparePrinting() before printing and endPrinting() after printing to minimize calculation
   * efforts
   */
    ...
    try {
      ...
      if (fitToPage) {
        ...
        int nrPagesInHeight =
            (int)
                Math.ceil(
                    zoomFactor * boundingRectangle.getHeight() / pageFormat.getImageableHeight());
        ...
        graphics2D.translate(
            -pageFormat.getImageableWidth() * xPageCoord,
  ...
  /**
   * Return the bounding box of all the descendants of the source view, that without BORDER. Should
   * that be implemented in LayoutManager as minimum size?
   */
  ...
  public void autoscroll(Point cursorLocn) {
    Rectangle r =
        new Rectangle(
            (int) cursorLocn.getX() - margin,
            (int) cursorLocn.getY() - margin,
            1 + 2 * margin,
            1 + 2 * margin);
