...
public class NodeView extends JComponent implements TreeModelListener {
		...
		if (sListener == null) {
			sListener = (pPropertyName, pNewValue, pOldValue) -> {
				if (Tools.safeEquals(pPropertyName, FreeMind.TOOLTIP_DISPLAY_TIME)) {
					...
					ToolTipManager.sharedInstance().setDismissDelay(Resources.getInstance()
							.getIntProperty(FreeMind.TOOLTIP_DISPLAY_TIME, 4000));
				}
			};
	...
	private void getCoordinates(LinkedList<Point> inList, int additionalDistanceForConvexHull,
			boolean byChildren, int transX, int transY) {
		if (!isVisible())
			return;
	...
	//
	/**
	 * The algorithm should be here the following (see Eclipse editor): Selected is the n-th node
	 * from above. Look for the last node visible on the screen and make this node the first one.
	 * Now select the n-th node from above.
	 * 
	 * Easier idea to implement: Store node y position as y0. Search for a node with the same parent
	 * with y position y0+height Scroll the window by height.
	 */
	...
	protected NodeView getNextVisibleSibling() {
		...
		while (sibling.getModel().getNodeLevel() < getMap().getSiblingMaxLevel()) {
			// can we drill down?
			NodeView first = sibling.getFirst(sibling.isRoot() ? lastSibling : null, this.isLeft(),
					!this.isLeft());
			if (first == null) {
				break;
			}
	...
	protected NodeView getPreviousVisibleSibling() {
		...
		// untill: leaf, closed node, max level
		while (sibling.getModel().getNodeLevel() < getMap().getSiblingMaxLevel()) {
			NodeView last = sibling.getLast(sibling.isRoot() ? previousSibling : null,
	...
	NodeView insert(MindMapNode newNode, int position) {
		NodeView newView =
				NodeViewFactory.getInstance().newNodeView(newNode, position, getMap(), this);
		newView.insert();
		return newView;
	}
	...
	private void updateText() {
		...
		if (!isHtml) {
			...
			for (String s : lines) {
				...
				widthMustBeRestricted = mainView.getPreferredSize().width > mapView
						.getZoomed(mapView.getMaxNodeWidth()) + mainView.getIconWidth();
		...
		if (isHtml) {
			...
			if (widthMustBeRestricted) {
				nodeText = nodeText.replaceFirst("(?i)<body>",
						"<body width=\"" + mapView.getMaxNodeWidth() + "\">");
			}
			setText(nodeText);
	...
	private void updateIcons() {
		...
		if (SHOW_ATTRIBUTE_ICON && (getModel().getAttributeTableLength() > 0)) {
			if (sAttributeIcon == null) {
				sAttributeIcon = freemind.view.ImageFactory.getInstance().createUnscaledIcon(
		...
		if (link != null) {
			...
			ImageIcon icon = freemind.view.ImageFactory.getInstance()
	...
	public int getMaxToolTipWidth() {
		if (maxToolTipWidth == 0) {
			try {
				maxToolTipWidth =
						Integer.parseInt(getViewFeedback().getProperty("max_tooltip_width"));
	...
	public int getZoomedFoldingSymbolHalfWidth() {
		int preferredFoldingSymbolHalfWidth =
				(int) ((getFoldingSymbolWidth() * mapView.getZoom()) / 2);
		return Math.min(preferredFoldingSymbolHalfWidth, getHeight() / 2);
	}

	/**
	 * @return the left/right point of the folding circle. To receive its center, the amount has to
	 *         be moved to left/right (depending on its side) by the folding circle width.
	 */
