 ...
 * Daniel Polansky, Christian Foltin, Dimitri Polivaev and others. See COPYING for Details
 *
...
public class NodeView extends JComponent implements TreeModelListener {
  ...
  protected static final Color dragColor = Color.lightGray; // the Color of
  // appearing
  // GradientBox
  // on
  // drag over
  ...
  protected NodeView(MindMapNode model, int position, MapView map, Container parent) {
    ...
    if (sListener == null) {
      sListener =
          (pPropertyName, pNewValue, pOldValue) -> {
            if (Tools.safeEquals(pPropertyName, FreeMind.TOOLTIP_DISPLAY_TIME)) {
              // control tooltip times:
              ToolTipManager.sharedInstance()
                  .setDismissDelay(
                      Resources.getInstance().getIntProperty(FreeMind.TOOLTIP_DISPLAY_TIME, 4000));
            }
          };
  ...
  protected void addFoldingListener() {
    if (mFoldingListener == null && getModel().hasVisibleChilds() && !getModel().isRoot()) {
      ...
      mFoldingListener.addActionListener(
          pE -> getViewFeedback().setFolded(getModel(), !getModel().isFolded()));
    }
  }
  ...
  void setMainView(MainView newMainView) {
    ...
    if (!model.isRoot()
        && "true"
            .equals(mapView.getViewFeedback().getProperty(FreeMindMain.ENABLE_NODE_MOVEMENT))) {
  ...
  /**
   * Returns the coordinates occupied by the node and its children as a vector of four point per
   * node.
   */
  ...
  private void getCoordinates(
    ...
    if (isContentVisible()) {
      ...
      inList.addLast(
          new Point(-additionalDistanceForConvexHull + x, -additionalDistanceForConvexHull + y));
      inList.addLast(
          new Point(
              -additionalDistanceForConvexHull + x, additionalDistanceForConvexHull + y + height));
      inList.addLast(
          new Point(
              additionalDistanceForConvexHull + x + width,
              additionalDistanceForConvexHull + y + height));
      inList.addLast(
          new Point(
              additionalDistanceForConvexHull + x + width, -additionalDistanceForConvexHull + y));
    }
    ...
    for (NodeView child : childrenViews) {
      child.getCoordinates(
          inList,
          additionalDistanceForConvexHull,
          true,
          transX + child.getX(),
          transY + child.getY());
    }
  ...
  public NodeView getParentView() {
    final Container parent = getParent();
    if (parent instanceof NodeView parentView) return parentView;
    return null;
  }

  public NodeView getVisibleParentView() {
    final Container parent = getParent();
    if (!(parent instanceof NodeView parentView)) return null;
  ...
  /**
   * Returns the point the edge should start given the point of the child node that should be
   * connected.
   *
  ...
  Point getMainViewOutPoint(NodeView targetView, Point destinationPoint) {
    ...
    return layoutManager.getMainViewOutPoint(this, targetView, destinationPoint);
  }
  ...
  public Point getLinkPoint(Point declination) {
    ...
    if (y != 0) {
      ...
      linkPoint =
          new Point(
              getContent().getWidth() / 2 + (x > 0 ? absLinkX : -absLinkX),
  ...
  /**
   * Returns the relative position of the Edge. This is used by bold edge to know how to shift the
   * line.
   */
  int getAlignment() {
  ...
  //
  /**
   * The algorithm should be here the following (see Eclipse editor): Selected is the n-th node from
   * above. Look for the last node visible on the screen and make this node the first one. Now
   * select the n-th node from above.
   *
   * <p>Easier idea to implement: Store node y position as y0. Search for a node with the same
   * parent with y position y0+height Scroll the window by height.
   */
  ...
  protected NodeView getNextVisibleSibling() {
    ...
    while (sibling.getModel().getNodeLevel() < getMap().getSiblingMaxLevel()) {
      // can we drill down?
      NodeView first =
          sibling.getFirst(sibling.isRoot() ? lastSibling : null, this.isLeft(), !this.isLeft());
      if (first == null) {
        break;
      }
      sibling = first;
  ...
  protected NodeView getPreviousVisibleSibling() {
    ...
    while (sibling.getModel().getNodeLevel() < getMap().getSiblingMaxLevel()) {
      NodeView last =
          sibling.getLast(sibling.isRoot() ? previousSibling : null, this.isLeft(), !this.isLeft());
      if (last == null) {
        break;
      }
      sibling = last;
  ...
  void insert() {
    ListIterator<? extends MindMapNode> it = getModel().childrenFolded();
    while (it.hasNext()) {
      insert(it.next(), 0);
    }
  }
  ...
  /**
   * Create views for the newNode and all his descendants, set their isLeft attribute according to
   * this view.
   */
  ...
  /**
   * This is a bit problematic, because getChildrenViews() only works if model is not yet removed.
   * (So do not _really_ delete the model before the view removed (it needs to stay in memory)
   */
  void remove() {
  ...
  private void updateText() {
    ...
    if (!isHtml) {
      ...
      for (String s : lines) {
        ...
        setText(s);
        widthMustBeRestricted =
            mainView.getPreferredSize().width
                > mapView.getZoomed(mapView.getMaxNodeWidth()) + mainView.getIconWidth();
    ...
    if (isHtml) {
      ...
      if (nodeText.contains("<img") && !nodeText.contains("<base ")) {
        try {
          nodeText =
              "<html><base href=\"" + mapView.getModel().getURL() + "\">" + nodeText.substring(6);
      ...
      // Set user HTML head
      String htmlLongNodeHead = getViewFeedback().getProperty("html_long_node_head");
      ...
      if (widthMustBeRestricted) {
        nodeText =
            nodeText.replaceFirst(
                "(?i)<body>", "<body width=\"" + mapView.getMaxNodeWidth() + "\">");
      }
      setText(nodeText);
      ...
      StringBuilder text =
          new StringBuilder("<html><table border=1 style=\"border-color: white\">");
      // String[] lines = nodeText.split("\n");
      for (int line = startingLine; line < lines.length; line++) {
        text.append("<tr><td style=\"border-color: white;\">")
            .append(
      ...
      if (widthMustBeRestricted) {
        text = text.replaceFirst("(?i)<p>", "<p width=\"" + mapView.getMaxNodeWidth() + "\">");
      }
      setText(text);
    } else {
      setText(nodeText);
  ...
  private void updateIcons() {
      ...
      if (sAttributeIcon == null) {
        sAttributeIcon =
            freemind.view.ImageFactory.getInstance()
                .createUnscaledIcon(
                    Resources.getInstance().getResource("images/showAttributes.gif"));
      }
    ...
    if (link != null) {
      ...
      ImageIcon icon =
          freemind.view.ImageFactory.getInstance()
              .createUnscaledIcon(Resources.getInstance().getResource(iconPath));
  ...
  private void updateIconPosition() {
    getMainView()
        .setHorizontalTextPosition(isLeft() ? SwingConstants.LEADING : SwingConstants.TRAILING);
  }
  ...
  boolean useSelectionColors() {
    return isSelected()
        && !MapView.standardDrawRectangleForSelection
  ...
  public void updateToolTip() {
    ...
    if (tooltips.isEmpty()) {
      ...
      // html table
      StringBuilder text =
          new StringBuilder("<html><table width=\"" + getMaxToolTipWidth() + "\">");
  ...
  public int getMaxToolTipWidth() {
    if (maxToolTipWidth == 0) {
      try {
        maxToolTipWidth = Integer.parseInt(getViewFeedback().getProperty("max_tooltip_width"));
   ...
   * 40ID-77081.news.dfncis.de&rnum=1&prev=/groups%3Fq%3Djava%2520komplement%25 C3
   * %25A4rfarbe%2520helligkeit%26hl%3Dde%26lr%3D%26ie%3DUTF-8%26sa%3DN%26as_qdr %3Dall%26tab%3Dwg
   */
  /**
   * Determines to a given color a color, that is the best contrary color. It is different from
   * {@link #getAntiColor2}.
   *
   * @since PPS 1.1.1
   */
  ...
  public void setPreferredChild(NodeView view) {
    ...
    if (view != null && parent instanceof NodeView parentView) {
      // set also preferred child of parents...
   ...
   * (non-Javadoc)
   *
   * @see javax.swing.event.TreeModelListener#treeStructureChanged(javax.swing.
  ...
  public int getZoomedFoldingSymbolHalfWidth() {
    int preferredFoldingSymbolHalfWidth = (int) ((getFoldingSymbolWidth() * mapView.getZoom()) / 2);
    return Math.min(preferredFoldingSymbolHalfWidth, getHeight() / 2);
  }

  /**
   * @return the left/right point of the folding circle. To receive its center, the amount has to be
   *     moved to left/right (depending on its side) by the folding circle width.
   */
  public Point getFoldingMarkPosition() {
    return getMainViewOutPoint(this, new Point());
  }
