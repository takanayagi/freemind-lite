...
public class VerticalRootNodeViewLayout extends NodeViewLayoutAdapter {
	private static final String USE_COMMON_OUT_POINT_FOR_ROOT_NODE_STRING =
	...
	protected void layout() {
		...
		if (getView().isContentVisible()) {
			...
			final int childVerticalShift =
					Math.min(rightChildVerticalShift, leftChildVerticalShift);
	...
	public Point getMainViewOutPoint(NodeView view, NodeView targetView, Point destinationPoint) {
		...
		return new Point(centerPoint.x + (int) (Math.cos(angle) * nWidth),
				centerPoint.y + (int) (Math.sin(angle) * nHeight));
	}

	public Point getMainViewInPoint(NodeView view) {
		return view.getMainView().getCenterPoint();
	}
