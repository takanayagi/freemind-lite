...
public class MindMapLinkRegistry {
  /**
   * All elements put into this sort of vectors are put into the SourceToLinks, too. This structure
   * is kept synchronous to the IDToLinks structure, but reversed.
   *
   * @author foltin
   * @date 23.01.2012
   */
  ...
  public String getState(MindMapNode node) {
    if (mTargetToId.containsKey(node)) return mTargetToId.get(node);
    return null;
  }
  ...
  /**
   * Method to keep track of the sources associated to a target node. This method also sets the new
   * id to the target. Moreover, it is not required that the target node is already registered. This
