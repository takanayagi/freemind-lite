...
/**
 * Interface for the registry, which manages the ids of nodes and the existing links in a map. Thus,
 * this interface is bound to a map model, because other maps have a different registry.
 */
public class MindMapLinkRegistry {
	/**
	 * All elements put into this sort of vectors are put into the SourceToLinks, too. This
	 * structure is kept synchronous to the IDToLinks structure, but reversed.
	 * 
	 * @author foltin
	 * @date 23.01.2012
	 */
	private class SynchronousVector extends Vector<MindMapLink> {
		...
		public synchronized boolean add(MindMapLink pE) {
			...
			if (pE != null) {
				MindMapNode source = pE.getSource();
	...
	/**
	 * This can be used, if the id has to be known, before a node can be labeled.
	 */
	...
	/**
	 * The second variant of the main method. The difference is that here an ID is proposed, but has
	 * not to be taken, though.
	 */
	...
	private Vector<MindMapLink> getAssignedLinksVector(String newId) {
		...
		if (mIdToLinks.containsKey(newId)) {
			vec = mIdToLinks.get(newId);
		} else {
			vec = new SynchronousVector();
			mIdToLinks.put(newId, vec);
		}
	...
	public void deregisterLinkTarget(MindMapNode target) throws java.lang.IllegalArgumentException {
		...
		// and process my sons:
		for (ListIterator<? extends MindMapNode> e = target.childrenUnfolded(); e.hasNext();) {
	...
	/**
	 * Method to keep track of the sources associated to a target node. This method also sets the
	 * new id to the target. Moreover, it is not required that the target node is already
	...
	public void registerLink(MindMapLink link) throws java.lang.IllegalArgumentException {
		...
		logger.fine(
				"Register link (" + link + ") from source node:" + source + " to target " + target);
	...
	public void deregisterLink(MindMapLink link) {
		...
		for (int i = vec.size() - 1; i >= 0; --i) {
			...
			if (vec.get(i) == link) {
				...
				logger.info("Deregister link  (" + link + ") from source node:" + link.getSource()
						+ " to target " + target);
			}
		}
