...
/** The MouseMotionListener which belongs to every NodeView. Handles delayed selection. */
public class CommonNodeMouseMotionListener implements NodeMouseMotionObserver {
  ...
  public void mouseEntered(MouseEvent e) {
    ...
    if (!JOptionPane.getFrameForComponent(e.getComponent()).isFocused()) return;
    createTimer(e);
    // c.select(e);
  }
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    MouseEvent ev = e;
    /*
     * For Mac see https://developer.apple.com/library/mac/#documentation/Java/Conceptual/
    ...
    if (SwingUtilities.isLeftMouseButton(ev) && ev.getModifiersEx() == 0) {
      // left button with no modifier keys
      c.plainClick(ev);
      ev.consume();
    }
  }
  ...
  protected Rectangle getControlRegion(Point2D p) {
    ...
    int side = 8;
    return new Rectangle((int) (p.getX() - side / 2.0), (int) (p.getY() - side / 2.0), side, side);
  }
  ...
  protected static class timeDelayedSelection extends TimerTask {
    ...
    public void run() {
      /*
       * formerly in ControllerAdapter. To guarantee, that point-to-select does not change
       * selection if any meta key is pressed.
       */
