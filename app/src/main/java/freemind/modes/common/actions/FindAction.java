...
public class FindAction extends FreemindAction {
	...
	public String getFindFromText() {
		...
		return plainNodeText.length() <= 30 ? plainNodeText
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (!found) {
			...
			controller.getController().informationMessage(messageText.replaceAll("\\$1", searchTerm)
					.replaceAll("\\$2", getFindFromText()), controller.getView().getSelected());
		}
	}
	...
	private void close(int pResult) {
		...
		if (pResult == JOptionPane.OK_OPTION) {
			Resources.getInstance().getProperties().setProperty(
	...
	void displayDialog() {
		...
		contentPane.add(mSearchField, new GridBagConstraints(2, 0, 10, 1, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 0, 0), 0, 0));
		ImageIcon findImage = freemind.view.ImageFactory.getInstance()
				.createIcon(Resources.getInstance().getResource("images/filefind_big.png"));
		contentPane.add(new JLabel(findImage), new GridBagConstraints(0, 0, 1, 2, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 0, 0), 0, 0));
		mFindInNotesTooBox =
				new JCheckBox(controller.getText("ExtendedFindDialog.find_search_in_notes_too"));
		mFindInNotesTooBox.setSelected(
				Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_SEARCH_IN_NOTES_TOO));
		Tools.setLabelAndMnemonic(mFindInNotesTooBox, null);
		contentPane.add(mFindInNotesTooBox, new GridBagConstraints(0, 2, 3, 1, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 0, 0), 0, 0));
		...
		contentPane.add(cancelButton, new GridBagConstraints(3, 3, 1, 1, 1.0, 1.0,
				GridBagConstraints.WEST, GridBagConstraints.BOTH, new Insets(5, 5, 0, 0), 0, 0));
	...
	private String getSearchTermAsEscapedString(String messageText) {
		String searchTerm =
				messageText.startsWith("<html>") ? HtmlTools.toXMLEscapedText(getSearchTerm())
						: getSearchTerm();
		// Fix for
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (subterms == null) {
				controller.getController().informationMessage(
						controller.getText("no_previous_find"), controller.getView().getSelected());
				return;
			}
	...
	private boolean find(LinkedList<MindMapNode> nodes, Collection<String> subterms,
			boolean caseSensitive) {
		// Precondition: if !caseSensitive then >>what<< is in lowercase.
		boolean searchInNotesToo =
				Resources.getInstance().getBoolProperty(FreeMind.RESOURCES_SEARCH_IN_NOTES_TOO);

		if (!findNodesUnfoldedByLastFind.isEmpty()) {

			// if (false) {
			ListIterator<MindMapNode> i =
					findNodesUnfoldedByLastFind.listIterator(findNodesUnfoldedByLastFind.size());
			while (i.hasPrevious()) {
