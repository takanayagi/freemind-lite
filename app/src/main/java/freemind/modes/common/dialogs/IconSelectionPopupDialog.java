...
public class IconSelectionPopupDialog extends JDialog implements KeyListener, MouseListener {
  ...
  public IconSelectionPopupDialog(
    ...
    if (numOfIcons <= xDimension * (xDimension - 1)) yDimension = xDimension - 1;
  ...
  private boolean canSelect(Position position) {
    return ((position.x() >= 0)
        && (position.x() < xDimension)
        && (position.y() >= 0)
        && (position.y() < yDimension)
        && (calculateIndex(position) < numOfIcons));
  }
  ...
  private void unhighlight(Position position) {
    iconLabels[calculateIndex(position)].setBorder(
  ...
  private void cursorLeft() {
    Position newPosition = new Position(getSelectedPosition().x() - 1, getSelectedPosition().y());
  ...
  private void cursorUp() {
    Position newPosition = new Position(getSelectedPosition().x(), getSelectedPosition().y() - 1);
    if (canSelect(newPosition)) select(newPosition);
  }
    ...
    for (int i = 0; i < icons.size(); i++) {
      ...
      if (iconKeyStroke != null
          && (keyEvent.getKeyCode() == iconKeyStroke.getKeyCode()
                  ...
                  && (iconKeyStroke.getModifiers() & KeyEvent.SHIFT_DOWN_MASK)
