...
public class IconSelectionPopupDialog extends JDialog implements KeyListener, MouseListener {
	...
	public IconSelectionPopupDialog(JFrame caller, Vector<IconInformation> icons,
		...
		iconPanel.setPreferredSize(
				new Dimension(xDimension * perIconSize, yDimension * perIconSize));
	...
	private boolean canSelect(Position position) {
		return ((position.x() >= 0) && (position.x() < xDimension) && (position.y() >= 0)
				&& (position.y() < yDimension) && (calculateIndex(position) < numOfIcons));
	}
	...
	private void select(Position position) {
		...
		final IconInformation iconInformation = icons.get(index);
		final String keyStroke =
				freeMindMain.getAdjustableProperty(iconInformation.getKeystrokeResourceName());
		if (keyStroke != null) {
	...
	private void unhighlight(Position position) {
		iconLabels[calculateIndex(position)]
	...
	private void cursorLeft() {
		Position newPosition =
				new Position(getSelectedPosition().x() - 1, getSelectedPosition().y());
	...
	private void cursorUp() {
		Position newPosition =
				new Position(getSelectedPosition().x(), getSelectedPosition().y() - 1);
		if (canSelect(newPosition))
			select(newPosition);
	}
	...
	/**
	 * Transfer shift masks from InputEvent to ActionEvent. But, why don't they use the same
	 * constants???? Java miracle.
	 */
	...
	public void keyPressed(KeyEvent keyEvent) {
		switch (keyEvent.getKeyCode()) {
			...
			case KeyEvent.VK_SPACE:
				keyEvent.consume();
				addIcon(keyEvent.getModifiersEx());
				return;
		}
		...
		if (index != -1) {
			...
			mModifiers = keyEvent.getModifiersEx();
			keyEvent.consume();
			this.dispose();
		}
	}
		...
		for (int i = 0; i < icons.size(); i++) {
			...
			if (iconKeyStroke != null
					&& (keyEvent.getKeyCode() == iconKeyStroke.getKeyCode()
							...
							&& (iconKeyStroke.getModifiers()
									& KeyEvent.SHIFT_DOWN_MASK) == (keyEvent.getModifiersEx()
	...
	public void mouseClicked(MouseEvent mouseEvent) {
		addIcon(mouseEvent.getModifiersEx());
	}
