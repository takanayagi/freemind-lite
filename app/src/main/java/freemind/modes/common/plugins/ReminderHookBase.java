...
public abstract class ReminderHookBase extends PermanentNodeHookAdapter {
  ...
  public void invoke(MindMapNode node) {
    ...
    if (timer == null) {
      ...
      MessageFormat formatter =
          new MessageFormat(getResourceString("plugins/TimeManagement.xml_reminderNode_tooltip"));
  ...
  private void scheduleTimer() {
    timer = new Timer(getRemindUserAtAsSecondsFromNow(), new TimerBlinkTask(false));
  ...
  public class TimerBlinkTask implements ActionListener {
    ...
    public void actionPerformed(ActionEvent pE) {
      ...
      setRemindUserAt(System.currentTimeMillis() + BLINK_INTERVAL_IN_MILLIES); // 3
      displayState((stateAdded) ? CLOCK_VISIBLE : CLOCK_INVISIBLE, getNode(), true);
    }
  }
