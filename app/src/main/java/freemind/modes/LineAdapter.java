...
public abstract class LineAdapter implements MindMapLine {
  ...
  protected void updateStandards() {
    if (getStandardColor() == null) {
      String stdColor = getMapFeedback().getProperty(getStandardColorPropertyString());
    ...
    if (getStandardStyle() == null) {
      String stdStyle = getMapFeedback().getProperty(getStandardStylePropertyString());
      if (stdStyle != null) {
  ...
  public int getWidth() {
    if (width == DEFAULT_WIDTH) return NORMAL_WIDTH;
    return width;
  }
  ...
  /**
   * As this color is static but used in at least three different objects (edges, clouds and links),
   * the abstract mechanism was chosen. The derived classes set and get the static instance
   * variable.
   */
