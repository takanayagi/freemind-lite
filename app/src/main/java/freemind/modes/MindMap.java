...
public interface MindMap extends TreeModel {
  ...
  String getAsPlainText(List<? extends MindMapNode> mindMapNodes);

  String getAsRTF(List<? extends MindMapNode> mindMapNodes);

  String getAsHTML(List<? extends MindMapNode> mindMapNodes);
  ...
  /**
   * Returns a string that may be given to the modes restore() to get this map again. The Mode must
   * take care that two different maps don't give the same restoreable key.
   */
  ...
  /**
   * @return returns the link registry associated with this mode, or null, if no registry is
   *     present.
   */
  ...
  /**
   * When the map source is changed (eg. on disk, there is a newer version edited from somebody
   * else), this observer can be used to notice this.
   *
   * @author foltin
   * @date 04.07.2011
   */
  interface MapSourceChangedObserver {
    /**
     * @param pMap
     * @return true, if the map was reloaded, false otherwise. This means, that if the method
     *     returns true, then the next change on disk is reported as well. If it returns false, the
   ...
   *     registered, unregistered at time t, and register again at time t+s, you should specify t
   *     here. If there was an event in between t and t+s, and event is issued directly.
   */
  ...
  /**
   * @param pMapSourceChangedObserver
   * @return the last saving time to be stored (see {@link
   *     MindMap#registerMapSourceChangedObserver(MapSourceChangedObserver, long)} )
   */
  ...
  /**
   * @return a list of all icons present in the mindmap. Convenience method for filters.
   */
  ...
  MindMapNode loadTree(
      Tools.ReaderCreator pReaderCreator, AskUserBeforeUpdateCallback pAskUserBeforeUpdateCallback)
      throws XMLParseException, IOException;

  MindMapNode createNodeTreeFromXml(Reader pReader, HashMap<String, NodeAdapter> pIDToTarget)
      throws XMLParseException, IOException;
