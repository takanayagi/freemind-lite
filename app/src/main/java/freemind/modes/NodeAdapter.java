...
public abstract class NodeAdapter implements MindMapNode {
	 ...
	 * view, maybe with a Vector in which the index specifies the MapView which contains the
	 * NodeViews
	 */
	...
	public void toggleStrikethrough() {
		...
		if (attributes.containsKey(TextAttribute.STRIKETHROUGH) && TextAttribute.STRIKETHROUGH_ON
				.equals(attributes.get(TextAttribute.STRIKETHROUGH))) {
	...
	public SortedMap<String, String> getToolTip() {
		...
		// replace jump mark
		if (link != null
				&& link.matches(".*\\" + FreeMindCommon.FREEMIND_FILE_EXTENSION + "(#.*)?")) {
	...
	public XMLElement save(Writer writer, MindMapLinkRegistry registry, boolean saveInvisible,
			boolean saveChildren) throws IOException {
		...
		for (MindMapLink mindMapLink : targetVector) {
			if (mindMapLink instanceof ArrowLinkAdapter) {
				XMLElement arrowLinkTargetElement = ((ArrowLinkAdapter) mindMapLink)
		...
		if (!sSaveOnlyIntrinsicallyNeededIds || (registry.isTargetOfLocalHyperlinks(label)
		...
		if (historyInformation != null) {
			...
			node.setAttribute(XMLElementAdapter.XML_NODE_HISTORY_LAST_MODIFIED_AT,
	...
	private void saveChildren(Writer writer, MindMapLinkRegistry registry, NodeAdapter node,
			boolean saveHidden) throws IOException {
