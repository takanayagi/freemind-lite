...
/**
 * This class represents a single Node of a Tree. It contains direct handles to its parent and
...
public abstract class NodeAdapter implements MindMapNode {
  ...
  /**
   * the edge which leads to this node, only root has none In future it has to hold more than one
   * view, maybe with a Vector in which the index specifies the MapView which contains the NodeViews
   */
      ...
      sSaveIdPropertyChangeListener =
          (propertyName, newValue, oldValue) -> {
            if (propertyName.equals(FreeMindCommon.SAVE_ONLY_INTRISICALLY_NEEDED_IDS)) {
              sSaveOnlyIntrinsicallyNeededIds = Boolean.parseBoolean(newValue);
            }
          };
  ...
  private void changeChildCloudIterativeLevels(int deltaLevel) {
    for (ListIterator<MindMapNode> e = childrenUnfolded(); e.hasNext(); ) {
      MindMapNode node = e.next();
      if (node instanceof NodeAdapter childNode) {
  ...
  public String getStyle() {
    ...
    if (style == null) {
      if (this.isRoot()) {
        returnedString = getMapFeedback().getProperty(FreeMind.RESOURCES_ROOT_NODE_STYLE);
      } else {
        String stdstyle = getMapFeedback().getProperty(FreeMind.RESOURCES_NODE_STYLE);
  ...
  public void toggleStrikethrough() {
    establishOwnFont();
    // The Map returned by TextAttribute can be immutable, so copy to a mutable Map.
    Map<TextAttribute, Object> attributes = new HashMap<>(font.getAttributes());
    if (attributes.containsKey(TextAttribute.STRIKETHROUGH)
        && TextAttribute.STRIKETHROUGH_ON.equals(attributes.get(TextAttribute.STRIKETHROUGH))) {
  ...
  public void setFontSize(int fontSize) {
    establishOwnFont();
    setFont(getMapFeedback().getFontThroughMap(font.deriveFont((float) fontSize)));
  }
  ...
  public boolean hasFoldedStrictDescendant() {
    ...
    for (ListIterator<MindMapNode> e = childrenUnfolded(); e.hasNext(); ) {
      MindMapNode node = e.next();
      if (node instanceof NodeAdapter child
          && (child.isFolded() || child.hasFoldedStrictDescendant())) {
        return true;
      }
    }
  ...
  public ListIterator<MindMapNode> childrenUnfolded() {
    return children != null
        ...
        : Collections.<MindMapNode>emptyList().listIterator();
  }
  ...
  public void remove(MutableTreeNode node) {
    if (node == this.preferredChild) { // mind preferred child :-) (PN)
      ...
      if (children.size() > index + 1) {
        ...
        this.preferredChild = (index > 0) ? children.get(index - 1) : null;
      }
    }
    node.setParent(null);
  ...
  private void recursiveCallRemoveChildren(
      MindMapNode node, MindMapNode removedChild, MindMapNode oldDad) {
    for (PermanentNodeHook hook : node.getActivatedHooks()) {
  ...
  public PermanentNodeHook addHook(PermanentNodeHook hook) {
    // add then
    if (hook == null) throw new IllegalArgumentException("Added null hook.");
    createHooks();
    hooks.add(hook);
    return hook;
  }
  ...
  public SortedMap<String, String> getToolTip() {
    ...
    if (link != null && link.endsWith(FreeMindCommon.FREEMIND_FILE_EXTENSION)) {
      ...
      boolean addIt = true;
      // this should be done only once per link, so we have to prevent doing that every time
      // again.
      if (result.containsKey(TOOLTIP_PREVIEW_KEY)) {
        // check, if the contained link belongs to the same file (ie. hasn't change in
        // between)
  ...
  public XMLElement save(
      Writer writer, MindMapLinkRegistry registry, boolean saveInvisible, boolean saveChildren)
      throws IOException {
    ...
    if (!HtmlTools.isHtmlNode(text)) {
      ...
      htmlElement.setAttribute(
          XMLElementAdapter.XML_NODE_XHTML_TYPE_TAG, XMLElementAdapter.XML_NODE_XHTML_TYPE_NODE);
    ...
    if (getXmlNoteText() != null) {
      ...
      htmlElement.setName(XMLElementAdapter.XML_NODE_XHTML_CONTENT_TAG);
      htmlElement.setAttribute(
          XMLElementAdapter.XML_NODE_XHTML_TYPE_TAG, XMLElementAdapter.XML_NODE_XHTML_TYPE_NOTE);
      htmlElement.setEncodedContent(convertToEncodedContent(getXmlNoteText()));
      node.addChild(htmlElement);
    }
    // save additional info:
    if (getAdditionalInfo() != null) {
      node.setAttribute(XMLElementAdapter.XML_NODE_ENCRYPTED_CONTENT, getAdditionalInfo());
    }
    ...
    for (MindMapLink mindMapLink : linkVector) {
      if (mindMapLink instanceof ArrowLinkAdapter) {
        XMLElement arrowLinkElement = ((ArrowLinkAdapter) mindMapLink).save();
    ...
    for (MindMapLink mindMapLink : targetVector) {
      if (mindMapLink instanceof ArrowLinkAdapter) {
        XMLElement arrowLinkTargetElement =
            ((ArrowLinkAdapter) mindMapLink).createArrowLinkTarget(registry).save();
    ...
    if (!sSaveOnlyIntrinsicallyNeededIds
        || (registry.isTargetOfLocalHyperlinks(label)
            || (!registry.getAllLinksIntoMe(this).isEmpty()))) {
      if (label != null) {
    ...
    for (int i = 0; i < getIcons().size(); ++i) {
      ...
      iconElement.setAttribute("BUILTIN", getIcons().get(i).getName());
  ...
  private void saveChildren(
      Writer writer, MindMapLinkRegistry registry, NodeAdapter node, boolean saveHidden)
      throws IOException {
    for (ListIterator<MindMapNode> e = node.childrenUnfolded(); e.hasNext(); ) {
      MindMapNode nd = e.next();
      if (nd instanceof NodeAdapter child) {
        if (saveHidden || child.isVisible()) child.save(writer, registry, saveHidden, true);
  ...
  public synchronized void setStateIcon(String key, ImageIcon icon) {
    ...
    if (stateIcons.isEmpty()) stateIcons = null;
  }
