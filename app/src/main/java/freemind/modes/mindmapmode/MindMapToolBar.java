...
public class MindMapToolBar extends FreeMindToolBar implements ZoomListener {
  ...
  /**
   * A combo box that doesn't fill the complete screen. See
   * http://stackoverflow.com/questions/13345640/does-anyone-know-how-to-layout-a-jtoolbar-that-doest-move-or-re-size-any-compon
   */
  ...
  public MindMapToolBar(MindMapController controller) {
    ...
    iconToolBar.setLayout(
        new GridLayout(0, getController().getIntProperty(FreeMind.ICON_BAR_COLUMN_AMOUNT, 1)));
  ...
  private void setZoomByItem(Object item) {
    if (item.equals(userDefinedZoom)) return;
