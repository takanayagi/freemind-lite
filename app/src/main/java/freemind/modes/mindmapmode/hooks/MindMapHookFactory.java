...
public class MindMapHookFactory extends HookFactoryAdapter {
	 ...
	 */
	private final static String pluginPrefixRegEx =
			".*(accessories(/|\\\\)|)plugins(/|\\\\)[^/\\\\]*";
		...
		if (importWizard == null) {
			...
			for (String xmlPluginFile : importWizard.CLASS_LIST) {
				if (xmlPluginFile.matches(pluginPrefixRegEx)) {
					...
					URL pluginURL = Resources.getInstance().getFreeMindClassLoader()
					...
					for (Object obj : plugin.getListChoiceList()) {
						if (obj instanceof PluginAction action) {
							...
							allRegistrations.add(new HookDescriptorRegistration(xmlPluginFile,
									plugin, registration));
	...
	public void decorateAction(String hookName, AbstractAction action) {
		...
		if (icon != null) {
			ImageIcon imageIcon = freemind.view.ImageFactory.getInstance()
		...
		for (HookDescriptorRegistration descriptor : allRegistrations) {
			...
			try {
				...
				Class<?> hookRegistrationClass =
						Class.forName(descriptor.getClassName(), true, loader);
