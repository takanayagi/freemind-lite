...
public class MindMapHookFactory extends HookFactoryAdapter {
  ...
  private static final String pluginPrefixRegEx =
      ".*(accessories(/|\\\\)|)plugins(/|\\\\)[^/\\\\]*";
    ...
    if (importWizard == null) {
      ...
      for (String xmlPluginFile : importWizard.CLASS_LIST) {
        if (xmlPluginFile.matches(pluginPrefixRegEx)) {
          ...
          URL pluginURL =
              Resources.getInstance().getFreeMindClassLoader().getResource(xmlPluginFile);
          // unmarshal xml:
          Plugin plugin = null;
          try {
          ...
          for (Object obj : plugin.getListChoiceList()) {
            if (obj instanceof PluginAction action) {
              pluginInfo.put(
                  action.getLabel(), new HookDescriptorPluginAction(xmlPluginFile, plugin, action));
              allPlugins.add(action.getLabel());

            } else if (obj instanceof PluginRegistration registration) {
              allRegistrations.add(
                  new HookDescriptorRegistration(xmlPluginFile, plugin, registration));
  ...
  private MindMapHook createJavaHook(String hookName, HookDescriptorPluginAction descriptor) {
    try {
      ...
      for (PluginClasspath plPath : descriptor.getPluginClasspath()) {
        path.append(plPath.getJar()).append(";");
      }
      freemind.main.Resources.getInstance()
          .logException(
              e,
              "Error occurred loading hook: "
  ...
  private void decorateHook(
      String hookName, final HookDescriptorPluginAction descriptor, MindMapHook hook) {
    hook.setProperties(descriptor.getProperties());
    hook.setName(hookName);
    hook.setPluginBaseClass(() -> getPluginBaseClass(descriptor));
  }
    ...
    if (icon != null) {
      ImageIcon imageIcon =
          freemind.view.ImageFactory.getInstance()
              .createIcon(descriptor.getPluginClassLoader().getResource(icon));
      action.putValue(AbstractAction.SMALL_ICON, imageIcon);
    }
    String key = descriptor.getKeyStroke();
    if (key != null) action.putValue(AbstractAction.ACCELERATOR_KEY, KeyStroke.getKeyStroke(key));
  }
