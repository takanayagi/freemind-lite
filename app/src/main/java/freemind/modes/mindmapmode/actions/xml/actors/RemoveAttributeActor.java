...
public class RemoveAttributeActor extends XmlActorAdapter {
	...
	public ActionPair getActionPair(MindMapNode selected, int pPosition) {
		RemoveAttributeAction setAttributeAction = getRemoveAttributeAction(selected, pPosition);
		NodeAction undoRemoveAttributeAction;
		if (pPosition == selected.getAttributeTableLength() - 1) {
			undoRemoveAttributeAction = getXmlActorFactory().getAddAttributeActor()
					.getAddAttributeAction(selected, selected.getAttribute(pPosition));
		} else {
			undoRemoveAttributeAction =
					getXmlActorFactory().getInsertAttributeActor().getInsertAttributeAction(
	 ...
	 * @return
	 */
	public RemoveAttributeAction getRemoveAttributeAction(MindMapNode pSelected, int pPosition) {
