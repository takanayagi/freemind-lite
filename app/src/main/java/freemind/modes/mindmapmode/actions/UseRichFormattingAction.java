/*
 * FreeMind - a program for creating and viewing mind maps Copyright (C) 2000-2006 Joerg Mueller,
...
public class UseRichFormattingAction extends NodeGeneralAction {
	public UseRichFormattingAction(final MindMapController modeController) {
		super(modeController, "use_rich_formatting", null, (map, selected) -> {
			...
			if (!HtmlTools.isHtmlNode(nodeText)) {
				modeController.setNodeText(selected, HtmlTools.plainToHTML(nodeText));
			}
		});
	}
