...
public class RevertActor extends XmlActorAdapter {
  ...
  public void revertMap(MindMap map, File file) throws IOException {
    ...
    RevertXmlAction undoAction = createRevertXmlAction(map, null, file.getName());
  ...
  public void openXmlInsteadOfMap(String xmlFileContent) {
    try {
      ...
      RevertXmlAction undoAction = createRevertXmlAction(getExMapFeedback().getMap(), null, null);
      execute(new ActionPair(doAction, undoAction));
    } catch (IOException e) {
  ...
  public RevertXmlAction createRevertXmlAction(File file) throws IOException {
    ...
    for (int c; (c = f.read()) != -1; ) buffer.append((char) c);
    f.close();
  ...
  /**
   * @param filePrefix is used to generate the name of the reverted map in case that fileName is
   *     null.
   */
  public RevertXmlAction createRevertXmlAction(
      String xmlPackedFile, String fileName, String filePrefix) {
    ...
    if (action instanceof RevertXmlAction revertAction) {
      try {
        ...
        if (revertAction.getLocalFileName() != null) {
          getExMapFeedback().load(new File(revertAction.getLocalFileName()));
        } else {
          // the map is given by xml. we store it and open it.
          String filePrefix = getExMapFeedback().getResourceString("freemind_reverted");
