...
public class EditAction extends MindmapAction {
	...
	public void edit(final NodeView node, final NodeView prevSelected, final KeyEvent firstEvent,
		...
		String htmlEditingOption =
				mMindMapController.getController().getProperty("html_editing_option");
		...
		if (!isHtmlNode && !isLongNode && editLong) {
			// ask user:
			int showResult =
					new OptionalDontShowMeAgainDialog(mMindMapController.getFrame().getJFrame(),
							mMindMapController.getSelectedView(), "edit.edit_rich_text",
							"edit.decision", mMindMapController,
		...
		boolean editHtml =
				isHtmlNode || (editLong && Tools.safeEquals(useRichTextInNewLongNodes, "true"));
		boolean editInternalWysiwyg =
				editHtml && Tools.safeEquals(htmlEditingOption, "internal-wysiwyg");
		...
		if (editInternalWysiwyg) {
			EditNodeWYSIWYG editNodeWYSIWYG = new EditNodeWYSIWYG(node, text, firstEvent,
					mMindMapController, new EditNodeBase.EditControl() {
						public void cancel() {
		...
		if (editExternal) {
			EditNodeExternalApplication editNodeExternalApplication =
					new EditNodeExternalApplication(node, text, firstEvent, mMindMapController,
							new EditNodeBase.EditControl() {
								...
								public void split(String newText, int position) {
									mMindMapController.splitNode(node.getModel(), position,
											newText);
									cancel();
								}
							}); // focus fix
		...
		if (isLongNode || editLong) {
			EditNodeDialog nodeEditDialog = new EditNodeDialog(node, text, firstEvent,
					mMindMapController, new EditNodeBase.EditControl() {

						public void cancel() {
							mMindMapController.setBlocked(false);
							mCurrentEditDialog = null;
							mMindMapController.getController().obtainFocusForSelected(); // focus
																							// fix
						}
		...
		// inline editing:
		EditNodeTextField textfield = new EditNodeTextField(node, text, firstEvent,
				mMindMapController, new EditNodeBase.EditControl() {
