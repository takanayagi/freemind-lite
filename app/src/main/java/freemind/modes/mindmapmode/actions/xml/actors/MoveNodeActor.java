...
public class MoveNodeActor extends NodeXmlActorAdapter {
  ...
  public void act(XmlAction action) {
    ...
    if (!node.isRoot()) node.getParentNode().setVGap(moveAction.getVGap());
  ...
  public ActionPair apply(MindMap model, MindMapNode selected) {
    // reset position
    if (selected.isRoot()) return null;
  ...
  private ActionPair getActionPair(MindMapNode selected, int parentVGap, int hGap, int shiftY) {
    ...
    MoveNodeXmlAction undoAction =
        moveNode(
            selected, selected.getParentNode().getVGap(), selected.getHGap(), selected.getShiftY());
