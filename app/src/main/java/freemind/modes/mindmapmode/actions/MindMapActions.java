 ...
 *
 * <p>All these methods do redisplay, because they are offered from the MindMapController for use.
 *
...
public interface MindMapActions {
  ...
  /**
   * The following modes are present: public final int NEW_CHILD_WITHOUT_FOCUS = 1; // old model of
   * insertion public final int NEW_CHILD = 2; public final int NEW_SIBLING_BEHIND = 3; public final
   * int NEW_SIBLING_BEFORE = 4; see MindMapController
   */
   ...
   * @param asSibling true: the past will be a direct sibling of target, otherwise it will become a
   *     child
  ...
  /**
   * This is the only way to instanciate new Hooks. THEY HAVE TO BE INVOKED AFTERWARDS! The hook is
   * equipped with the map and controller information. Furthermore, the hook is added to the node,
   * if it is an instance of the PermanentNodeHook. If the hook policy specifies, that only one
   * instance may exist per node, it returns this instance if it already exists.
   */
  ...
  /**
   * Inserts a new attribute at a given place of the attributes table. To insert an attribute as the
   * last item, {link {@link #addAttribute(MindMapNode, Attribute)} instead.
   */
