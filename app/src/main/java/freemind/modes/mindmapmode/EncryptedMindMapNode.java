...
public class EncryptedMindMapNode extends MindMapNodeModel {
  ...
  /**
   * is only set to false by the load mechanism. If the node is generated or it is decrypted once,
   * this is always true.
   */
  private boolean isDecrypted = true;

  /** password have to be stored in a StringBuffer as Strings cannot be deleted or overwritten. */
      ...
      try {
        ...
        if (childXml.startsWith(MapAdapter.MAP_INITIAL_START)) {
          ...
          StringBuilder mapContent =
              new StringBuilder(MapAdapter.MAP_INITIAL_START + "0.8.0\"><node TEXT=\"DUMMY\">");
  ...
  private MindMapNode getNodeFromXml(String childXml) throws IOException {
    ...
    return getMap().loadTree(new Tools.StringReaderCreator(childXml), MapAdapter.sDontAskInstance);
  }
  ...
  private boolean equals(StringBuffer givenPassword, StringBuffer password2) {
    if (givenPassword.length() != password.length()) return false;
  ...
  private String encryptXml(StringBuffer childXml) {
    try {
      ...
      // Encrypt
      return encrypter.encrypt(childXml.toString());
    } catch (Exception e) {
      freemind.main.Resources.getInstance().logException(e);
    }
  ...
  private String decryptXml(String encryptedString, StringBuffer pwd) {
    ...
    // logger.warning("Perhaps wrong algorithm used (due to a Java bug, in FreeMind 0.8.0 and
    // Java4-5 DES whereas with Java6 Triple DES was used. Trying Triple DES...");
    // decrypted = new
  ...
  /**
   * isShuttingDown is used to fold an encrypted node properly. If it is encrypted, it has no
   * children. Thus, the formely existing children can't be removed. Thus, this flag postpones the
   * childlessness of a node until it tree structure is updated.
   *
  ...
  /**
   * @return Returns the isAccessible (ie. if the node is decrypted (isAccessible==true) or not).
   */
