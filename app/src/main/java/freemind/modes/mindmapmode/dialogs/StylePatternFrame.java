...
public class StylePatternFrame extends JPanel implements TextTranslator, PropertyChangeListener {
  ...
  private static final String[] EDGE_STYLES =
      new String[] {
  ...
  private static final String[] EDGE_WIDTHS =
      new String[] {
        "EdgeWidth_parent",
        "EdgeWidth_thin",
        "EdgeWidth_1",
  ...
  /**
   * Denotes pairs property -> ThreeCheckBoxProperty such that the boolean property can be set, when
   * the format property is changed.
   */
  private HashMap<ThreeCheckBoxProperty, PropertyControl> mPropertyChangePropagation =
      new HashMap<>();
   ...
   */
  public StylePatternFrame(
      TextTranslator pTranslator, MindMapController pMindMapController, StylePatternFrameType pType)
      throws HeadlessException {
    super();
  ...
  private Vector<PropertyControl> getControls() {
    ...
    if (StylePatternFrameType.WITH_NAME_AND_CHILDS.equals(mType)) {
      ...
      // child pattern
      mSetChildPattern =
          new ThreeCheckBoxProperty(SET_CHILD_PATTERN + ".tooltip", SET_CHILD_PATTERN);
      controls.add(mSetChildPattern);
      Vector<String> childNames = new Vector<>();
      mChildPattern =
          new ComboProperty(CHILD_PATTERN + ".tooltip", CHILD_PATTERN, childNames, childNames);
    ...
    mSetNodeBackgroundColor =
        new ThreeCheckBoxProperty(
            SET_NODE_BACKGROUND_COLOR + ".tooltip", SET_NODE_BACKGROUND_COLOR);
    controls.add(mSetNodeBackgroundColor);
    mNodeBackgroundColor =
        new ColorProperty(
            NODE_BACKGROUND_COLOR + ".tooltip",
    ...
    mNodeStyle =
        new ComboProperty(NODE_STYLE + ".tooltip", NODE_STYLE, MindMapNode.NODE_STYLES, this);
    controls.add(mNodeStyle);
    mIconInformationVector = new Vector<>();
    Vector<IconAction> iconActions = mMindMapController.iconActions;
    ...
    mIcon =
        new IconProperty(
            ICON + ".tooltip", ICON, mMindMapController.getFrame(), mIconInformationVector);
    controls.add(mIcon);
    controls.add(new NextLineProperty());
    controls.add(new SeparatorProperty("NodeFont"));
    mSetNodeFontName =
        new ThreeCheckBoxProperty(SET_NODE_FONT_NAME + ".tooltip", SET_NODE_FONT_NAME);
    ...
    mSetNodeFontSize =
        new ThreeCheckBoxProperty(SET_NODE_FONT_SIZE + ".tooltip", SET_NODE_FONT_SIZE);
    controls.add(mSetNodeFontSize);
    Vector<String> sizesVector = new Vector<>();
    Collections.addAll(sizesVector, sizes);
    mNodeFontSize =
        new IntegerComboProperty(NODE_FONT_SIZE + ".tooltip", NODE_FONT_SIZE, sizes, sizesVector);
    controls.add(mNodeFontSize);
    mSetNodeFontBold =
        new ThreeCheckBoxProperty(SET_NODE_FONT_BOLD + ".tooltip", SET_NODE_FONT_BOLD);
    ...
    mSetNodeFontStrikethrough =
        new ThreeCheckBoxProperty(
            SET_NODE_FONT_STRIKETHROUGH + ".tooltip", SET_NODE_FONT_STRIKETHROUGH);
    controls.add(mSetNodeFontStrikethrough);
    mNodeFontStrikethrough =
        new BooleanProperty(NODE_FONT_STRIKETHROUGH + ".tooltip", NODE_FONT_STRIKETHROUGH);
    controls.add(mNodeFontStrikethrough);
    mSetNodeFontItalic =
        new ThreeCheckBoxProperty(SET_NODE_FONT_ITALIC + ".tooltip", SET_NODE_FONT_ITALIC);
  ...
  public void setPattern(Pattern pattern) {
    FreeMind fmMain = (FreeMind) mMindMapController.getFrame();
    setPatternControls(
        pattern.getPatternNodeColor(),
        mSetNodeColor,
        mNodeColor,
        fmMain.getDefaultProperty(FreeMind.RESOURCES_NODE_TEXT_COLOR));
    setPatternControls(
        pattern.getPatternNodeBackgroundColor(),
        mSetNodeBackgroundColor,
        mNodeBackgroundColor,
        fmMain.getDefaultProperty(FreeMind.RESOURCES_BACKGROUND_COLOR));
    setPatternControls(
        pattern.getPatternNodeStyle(), mSetNodeStyle, mNodeStyle, MindMapNode.STYLE_AS_PARENT);
    setPatternControls(pattern.getPatternNodeText(), mSetNodeText, mNodeText, "");
    setPatternControls(
        pattern.getPatternEdgeColor(),
        mSetEdgeColor,
        mEdgeColor,
        fmMain.getDefaultProperty(FreeMind.RESOURCES_EDGE_COLOR));
    setPatternControls(pattern.getPatternEdgeStyle(), mSetEdgeStyle, mEdgeStyle, EDGE_STYLES[0]);
    setPatternControls(
    ...
    setPatternControls(
        ...
        mSetNodeFontName,
        mNodeFontName,
        mMindMapController.getController().getDefaultFontFamilyName());
    setPatternControls(pattern.getPatternNodeFontSize(), mSetNodeFontSize, mNodeFontSize, sizes[0]);
    setPatternControls(
        pattern.getPatternNodeFontBold(),
        mSetNodeFontBold,
        mNodeFontBold,
        BooleanProperty.TRUE_VALUE);
    setPatternControls(
        pattern.getPatternNodeFontStrikethrough(),
        mSetNodeFontStrikethrough,
        mNodeFontStrikethrough,
        BooleanProperty.TRUE_VALUE);
    setPatternControls(
    ...
    if (StylePatternFrameType.WITH_NAME_AND_CHILDS.equals(mType)) {
      ...
      setPatternControls(
          ...
          mSetChildPattern,
          mChildPattern,
          (!mPatternList.isEmpty()) ? mPatternList.get(0).getName() : null);
    }
  ...
  private void setPatternControls(
      PatternPropertyBase patternProperty,
      PropertyBean threeCheckBoxProperty,
      PropertyBean property,
      String defaultValue) {
    setPatternControls(
        patternProperty, threeCheckBoxProperty, property, defaultValue, new IdentityTransformer());
  }
  ...
  private String transformStringToWidth(String value) {
    HashMap<String, Integer> transformator = getEdgeWidthTransformation();
    int intWidth = transformator.get(value);
  ...
  private HashMap<String, Integer> getEdgeWidthTransformation() {
    ...
    transformator.put(EDGE_WIDTHS[0], EdgeAdapter.WIDTH_PARENT);
  ...
  public Pattern getResultPattern(Pattern pattern) {
    pattern.setPatternNodeColor(
        (PatternNodeColor) getPatternResult(new PatternNodeColor(), mSetNodeColor, mNodeColor));
    pattern.setPatternNodeBackgroundColor(
        (PatternNodeBackgroundColor)
            getPatternResult(
                new PatternNodeBackgroundColor(), mSetNodeBackgroundColor, mNodeBackgroundColor));
    pattern.setPatternNodeStyle(
        (PatternNodeStyle) getPatternResult(new PatternNodeStyle(), mSetNodeStyle, mNodeStyle));
    pattern.setPatternNodeText(
        (PatternNodeText) getPatternResult(new PatternNodeText(), mSetNodeText, mNodeText));
    /* edges */
    pattern.setPatternEdgeColor(
        (PatternEdgeColor) getPatternResult(new PatternEdgeColor(), mSetEdgeColor, mEdgeColor));
    pattern.setPatternEdgeStyle(
        (PatternEdgeStyle) getPatternResult(new PatternEdgeStyle(), mSetEdgeStyle, mEdgeStyle));
    pattern.setPatternEdgeWidth(
        (PatternEdgeWidth)
            getPatternResult(
                new PatternEdgeWidth(), mSetEdgeWidth, mEdgeWidth, new EdgeWidthBackTransformer()));
    /* font */
    pattern.setPatternNodeFontName(
        (PatternNodeFontName)
            getPatternResult(new PatternNodeFontName(), mSetNodeFontName, mNodeFontName));
    pattern.setPatternNodeFontSize(
        (PatternNodeFontSize)
            getPatternResult(new PatternNodeFontSize(), mSetNodeFontSize, mNodeFontSize));
    pattern.setPatternNodeFontBold(
        (PatternNodeFontBold)
            getPatternResult(new PatternNodeFontBold(), mSetNodeFontBold, mNodeFontBold));
    pattern.setPatternNodeFontStrikethrough(
        (PatternNodeFontStrikethrough)
            getPatternResult(
                new PatternNodeFontStrikethrough(),
                mSetNodeFontStrikethrough,
                mNodeFontStrikethrough));
    pattern.setPatternNodeFontItalic(
        (PatternNodeFontItalic)
            getPatternResult(new PatternNodeFontItalic(), mSetNodeFontItalic, mNodeFontItalic));
    pattern.setPatternIcon((PatternIcon) getPatternResult(new PatternIcon(), mSetIcon, mIcon));
    if (StylePatternFrameType.WITH_NAME_AND_CHILDS.equals(mType)) {
      pattern.setName(mName.getValue());
      pattern.setPatternChild(
          (PatternChild) getPatternResult(new PatternChild(), mSetChildPattern, mChildPattern));
    }
    return pattern;
  }
  ...
  private PatternPropertyBase getPatternResult(
      ...
      ThreeCheckBoxProperty threeCheckBoxProperty,
      PropertyBean property) {
  ...
  /** Used to enable/disable the attribute controls, if the check boxes are changed. */
  public void propertyChange(PropertyChangeEvent pEvt) {
    ...
    if (mPropertyChangePropagation.containsKey(pEvt.getSource())) {
      ...
      PropertyControl bean = mPropertyChangePropagation.get(booleanProp);
