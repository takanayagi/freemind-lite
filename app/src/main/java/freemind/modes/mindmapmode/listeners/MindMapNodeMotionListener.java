...
public class MindMapNodeMotionListener extends NodeMotionAdapter {
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if ((e.getModifiersEx() & InputEvent.BUTTON1_DOWN_MASK) == (InputEvent.BUTTON1_DOWN_MASK)) {
      ...
      Tools.convertPointToAncestor(motionListenerView, dragNextPoint, JScrollPane.class);
      if (!isActive()) {
        setDragStartingPoint(dragNextPoint, node);
      } else {
  ...
  private int getHGap(Point dragNextPoint, MindMapNode node, Point dragStartingPoint) {
    ...
    if (node.isLeft()) hGapChange = -hGapChange;
    oldHGap += hGapChange;
    return oldHGap;
  }
  ...
  public void mouseClicked(MouseEvent e) {
    if (e.getButton() == 1 && e.getClickCount() == 2) {
      ...
      if (e.getModifiersEx() == InputEvent.CTRL_DOWN_MASK) {
        NodeView nodeV = getNodeView(e);
        MindMapNode node = nodeV.getModel();
        c.moveNodePosition(node, NodeAdapter.VGAP, node.getHGap(), node.getShiftY());
      }
    }
  }
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    NodeMotionListenerView v = (NodeMotionListenerView) e.getSource();
    if (!v.contains(e.getX(), e.getY())) v.setMouseExited();
    if (!isActive()) return;
    NodeView nodeV = getNodeView(e);
