...
public class MindMapNodeDropListener implements DropTargetListener {
  ...
  private boolean isDropAcceptable(DropTargetDropEvent event) {
    ...
    if (!node.isWriteable()) return false;
        ...
        if (mMindMapController.getView().getSelecteds().size() >= 5) {
          yesorno =
              JOptionPane.showConfirmDialog(
                  ...
                  mMindMapController.getView().getSelecteds().size() + " links to the same node",
   ...
   *
   * <p>I think the difference between dragAcceptable and dropAcceptable is that in dragAcceptable,
