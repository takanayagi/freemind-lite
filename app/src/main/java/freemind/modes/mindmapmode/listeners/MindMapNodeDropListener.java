...
public class MindMapNodeDropListener implements DropTargetListener {
	...
	private boolean isDropAcceptable(DropTargetDropEvent event) {
		MindMapNode node =
				((MainView) event.getDropTargetContext().getComponent()).getNodeView().getModel();
	...
	public void drop(DropTargetDropEvent dtde) {
		try {
			...
			if (dtde.isLocalTransfer()
				...
				String sourceAction =
						(String) t.getTransferData(MindMapNodesSelection.dropActionFlavor);
			...
			if (dropAction == DnDConstants.ACTION_LINK) {
				...
				// MindMapMapModel mindMapMapModel = (MindMapMapModel)
				...
				if (mMindMapController.getView().getSelecteds().size() >= 5) {
					yesorno = JOptionPane.showConfirmDialog(
							mMindMapController.getFrame().getContentPane(),
							mMindMapController.getText("lots_of_links_warning"),
							mMindMapController.getView().getSelecteds().size()
									+ " links to the same node",
							JOptionPane.YES_NO_OPTION);
				}
				if (yesorno == JOptionPane.YES_OPTION) {
					for (NodeView nodeView : mMindMapController.getView().getSelecteds()) {
				...
				if (!targetNode.isWriteable()) {
					...
					JOptionPane.showMessageDialog(mMindMapController.getFrame().getContentPane(),
							message, "Freemind", JOptionPane.ERROR_MESSAGE);
					return;
				}
					...
					do {
						if (selecteds.contains(actualNode)) {
							...
							JOptionPane.showMessageDialog(
									mMindMapController.getFrame().getContentPane(), message,
				...
				if (!result && DnDConstants.ACTION_MOVE == dropAction) {
					// FIXME an error occurred. how to react?
	 ...
	 * The name "dragEnter" seems to be confusing to me.
	 * 
	 * I think the difference between dragAcceptable and dropAcceptable is that in dragAcceptable,
	 * you tell if the type of the thing being dragged is OK, where in dropAcceptable, you tell if
	 * your really willing to accept the item.
	 */
