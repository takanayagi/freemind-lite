/*
 * Created on 04.02.2007 Copyright (C) 2006 Dimitri Polivaev
 */
...
class MindMapHTMLWriter {
	...
	MindMapHTMLWriter(Writer fileout) {
		...
		basedOnHeadings =
				getProperty("html_export_folding").equals("html_export_based_on_headings");
	}
	...
	void saveHTML(MindMapNodeModel rootNodeOfBranch) throws IOException {
		...
		writeFoldingCode = (htmlExportFoldingOption.equals("html_export_fold_currently_folded")
	...
	private void writeBodyWithFolding(MindMapNodeModel rootNodeOfBranch) throws IOException {
		...
		fileout.write(
				"<SPAN class=\"foldspecial\" onclick=\"unfold_document()\">All -</SPAN>" + el);
	...
	private void writeJavaScript() throws IOException {
		fileout.write(el + "<script type=\"text/javascript\">" + el
				+ "   // Here we implement folding. It works fine with MSIE5.5, MSIE6.0 and" + el
				+ "   // Mozilla 0.9.6." + el + el + "   if (document.layers) {" + el
				+ "      //Netscape 4 specific code" + el + "      pre = 'document.';" + el
				+ "      post = ''; }" + el + "   if (document.getElementById) {" + el
				+ "      //Netscape 6 specific code" + el
				+ "      pre = 'document.getElementById(\"';" + el + "      post = '\").style'; }"
				+ el + "   if (document.all) {" + el + "      //IE4+ specific code" + el
				+ "      pre = 'document.all.';" + el + "      post = '.style'; }" + el + el
				+ "function layer_exists(layer) {" + el + "   try {" + el
				+ "      eval(pre + layer + post);" + el + "      return true; }" + el
				+ "   catch (error) {" + el + "      return false; }}" + el + el
				+ "function show_layer(layer) {" + el
				+ "   eval(pre + layer + post).position = 'relative'; " + el
				+ "   eval(pre + layer + post).visibility = 'visible'; }" + el + el
				+ "function hide_layer(layer) {" + el
				+ "   eval(pre + layer + post).visibility = 'hidden';" + el
				+ "   eval(pre + layer + post).position = 'absolute'; }" + el + el
				+ "function hide_folder(folder) {" + el + "    hide_folding_layer(folder)" + el
				+ "    show_layer('show'+folder);" + el + el
				+ "    scrollBy(0,0); // This is a work around to make it work in Browsers (Explorer, Mozilla)"
				+ el + "}" + el + el + "function show_folder(folder) {" + el
				+ "    // Precondition: all subfolders are folded" + el + el
				+ "    show_layer('hide'+folder);" + el + "    hide_layer('show'+folder);" + el
				+ "    show_layer('fold'+folder);" + el + el
				+ "    scrollBy(0,0); // This is a work around to make it work in Browsers (Explorer, Mozilla)"
				+ el + el + "    var i;" + el
				+ "    for (i=1; layer_exists('fold'+folder+'_'+i); ++i) {" + el
				+ "       show_layer('show'+folder+'_'+i); }" + el + "}" + el
				...
				+ el + el + "    var i;" + el
				+ "    for (i=1; layer_exists('fold'+folder+'_'+i); ++i) {" + el
				+ "       show_folder_completely(folder+'_'+i); }" + el + "}" + el + el
				+ el + el + "function hide_folding_layer(folder) {" + el + "   var i;" + el
				+ "   for (i=1; layer_exists('fold'+folder+'_'+i); ++i) {" + el
				+ "       hide_folding_layer(folder+'_'+i); }" + el + el
				+ "   hide_layer('hide'+folder);" + el + "   hide_layer('show'+folder);" + el
				+ "   hide_layer('fold'+folder);" + el + el
				+ "   scrollBy(0,0); // This is a work around to make it work in Browsers (Explorer, Mozilla)"
				+ el + "}" + el + el + "function fold_document() {" + el + "   var i;" + el
				+ "   var folder = '1';" + el
				+ "   for (i=1; layer_exists('fold'+folder+'_'+i); ++i) {" + el
				+ "       hide_folder(folder+'_'+i); }" + el + "}" + el + el
				+ "function unfold_document() {" + el + "   var i;" + el + "   var folder = '1';"
				+ el + "   for (i=1; layer_exists('fold'+folder+'_'+i); ++i) {" + el
				+ "       show_folder_completely(folder+'_'+i); }" + el + "}" + el + el
				+ "</script>" + el);
	}
	...
	private void writeStyle() throws IOException {
		...
		if (writeFoldingCode) {
			fileout.write("    span.foldopened { color: white; font-size: xx-small;" + el
					+ "    border-width: 1; font-family: monospace; padding: 0em 0.25em 0em 0.25em; background: #e0e0e0;"
					+ el + "    VISIBILITY: visible;" + el + "    cursor:pointer; }" + el + el
					+ el + "    span.foldclosed { color: #666666; font-size: xx-small;" + el
					+ "    border-width: 1; font-family: monospace; padding: 0em 0.25em 0em 0.25em; background: #e0e0e0;"
					+ el + "    VISIBILITY: hidden;" + el + "    cursor:pointer; }" + el + el
		...
		fileout.write(el + "    span.l { color: red; font-weight: bold; }" + el + el
	...
	private int saveHTML(MindMapNodeModel model, String parentID, int lastChildNumber,
		...
		if (model.hasChildren()) {
			if (getProperty("html_export_folding").equals("html_export_based_on_headings")) {
				for (ListIterator<MindMapNode> e = model.sortedChildrenUnfolded(); e.hasNext();) {
					MindMapNodeModel child = (MindMapNodeModel) e.next();
					lastChildNumber = saveHTML(child, parentID, lastChildNumber, /* isRoot= */false,
				...
				for (ListIterator<MindMapNode> e = model.sortedChildrenUnfolded(); e.hasNext();) {
					MindMapNodeModel child = (MindMapNodeModel) e.next();
					localLastChildNumber = saveHTML(child, localParentID, localLastChildNumber,
	...
	private String fontStyle(MindMapNodeModel model) {
		...
		if (model.getFont() != null && model.getFont().getSize() != 0) {
			int defaultFontSize = Integer.parseInt(getProperty("defaultfontsize"));
	...
	private void writeModelContent(MindMapNodeModel model) throws IOException {
		if (model.toString().matches(" *")) {
			...
			if (HtmlTools.isHtmlNode(output)) {
				...
				fileout.write(saveHTML_escapeUnicodeAndSpecialCharacters(model.toString()));
			}
		}
	...
	private void writeIcons(MindMapNodeModel model) throws IOException {
		for (int i = 0; i < model.getIcons().size(); ++i) {
			fileout.write("<img src=\"" + model.getIcons().get(i).getIconFileName()
					+ "\" alt=\"" + model.getIcons().get(i).getDescription() + "\">");
		}
	}
	...
	private boolean isHeading(MindMapNodeModel model, int depth) {
		return basedOnHeadings && model.hasChildren() && depth <= 6 && !hasHtml(model);
	}
