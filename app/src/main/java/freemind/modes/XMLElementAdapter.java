...
public class XMLElementAdapter extends XMLElement {
	...
	public void setName(String name) {
		...
		switch (name) {
			...
			case "edge" -> userObject = getMap().createEdgeAdapter(null);
			case "cloud" -> userObject = getMap().createCloudAdapter(null);
			case "arrowlink" -> userObject = getMap().createArrowLinkAdapter(null, null);
			case "linktarget" -> userObject = getMap().createArrowLinkTarget(null, null);
			case "font" -> userObject = null;
			case XML_NODE_ATTRIBUTE -> userObject = null;
			case XML_NODE_ATTRIBUTE_LAYOUT -> userObject = null;
			case "map" -> userObject = null;
			case XML_NODE_ATTRIBUTE_REGISTRY -> userObject = null;
			case XML_NODE_REGISTERED_ATTRIBUTE_NAME -> userObject = null;
			case XML_NODE_REGISTERED_ATTRIBUTE_VALUE -> userObject = null;
			case "icon" -> userObject = null;
			case "hook" ->
				// we gather the xml element and send it to the hook after
				// completion.
					userObject = new XMLElement();
			default -> userObject = new XMLElement(); // for childs of hooks
		}
	}
	...
	public void addChild(XMLElement child) {
		...
		if (userObject instanceof NodeAdapter node) {
			if (child.getUserObject() instanceof NodeAdapter) {
			...
			else if (child.getUserObject() instanceof EdgeAdapter edge) {
				edge.setTarget(node);
				node.setEdge(edge);
			} else if (child.getUserObject() instanceof CloudAdapter cloud) {
				cloud.setTarget(node);
				node.setCloud(cloud);
			} else if (child.getUserObject() instanceof ArrowLinkTarget arrowLinkTarget) {
			...
			} else if (child.getUserObject() instanceof ArrowLinkAdapter arrowLink) {
				...
				node.addAttribute((Attribute) child.getUserObject());
				...
				String loadName = (String) child.getAttribute("NAME");
				PermanentNodeHook hook = null;
				try {
					// loadName=loadName.replace('/', File.separatorChar);
					/*
					 * The next code snippet is an exception. Normally, hooks have to be created via
					 * the ModeController. DO NOT COPY.
					 */
	...
	public void setAttribute(String name, Object value) {
		...
		if (userObject instanceof EdgeAdapter edge) {
			switch (name) {
				case "STYLE" -> edge.setStyle(sValue);
				case "COLOR" -> edge.setColor(Tools.xmlToColor(sValue));
				case "WIDTH" -> {
		...
		if (userObject instanceof CloudAdapter cloud) {
			switch (name) {
				case "STYLE" -> cloud.setStyle(sValue);
				case "COLOR" -> cloud.setColor(Tools.xmlToColor(sValue));
				case "WIDTH" -> cloud.setWidth(Integer.parseInt(sValue));
			}
			return;
		}
		...
		if (userObject instanceof ArrowLinkAdapter arrowLink) {
			switch (name) {
				case "STYLE" -> arrowLink.setStyle(sValue);
				case "ID" -> arrowLink.setUniqueId(sValue);
				case "COLOR" -> arrowLink.setColor(Tools.xmlToColor(sValue));
				case "DESTINATION" -> arrowLink.setDestinationLabel(sValue);
				case "REFERENCETEXT" -> arrowLink.setReferenceText((sValue));
				case "STARTINCLINATION" -> arrowLink.setStartInclination(Tools.xmlToPoint(sValue));
				case "ENDINCLINATION" -> arrowLink.setEndInclination(Tools.xmlToPoint(sValue));
				case "STARTARROW" -> arrowLink.setStartArrow(sValue);
				case "ENDARROW" -> arrowLink.setEndArrow(sValue);
				case "WIDTH" -> arrowLink.setWidth(Integer.parseInt(sValue));
			}
		...
		if (getName().equals("font")) {
			...
			// Styling
				...
				switch (name) {
					...
					case "STRIKETHROUGH" -> fontStyleStrikethrough = true;
				}
			}
		}
		/* icons */
	...
	private NodeAdapter setNodeAttribute(String name, String sValue, NodeAdapter node) {
		switch (name) {
			...
			case "LINK" -> node.setLink(sValue);
			case "STYLE" -> node.setStyle(sValue);
			case "ID" ->
				// do not set label but annotate in list:
				// System.out.println("(sValue, node) = " + sValue + ", "+ node);
					mIdToTarget.put(sValue, node);
			case "VSHIFT" -> node.setShiftY(Integer.parseInt(sValue));
			case "VGAP" -> node.setVGap(Integer.parseInt(sValue));
			case "HGAP" -> node.setHGap(Integer.parseInt(sValue));
		}
		return node;
	}

	/**
	 * Sets all attributes that were formely applied to the current userObject to a given (new)
	 * node. Thus, the instance of a node can be changed after the creation. (At the moment,
	 * relevant for encrypted nodes).
	 */
		...
		switch (getName()) {
			...
			case "font" -> {
				...
				if (fontStyleStrikethrough) {
					Map<TextAttribute, Object> attr = new HashMap<>(font.getAttributes());
					attr.put(TextAttribute.STRIKETHROUGH, TextAttribute.STRIKETHROUGH_ON);
					font = new Font(attr);
				}
				userObject = mMapFeedback.getFontThroughMap(font);
			}
			case "icon" -> userObject = MindIcon.factory(iconName);
			case XML_NODE_ATTRIBUTE -> userObject = new Attribute(attributeName, attributeValue);
	 ...
	 * 
	 * Case I: Source+Destination are pasted (Ia: cut, Ib: copy) Case II: Source is pasted,
	 * Destination remains unchanged in the map (IIa: cut, IIb: copy) Case III: Destination is
	 * pasted, Source remains unchanged in the map (IIIa: cut, IIIb: copy)
	 */
	public void processUnfinishedLinks(MindMapLinkRegistry registry) {
		...
		for (String key : mIdToTarget.keySet()) {
			...
			/*
			 * key is the proposed name for the target, is changed by the registry, if already
			 * present.
			 */
		...
		for (Object arrowObject : mArrowLinkAdapters) {
			if (arrowObject instanceof ArrowLinkTarget linkTarget) {
				...
				if (mIdToTarget.containsKey(oldId)) {
					...
					logger.severe("Cannot find the label " + oldId + " in the map. The link target "
				...
				if (!mIdToTarget.containsKey(targetCurrentId)) {
					...
					if (link == null) {
						logger.severe("Cannot find the label " + linkTarget.getUniqueId()
								+ " for the link in the map. The link target " + linkTarget
								+ " is not restored.");
					} else {
						// double the link.
				...
				if (mIdToTarget.containsKey(oldId)) {
					...
					logger.severe("Cannot find the label " + oldId + " in the map. The link "
							+ arrowLink + " is not restored.");
					continue;
				}
				// set the new ID:
