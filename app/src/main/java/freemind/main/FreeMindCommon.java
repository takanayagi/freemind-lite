...
public class FreeMindCommon {
   ...
   *
   * <p>If FreeMind is not started and you double-click a .mm file on Mac OS X the .mm file is not
   * passed to Java's main method but handleOpenFile is called which happens during startup where it
   * is not safe to already load the map. Therefore the event is stored in this property and later
              ...
              new File(
                  URLDecoder.decode(
                      url.getPath()
                          ...
                          .replaceFirst("!.*$", "")
                          .replaceFirst(classname.replace('.', '/') + ".class$", ""),
                      "UTF-8"));
  ...
  public String getAdjustableProperty(final String label) {
    ...
    if (value.startsWith("?") && !value.equals("?")) {
      ...
      String localValue =
          ((FreeMindResourceBundle) getResources())
