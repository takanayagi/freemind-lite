...
public class FreeMindCommon {
	...
	private class FreeMindResourceBundle extends ResourceBundle {
		...
		FreeMindResourceBundle() {
			try {
				 ...
				 * even, when a single string is missing inside a bundle and not only, when the
				 * complete bundle is missing.
				 */
		...
		protected Object handleGetObject(String key) {
			try {
				...
				if (key != null && key.startsWith("__")) {
					...
					logger.config("Resource string not found: " + key);
		...
		String getResourceString(String key, String pDefault) {
			try {
				try {
					...
					String defaultString = defaultResources.getString(key);
					logger.config("Using default resource string for missing key: " + key);
					return defaultString + POSTFIX_TRANSLATE_ME;
				}
			} catch (Exception e) {
				logger.config("No default resource string found, using system default: " + key);
				return pDefault;
			}
		}
	}
	...
	public static final String SAVE_ONLY_INTRISICALLY_NEEDED_IDS =
	 ...
	 * <p>
	 * If FreeMind is not started and you double-click a .mm file on Mac OS X the .mm file is not
	 * passed to Java's main method but handleOpenFile is called which happens during startup where
	 * it is not safe to already load the map. Therefore the event is stored in this property and
	 * later processed by loadMaps.
	 * <p>
	 * Related issues
	 * <ul>
	...
	public String getFreemindBaseDirOld() {
		if (baseDir == null) {
			...
			/*
			 * fc: Now, if freemind.jar is the first, firstpos == -1. This results in bad results in
			 * the substring method, or not??
			 */
			int firstpos = classPath.lastIndexOf(File.pathSeparator, lastpos) + 1;
			lastpos -= 1;
			if (lastpos > firstpos) {
				logger.info(
						"First " + firstpos + " and last " + lastpos + " and string " + classPath);
			 ...
			 * Example: /home/foltin/freemindapp/lib/freemind.jar gives /home/foltin/freemindapp
			 */
	...
	/*
	 * We define the base dir of FreeMind as the directory where accessories, plugins and other
	 * things are to be found. We expect it to be either the directory where the main jar file is
	 * (freemind.jar), or the root of the class hierarchy (if no jar file is used), after any 'lib'
	 * directory is removed. One can overwrite this definition by setting the freemind.base.dir
	 * property.
	 */
			...
			try {
				...
				if (dir == null) {
					...
					URL url = this.getClass()
							.getResource(classname.replaceFirst(
					 ...
					 * /home/foltin/freemindapp/lib/freemind.jar gives /home/foltin/freemindapp
					 */
				...
				if (!file.exists()) {
					throw new IllegalArgumentException(
							"FreeMind base dir '" + file + "' does not exist.");
				}
				if (!file.isDirectory()) {
					throw new IllegalArgumentException(
							"FreeMind base dir (!) '" + file + "' is not a directory.");
				}
