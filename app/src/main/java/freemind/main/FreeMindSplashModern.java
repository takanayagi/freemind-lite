...
public class FreeMindSplashModern extends JFrame implements IFreeMindSplash {
	...
	private class FeedBackImpl implements FeedBack {
		...
		public void progress(final int act, String messageId, Object[] pMessageParameters) {
			MessageFormat formatter = new MessageFormat(frame.getResourceString(messageId));
			...
			logger.fine("Task: " + lastTaskId + " (" + act + ") last " + (timeDifference) / 1000.0
	...
	public FreeMindSplashModern(final FreeMindMain frame) {
		...
		JLabel splashImageLabel = new JLabel(splashImage) {
			...
			private Font versionTextFont = Tools.isAvailableFontFamily("Century Gothic")
			...
			public void paint(Graphics g) {
				...
				int xCoordinate = (int) (getSize().getWidth() / 2.0 - mWidth / 2.0);
				...
				if (progressString != null) {
					...
					g2.drawString(progressString, xBase + calcXRelative(1),
							yBase - calcYRelative(4));
					g2.setColor(new Color(0xc8, 0xdf, 0x8b));
					g2.draw(new Rectangle(xBase + calcXRelative(2), yBase, width,
							calcYRelative(3)));
