 ...
 * {@link #enumerateAttributeNames() enumerateAttributeNames}. The attribute values can be retrieved
 * using the method {@link #getStringAttribute(java.lang.String) getStringAttribute}. The following
 ...
 * enumerateChildren}. The number of child elements can be retrieved using {@link #countChildren()
 * countChildren}.</DD>
 * </DL>
 * <DL>
 ...
 * createAnotherElement} which has to return a new copy of the receiver.</DD>
 * </DL>
 * <P>
 * 
 * 
...
public class XMLElement {
	...
	/**
	 * Major version of NanoXML. Classes with the same major and minor version are binary
	 * compatible. Classes with the same major version are source compatible. If the major version
	 * is different, you may need to modify the client source code.
	 * 
	...
	/**
	 * Minor version of NanoXML. Classes with the same major and minor version are binary
	 * compatible. Classes with the same major version are source compatible. If the major version
	 * is different, you may need to modify the client source code.
	 * 
	 ...
	 * <dd>
	 * <ul>
	 * <li>The field is <code>null</code> iff the element is not initialized by either parse or
	 * setName.
	 ...
	 * 
	 * The entity conversion table. <code>true</code> if leading and trailing whitespace in PCDATA
	 * content has to be removed. <code>true</code> if the case of element and attribute names have
	 * to be ignored.
	 * 
	 * </dl>
	 * <dl>
	 ...
	 * Creates and initializes a new XML element.
	 * <P>
	 * This constructor should <I>only</I> be called from {@link #createAnotherElement()
	 ...
	 * <ul>
	 * <li><code>entities != null</code>
	 * <li>if <code>fillBasicConversionTable == false</code> then <code>entities</code> contains at
	 * least the following entries: <code>amp</code>, <code>lt</code>, <code>gt</code>,
	 ...
	 * The name of the attribute. The value of the attribute.
	 * 
	 * @deprecated Use {@link #setAttribute(java.lang.String, java.lang.Object) setAttribute}
	 *             instead.
	 */
	@Deprecated
	 ...
	 * "1"</code> and the element contains the attribute <code>attr="one"</code> , then
	 * <code>getAttribute("attr", mapping, defaultKey, false)</code> returns <code>"1"</code>.
	 * 
	 * The name of the attribute. Hashtable mapping keys to values. Key to use if the attribute is
	 ...
	 *      getAttribute(String, Object)
	 */
	public Object getAttribute(String name, Hashtable<String, ?> valueSet, String defaultKey,
	 ...
	 *      getStringAttribute(String, String)
	 */
	public String getStringAttribute(String name, Hashtable<String, String> valueSet,
	 ...
	 * value corresponding to defaultKey is returned.
	 * <P>
	 * As an example, if valueSet contains the mapping <code>"one" => 1</code> and the element
	 * contains the attribute <code>attr="one"</code>, then
	 ...
	 * <li><code>defaultKey</code> is either <code>null</code>, a key in <code>valueSet</code> or an
	 * integer.
	 * </ul>
	 * </dd>
	 * </dl>
	 ...
	 *      int)
	 */
	public int getIntAttribute(String name, Hashtable<Object, Integer> valueSet, String defaultKey,
	 ...
	 * <P>
	 * As an example, if valueSet contains the mapping <code>"one" =&gt;
	 * 1.0</code> and the element contains the attribute <code>attr="one"</code> , then
	 * <code>getDoubleAttribute("attr", mapping, defaultKey, false)</code> returns <code>1.0</code>.
	 * 
	 * The name of the attribute. Hashtable mapping keys to values. Key to use if the attribute is
	 * missing. <code>true</code> if literal numbers are valid.
	 * 
	 ...
	 * <li><code>defaultKey</code> is either <code>null</code>, a key in <code>valueSet</code> or a
	 * double.
	 * </ul>
	 * </dd>
	 * </dl>
	...
	/**
	 * Returns an attribute of the element. If the attribute doesn't exist,
	 * <code>defaultValue</code> is returned. If the value of the attribute is equal to
	 * <code>trueValue</code>, <code>true</code> is returned. If the value of the attribute is equal
	 * to <code>falseValue</code>, <code>false</code> is returned. If the value doesn't match
	 ...
	 * <li>the reader points to the first character following the last '&gt;' character of the XML
	 * element
	 * </ul>
	 * </dd>
	 * </dl>
	 ...
	 * Reads one XML element from a java.io.Reader and parses it.
	 * 
	 * The reader from which to retrieve the XML data. The line number of the first line in the
	 * data.
	 * 
	 * </dl>
	 * <dl>
	 ...
	 * Reads one XML element from a String and parses it.
	 * 
	 * The reader from which to retrieve the XML data. The first character in <code>string</code> to
	 * scan.
	 * 
	 * </dl>
	 * <dl>
	 ...
	 * 
	 * The reader from which to retrieve the XML data. The first character in <code>string</code> to
	 * scan. The character where to stop scanning. This character is not scanned.
	 * 
	 * </dl>
	 * <dl>
	 * <dt><b>Preconditions:</b></dt>
	 ...
	 * 
	 * The reader from which to retrieve the XML data. The first character in <code>string</code> to
	 * scan. The character where to stop scanning. This character is not scanned. The line number of
	...
	/**
	 * Scans an identifier from the current reader. The scanned identifier is appended to
	 * <code>result</code>.
	 * 
	...
	/**
	 * This method scans an identifier from the current reader. The scanned whitespace is appended
	 * to <code>result</code>.
	 * 
	 * @return the next character following the whitespace.
	 * 
				...
				if (ch == '>' && lastOpeningBreak >= 0) {
					...
					if (sContentEndTagPattern == null) {
						sContentEndTagPattern =
								Pattern.compile(XML_NODE_XHTML_CONTENT_END_TAG_REGEXP);
					}
	...
	/**
	 * Resolves an entity. The name of the entity is read from the reader. The value of the entity
	 * is appended to <code>buf</code>.
	 * 
