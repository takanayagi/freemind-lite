...
class Preview extends JComponent {
  ...
  public Preview(MapView view, double zoom, PageFormat pPageFormat) {
    ...
    if (zoom == 0.0) {
      ...
      else this.zoom = DEFAULT_PREVIEW_SIZE / format.getWidth();
    } else this.zoom = zoom;
    resize();
  }
  ...
  public void paint(Graphics g) {
    ...
    if (previewPageImage == null) {
      previewPageImage =
          (BufferedImage) createImage(getPageWidth(format) - 1, getPageHeight(format) - 1);
  ...
  public void resize() {
    int size =
        (int) Math.max(getPageFormat().getWidth() * zoom, getPageFormat().getHeight() * zoom);
