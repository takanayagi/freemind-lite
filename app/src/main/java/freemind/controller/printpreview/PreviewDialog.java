 ...
 *
 * Copyright (C) 2003 Jens Kaiser. Created by Dimitri Polivaev.
 *
...
public class PreviewDialog extends JDialog implements ActionListener {
  ...
  private JButton getButton(String name, String iconName, AbstractAction action) {
    ...
    if (imageURL != null) icon = freemind.view.ImageFactory.getInstance().createIcon(imageURL);

    if (action != null) {
      if (icon != null)
        action.putValue(
            Action.SMALL_ICON, freemind.view.ImageFactory.getInstance().createIcon(imageURL));
