...
public class ConditionFactory {
  ...
  static final NamedObject FILTER_NODE =
      Resources.getInstance().createTranslatedString("filter_node");
  static final NamedObject FILTER_ICON =
  ...
  static String createDescription(
      String attribute, String simpleCondition, String value, boolean ignoreCase) {
    return attribute
        + " "
        + simpleCondition
        + (value != null ? " \"" + value + "\"" : "")
        + (ignoreCase && value != null
            ? ", " + Resources.getInstance().getResourceString(FILTER_IGNORE_CASE)
            : "");
  }
  ...
  public Condition createAttributeCondition(
      String attribute, NamedObject simpleCondition, String value, boolean ignoreCase) {
    if (simpleCondition.equals(FILTER_EXIST)) // XXX FILTER_EXIST should be NamedObject?
    return new AttributeExistsCondition(attribute);
    if (simpleCondition.equals(
        FILTER_DOES_NOT_EXIST)) // XXX FILTER_DOES_NOT_EXIST should be NamedObject?
  ...
  public Condition createCondition(
      NamedObject attribute, NamedObject simpleCondition, String value, boolean ignoreCase) {
  ...
  protected Condition createNodeCondition(
    ...
    if (ignoreCase) {
      ...
      if (simpleCondition.equals(FILTER_GT)) return new NodeCompareCondition(value, true, 1, true);
      if (simpleCondition.equals(FILTER_GE))
        return new NodeCompareCondition(value, true, -1, false);
      if (simpleCondition.equals(FILTER_LT)) return new NodeCompareCondition(value, true, -1, true);
      if (simpleCondition.equals(FILTER_LE)) return new NodeCompareCondition(value, true, 1, false);
    } else {
      if (simpleCondition.equals(FILTER_CONTAINS)) {
        if (value.isEmpty()) return null;
      ...
      if (simpleCondition.equals(FILTER_GT)) return new NodeCompareCondition(value, false, 1, true);
      if (simpleCondition.equals(FILTER_GE))
