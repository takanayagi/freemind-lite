 ...
 */
/*
 * Created on 05.05.2005 Copyright (C) 2005 Dimitri Polivaev
 */
...
public class FilterComposerDialog extends JDialog {
  ...
  private class AddConditionAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (selectedItem instanceof NamedObject attribute) {
        newCond =
            FilterController.getConditionFactory()
                .createCondition(attribute, simpleCond, value, ignoreCase);
      } else {
        String attribute = selectedItem.toString();
        newCond =
            FilterController.getConditionFactory()
                .createAttributeCondition(attribute, simpleCond, value, ignoreCase);
      }
      DefaultComboBoxModel<Condition> model =
          (DefaultComboBoxModel<Condition>) conditionList.getModel();
  ...
  private class CreateNotSatisfiedConditionAction extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (min >= 0) {
        ...
        if (min == max) {
          Condition oldCond = conditionList.getSelectedValue();
  ...
  private class CloseAction implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (source == btnOK || source == btnCancel) dispose();
      else initInternalConditionModel();
    }
  }

  private static class MindMapFilterFileFilter extends FileFilter {
    ...
    public boolean accept(File f) {
      ...
      if (extension != null) {
        return extension.equals(FilterController.FREEMIND_FILTER_EXTENSION_WITHOUT_DOT);
      }
      return false;
    }
  ...
  protected FreeMindFileDialog getFileChooser() {
    return Resources.getInstance().getStandardFileChooser(MindMapFilterFileFilter.filter);
  }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        try {
          ...
          mFilterController.loadConditions(internalConditionsModel, theFile.getCanonicalPath());
        } catch (Exception ex) {
    ...
    public void itemStateChanged(ItemEvent e) {
      if (e.getStateChange() == ItemEvent.SELECTED) {
        ...
        if (attributes.getSelectedIndex() > NODE_POSITION) {
          ...
          addAttributeValuesRecursively(
              attributeName, mController.getMap().getRootNode(), attributesInMap);
  ...
  private DefaultComboBoxModel<Condition> internalConditionsModel;
  ...
  public FilterComposerDialog(Controller controller, final FilterToolbar pFilterToolbar) {
    ...
    simpleNodeConditionComboBoxModel =
        new DefaultComboBoxModel<>(
            (Condition[]) FilterController.getConditionFactory().getNodeConditionNames());
    simpleIconConditionComboBoxModel =
        new DefaultComboBoxModel<>(
            (Condition[]) FilterController.getConditionFactory().getIconConditionNames());
    ...
    simpleAttributeConditionComboBoxModel =
        new DefaultComboBoxModel<>(
            (Condition[]) FilterController.getConditionFactory().getAttributeConditionNames());
    ...
    JLabel conditionColumnHeader =
        new JLabel(Resources.getInstance().getResourceString("filter_conditions"));
  ...
  private void addAttributeValuesRecursively(
      String pKey, MindMapNode pNode, SortedListModel pAttributesInMap) {
  ...
  void mapChanged(MindMap newMap) {
    if (newMap != null) {
      ...
      if (attributes.getSelectedIndex() > 1) attributes.setSelectedIndex(0);
  ...
  private void initInternalConditionModel() {
    ...
    for (int i = 2; i < externalConditionsModel.getSize(); i++) {
      final Condition element = externalConditionsModel.getElementAt(i);
