...
public class FilterComposerDialog extends JDialog {
	...
	private class AddConditionAction extends AbstractAction {
		...
		AddConditionAction() {
			...
			Tools.setLabelAndMnemonic(this,
					Resources.getInstance().getResourceString("filter_add"));
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (selectedItem instanceof NamedObject attribute) {
				...
				newCond = FilterController.getConditionFactory().createAttributeCondition(attribute,
						simpleCond, value, ignoreCase);
			}
	...
	private class DeleteConditionAction extends AbstractAction {
		...
		DeleteConditionAction() {
			super();
			Tools.setLabelAndMnemonic(this,
					Resources.getInstance().getResourceString("filter_delete"));
		}
	...
	private class CreateNotSatisfiedConditionAction extends AbstractAction {
		...
		CreateNotSatisfiedConditionAction() {
			super();
			Tools.setLabelAndMnemonic(this,
					Resources.getInstance().getResourceString("filter_not"));
		}
	...
	private class CreateConjunctConditionAction extends AbstractAction {
		...
		CreateConjunctConditionAction() {
			super();
			Tools.setLabelAndMnemonic(this,
					Resources.getInstance().getResourceString("filter_and"));
		}
		...
		public void itemStateChanged(ItemEvent e) {
			if (e.getStateChange() == ItemEvent.SELECTED) {
				...
				if (attributes.getSelectedIndex() > NODE_POSITION) {
					...
					addAttributeValuesRecursively(attributeName, mController.getMap().getRootNode(),
							attributesInMap);
	...
	private void addAttributeValuesRecursively(String pKey, MindMapNode pNode,
			SortedListModel pAttributesInMap) {
