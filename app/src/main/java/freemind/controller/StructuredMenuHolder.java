/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2000-2004 Joerg Mueller,
...
public class StructuredMenuHolder {
	...
	public StructuredMenuHolder() {
		...
		if (sSelectedIcon == null) {
			sSelectedIcon = freemind.view.ImageFactory.getInstance()
					.createIcon(Resources.getInstance().getResource(SELECTED_ICON_PATH));
		}
	...
	/**
	 * @param item is an action. If it derives from MenuItemSelectedListener, a check box is used.
	 */
	public JMenuItem addAction(Action item, String category) {
		...
		/*
		 * Dimitry, Eric and Dan requested to have the check marks with the original
		 * JCheckBoxMenuItem.
		 */
	...
	private void adjustTooltips(StructuredMenuItemHolder holder) {
		if (Tools.isMacOsX()) {
			...
			if (toolTipText != null) {
				...
				logger.finest("Old tool tip: " + toolTipText + ", New tool tip: "
						+ toolTipTextWithoutTags);
	...
	/**
	 */
	private void addMenu(Object item, StringTokenizer tokens) {
		MapTokenPair categoryPair = getCategoryMap(tokens, menuMap);
		// add the item:
	...
	private static class MapTokenPair {
		...
		MapTokenPair(Map<String, Object> map, String token, Vector<String> order) {
			this.map = map;
			this.token = token;
			this.order = order;
		}
	...
	private MapTokenPair getCategoryMap(StringTokenizer tokens, Map<String, Object> thisMap) {
		if (tokens.hasMoreTokens()) {
			String nextToken = tokens.nextToken();
			if (tokens.hasMoreTokens()) {
				if (!thisMap.containsKey(nextToken)) {
					Map<String, Vector<String>> newMap = new HashMap<>();
					Vector<String> newOrder = new Vector<>();
					newMap.put(ORDER_NAME, newOrder);
					thisMap.put(nextToken, newMap);
				}
				@SuppressWarnings("unchecked")
				Map<String, Object> nextMap = (Map<String, Object>) thisMap.get(nextToken);
				@SuppressWarnings("unchecked")
	...
	public void updateMenus(final JMenuBar myItem, String prefix) {
		...
		@SuppressWarnings("unchecked")
		Map<String, Object> myMap = (Map<String, Object>) pair.map.get(pair.token);
	...
	private static class MenuItemAdder implements MenuAdder {
		...
		public MenuItemAdder(JMenu pMenuItem) {
			...
			mAmountOfVisibleMenuItems =
					Resources.getInstance().getIntProperty(AMOUNT_OF_VISIBLE_MENU_ITEMS, 20);
		...
		public void addMenuItem(StructuredMenuItemHolder holder) {
			...
			if (mItemCounter > mAmountOfVisibleMenuItems) {
				String label =
						Resources.getInstance().getResourceString("StructuredMenuHolder.next");
				if (mMenuCounter > 0) {
					label += " " + mMenuCounter;
				}
	...
	static private void adjustMenuItem(JMenuItem item) {
		if (item.getIcon() == null) {
			...
			if (item.getIcon().getIconWidth() < ICON_SIZE) {
				item.setIconTextGap(
						item.getIconTextGap() + (ICON_SIZE - item.getIcon().getIconWidth()));
			}
		}
	}
	...
	private static class DefaultMenuAdderCreator implements MenuAdderCreator {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see freemind.controller.StructuredMenuHolder.MenuAdderCreator#createAdder
		 * (javax.swing.JMenu)
		 */
	...
	private void updateMenus(MenuAdder menuAdder, Map<String, Object> thisMap,
		...
		for (String category : myVector) {
			...
			if (nextObject instanceof StructuredMenuItemHolder holder) {
				 ...
				 * if(nextObject instanceof JMenuItem) { menuAdder.addMenuItem((JMenuItem)
				 * nextObject); }
				 *//*
					 * else if(nextObject instanceof Action){ menuAdder.addAction((Action)
					 * nextObject); }
					 */else if (nextObject instanceof Map) {
				menuAdder.addCategory(category);
				@SuppressWarnings("unchecked")
				Map<String, Object> nextMap = (Map<String, Object>) nextObject;
				MenuAdder nextItem;
	...
	public static class StructuredMenuListener implements javax.swing.event.MenuListener {
