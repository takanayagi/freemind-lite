...
/**
 * Manages the list of MapModules. As this task is very complex, I exported it from Controller to
 * this class to keep Controller simple.
 *
 * <p>The information exchange between controller and this class is managed by observer pattern (the
 * controller observes changes to the map modules here).
 *
 * <p>TODO: Use an vector with the map modules ordered by the screen order.
 */
public class MapModuleManager {

  public interface MapModuleChangeObserver {
    /**
     * The params may be null to indicate the there was no previous map, or that the last map is
     * closed now.
     */
    boolean isMapModuleChangeAllowed(
        MapModule oldMapModule, Mode oldMode, MapModule newMapModule, Mode newMode);

    void beforeMapModuleChange(
        MapModule oldMapModule, Mode oldMode, MapModule newMapModule, Mode newMode);

    void afterMapClose(MapModule oldMapModule, Mode oldMode);

    void afterMapModuleChange(
        MapModule oldMapModule, Mode oldMode, MapModule newMapModule, Mode newMode);
  ...
  public static class MapModuleChangeObserverCompound implements MapModuleChangeObserver {
    ...
    public boolean isMapModuleChangeAllowed(
      ...
      for (MapModuleChangeObserver observer : new Vector<>(listeners)) {
        returnValue =
            observer.isMapModuleChangeAllowed(oldMapModule, oldMode, newMapModule, newMode);
        if (!returnValue) {
          break;
        }
      }
  ...
  public interface MapTitleContributor {
     ...
     * @param pMapModule
     * @param pModel
     * @return The current title can be changed or something can be added, but it must be returned
     *     as a whole.
     */
  ...
  public void newMapModule(MindMap map, ModeController modeController) {
    MapModule mapModule =
        new MapModule(
            map, new MapView(map, modeController), modeController.getMode(), modeController);
  ...
  void nextMapModule() {
    ...
    if (getMapModule() != null) index = mapModuleVector.indexOf(getMapModule());
    else index = size - 1;
  ...
  // Change MapModules
  /**
   * This is the question whether the map is already opened. If this is the case, the map is
   * automatically opened + returns true. Otherwise does nothing + returns false.
   */
  ...
  /**
   * Checks, whether or not a given url is already opened. Unlike tryToChangeToMapModule, it does
   * not consider the map+extension identifiers nor switches to the module.
   *
  ...
  public String checkIfFileIsAlreadyOpened(URL urlToCheck) throws MalformedURLException {
    for (MapModule module : mapModuleVector) {
      if (module.getModel() != null) {
        ...
        if (sameFile(urlToCheck, moduleUrl)) return module.getDisplayName();
      }
    }
    return null;
  }
  ...
  private void fireNumberOfOpenMapInformation() {
    listener.numberOfOpenMapInformation(
        mapModuleVector.size(), mapModuleVector.indexOf(getMapModule()));
  }
