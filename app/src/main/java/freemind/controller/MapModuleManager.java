 ...
 * 
 * The information exchange between controller and this class is managed by observer pattern (the
...
public class MapModuleManager {
	...
	public interface MapModuleChangeObserver {
		...
		void beforeMapModuleChange(MapModule oldMapModule, Mode oldMode, MapModule newMapModule,
				Mode newMode);

		void afterMapClose(MapModule oldMapModule, Mode oldMode);

		void afterMapModuleChange(MapModule oldMapModule, Mode oldMode, MapModule newMapModule,
				Mode newMode);
	...
	public static class MapModuleChangeObserverCompound implements MapModuleChangeObserver {
		...
		public boolean isMapModuleChangeAllowed(MapModule oldMapModule, Mode oldMode,
			...
			for (MapModuleChangeObserver observer : new Vector<>(listeners)) {
				returnValue = observer.isMapModuleChangeAllowed(oldMapModule, oldMode, newMapModule,
						newMode);
				if (!returnValue) {
					break;
				}
	...
	public boolean changeToMapModule(String mapModuleDisplayName) {
		...
		if (mapModuleCandidate == null) {
			throw new IllegalArgumentException(
					"Map module " + mapModuleDisplayName + " not found.");
		}
