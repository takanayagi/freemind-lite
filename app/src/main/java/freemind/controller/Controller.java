...
/**
 * Provides the methods to edit/change a Node. Forwards all messages to MapModel(editing) or
 * MapView(navigation).
 */
public class Controller implements MapModuleChangeObserver {
	...
	private HashSet<MapModuleManager.MapTitleChangeListener> mMapTitleChangeListenerSet =
			new HashSet<>();
	...
	/**
	 * Converts from a local link to the real file URL of the documentation map. (Used to change
	 * this behaviour under MacOSX).
	 */
	...
	public void init() {
		...
		JToolBar filterToolbar = mFilterController.getFilterToolbar();
	...
	/**
	 * Does basic initializations of this class. Normally, init is called, but if you don't need the
	 * actions, call this method instead.
	 */
	...
	/**
	 * @return the current modeController, or null, if FreeMind is just starting and there is no
	...
	public MapView getView() {
		if (getMapModule() != null) {
			...
			// System.err.println("[Freemind-Developer-Internal-Warning (do not write a bug
			// report, please)]: Tried to get view without being able to get map module.");
			return null;
		}
	}
	...
	public static Color showCommonJColorChooserDialog(Component component, String title,
	...
	public void errorMessage(Object message) {
		...
		JOptionPane.showMessageDialog(getFrame().getContentPane(), myMessage, "FreeMind",
	...
	public void setZoom(float zoom) {
		...
		String stringResult =
				Resources.getInstance().format("user_defined_zoom_status_bar", messageArguments);
	...
	private void quit() {
		...
		if (!mapModuleVector.isEmpty()) {
			String displayName = mapModuleVector.get(0).getDisplayName();
			getMapModuleManager().changeToMapModule(displayName);
		}
		while (!mapModuleVector.isEmpty()) {
			if (getMapModule() != null) {
				StringBuffer restorableBuffer = new StringBuffer();
				boolean closingNotCancelled = getMapModuleManager().close(false, restorableBuffer);
	...
	/**
	 * Manages the history of visited maps. Maybe explicitly closed maps should be removed from
	 * History too?
	 */
		...
		PrintAction(Controller controller, boolean isDlg) {
			super(isDlg ? controller.getResourceString("print_dialog")
					...
					freemind.view.ImageFactory.getInstance()
							.createIcon(getResource("images/fileprint.png")));
			setEnabled(false);
			this.isDlg = isDlg;
		}
	...
	private class PrintPreviewAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			PreviewDialog previewDialog =
					new PreviewDialog(controller.getResourceString("print_preview_title"),
							getView(), getPageFormat());
			previewDialog.pack();
	...
	private class PageAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			final JCheckBox fitToPage = new JCheckBox(getResourceString("fit_to_page"),
					Resources.getInstance().getBoolProperty("fit_to_page"));
			...
			fitToPage.addItemListener(e2 -> userZoom.setEditable(e2.getStateChange() == ItemEvent.DESELECTED));
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (urlText != null && urlText.startsWith(".")) {
					url = localDocumentationLinkConverter.convertLocalLink(urlText);
				} else {
	...
	private class LicenseAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			JOptionPane.showMessageDialog(getView(), controller.getResourceString("license_text"),
					controller.getResourceString("license"), JOptionPane.INFORMATION_MESSAGE);
		}
	}
	...
	private class NavigationPreviousMapAction extends AbstractAction {
		NavigationPreviousMapAction(Controller controller) {
			super(controller.getResourceString("previous_map"), freemind.view.ImageFactory
					.getInstance().createIcon(getResource("images/1leftarrow.png")));
			setEnabled(false);
		}
	...
	private class ShowFilterToolbarAction extends AbstractAction {
		ShowFilterToolbarAction(Controller controller) {
			super(getResourceString("filter_toolbar"), freemind.view.ImageFactory.getInstance()
					.createIcon(getResource("images/filter.gif")));
		}
	...
	private class NavigationNextMapAction extends AbstractAction {
		NavigationNextMapAction(Controller controller) {
			super(controller.getResourceString("next_map"), freemind.view.ImageFactory.getInstance()
					.createIcon(getResource("images/1rightarrow.png")));
			setEnabled(false);
		}
	...
	private class NavigationMoveMapLeftAction extends AbstractAction {
		NavigationMoveMapLeftAction(Controller controller) {
			super(controller.getResourceString("move_map_left"), freemind.view.ImageFactory
					.getInstance().createIcon(getResource("images/draw-arrow-back.png")));
			setEnabled(false);
		}
		...
		public void actionPerformed(ActionEvent event) {
			if (mTabbedPane != null) {
				...
				int previousIndex =
						(selectedIndex > 0) ? (selectedIndex - 1) : (mTabbedPane.getTabCount() - 1);
	...
	private class NavigationMoveMapRightAction extends AbstractAction {
		NavigationMoveMapRightAction(Controller controller) {
			super(controller.getResourceString("move_map_right"), freemind.view.ImageFactory
					.getInstance().createIcon(getResource("images/draw-arrow-forward.png")));
			setEnabled(false);
		}
		...
		public void actionPerformed(ActionEvent event) {
			if (mTabbedPane != null) {
				int selectedIndex = mTabbedPane.getSelectedIndex();
				int previousIndex =
						(selectedIndex >= mTabbedPane.getTabCount() - 1) ? 0 : (selectedIndex + 1);
				moveTab(selectedIndex, previousIndex);
			}
		}
	}
	...
	/**
	 * @param listener The new listener. All currently available properties are sent to the listener
	 *        after registration. Here, the oldValue parameter is set to null.
	 */
	...
	public class PropertyAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			final OptionPanel options =
					new OptionPanel((FreeMind) getFrame(), dialog, props -> {
						Vector<String> sortedKeys = new Vector<>(props.stringPropertyNames());
						...
						for (String key : sortedKeys) {
							...
							propertiesChanged = propertiesChanged
									|| !newProperty.equals(controller.getProperty(key));
	...
	private static class OpenURLAction extends AbstractAction {
		...
		OpenURLAction(Controller controller, String description, String url) {
			super(description, freemind.view.ImageFactory.getInstance()
					.createIcon(controller.getResource("images/Link.png")));
			c = controller;
			this.url = url;
		}
	...
	private void tabSelectionChanged() {
		...
		logger.fine("Selected index of tab is now: " + selectedIndex + " with title:"
				+ module.toString());
	...
	public void insertComponentIntoSplitPane(JComponent pMindMapComponent,
			SplitComponentType pSplitComponentType) {
		if (mOptionalSplitPane == null) {
			mOptionalSplitPane = new JOptionalSplitPane();
			mOptionalSplitPane.setLastDividerPosition(
					getIntProperty(FreeMind.RESOURCES_OPTIONAL_SPLIT_DIVIDER_POSITION, -1));
