...
public class FreeMindFormBuilder extends AbstractFormBuilder<FreeMindFormBuilder> {
  ...
  /**
   * Adds a component to the panel using the default constraints with the given columnSpan. Proceeds
   ...
   *
   * <p>The created label is labeling the given component; so the component gets the focus if the
  ...
  /**
   * Ensures that we have a gap row before the next component row. Checks if the current row is the
   * given {@code RowSpec} and appends this row spec if necessary.
   *
