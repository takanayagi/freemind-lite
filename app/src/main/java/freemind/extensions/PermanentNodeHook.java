...
/**
 * Basic interface for all node hooks that are permanent. Thus, there are methods that inform the
 * plugin about changes on the node, it is stick to. Moreover, methods for loading and saving the
 * hook are present.
 *
 * @author foltin
 */
public interface PermanentNodeHook extends NodeHook {
  ...
  /**
   * Is called if the addedChildNode is inserted as a direct child of the node, this hook is
   * attached to. The cases in which this method is called contain new nodes, paste, move, etc.
   *
   * <p>Ah, don't call propagate in this method, as paste introduces nodes with the hook and you'll
   * have them twice, ... see onNewChild
   */
  void onAddChild(MindMapNode addedChildNode);

  /**
   * Is only called, if a new nodes is inserted as a child. Remark: In this case onAddChild is
   * called too and moreover *before* this method. see onAddChild.
   */
  void onNewChild(MindMapNode newChildNode);

  /**
   * This method is called, if a child is added to me or to any of my children. (See
  ...
  /**
   * This method is called, if a child is removed to me or to any of my children. (See
   * onUpdateChildrenHook)
   *
   * @param oldDad TODO
   */
