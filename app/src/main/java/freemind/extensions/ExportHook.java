...
public class ExportHook extends ModeControllerHookAdapter {
  ...
  public static File chooseImageFile(
      String type, String description, String nameExtension, ModeController controller) {
    ...
    if (mmFile != null) {
      String proposedName =
          mmFile.getAbsolutePath().replaceFirst("\\.[^.]*?$", "")
    ...
    if (chosenFile.exists()) { // If file exists, ask before overwriting.
      String overwriteText =
          MessageFormat.format(controller.getText("file_already_exists"), chosenFile.toString());
      int overwriteMap =
          JOptionPane.showConfirmDialog(
              component, overwriteText, overwriteText, JOptionPane.YES_NO_OPTION);
  ...
  public BufferedImage createBufferedImage() {
    ...
    view.print(g);
    myImage =
        myImage.getSubimage(innerBounds.x, innerBounds.y, innerBounds.width, innerBounds.height);
    view.endPrinting();
    return myImage;
    // NodeAdapter root = (NodeAdapter) getController().getMap().getRoot();
