...
public class ExportHook extends ModeControllerHookAdapter {
	...
	public static File chooseImageFile(String type, String description, String nameExtension,
		...
		if (chosenFile.exists()) { // If file exists, ask before overwriting.
			String overwriteText = MessageFormat.format(controller.getText("file_already_exists"),
					chosenFile.toString());
			int overwriteMap = JOptionPane.showConfirmDialog(component, overwriteText,
	...
	protected void copyFromResource(String prefix, String fileName, String destinationDirectory) {
		...
		try {
			...
			OutputStream out = new FileOutputStream(destinationDirectory + "/" + fileName);

			// Transfer bytes from in to out
			Tools.copyStream(in, out, true);
		} catch (Exception e) {
			logger.severe("File not found or could not be copied. " + "Was earching for " + prefix
					+ fileName + " and should go to " + destinationDirectory);
	...
	protected void copyFromFile(String dir, String fileName, String destinationDirectory) {
		...
		try {
			...
			logger.severe("File not found or could not be copied. " + "Was earching for " + dir
					+ fileName + " and should go to " + destinationDirectory);
			freemind.main.Resources.getInstance().logException(e);
		}
