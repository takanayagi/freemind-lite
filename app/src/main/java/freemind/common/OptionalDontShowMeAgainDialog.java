...
public class OptionalDontShowMeAgainDialog {
	...
	public interface DontShowPropertyHandler {
		/**
		 * @return accepted are the following values as return values: * "" (means: show this
		 *         dialog) * "true" (means: the answer was ok and I want to remember that). *
	...
	public OptionalDontShowMeAgainDialog(JFrame pFrame, Component pComponent, String pMessageId,
			String pTitleId, TextTranslator pTextTranslator,
	...
	public OptionalDontShowMeAgainDialog show() {
		...
		mDialog.getContentPane().add(new JLabel(mTextTranslator.getText(mMessageId)),
				new GridBagConstraints(1, 0, 1, 1, 10.0, 4.0, GridBagConstraints.WEST,
						GridBagConstraints.BOTH, new Insets(5, 5, 0, 0), 0, 10));
		ImageIcon questionMark = freemind.view.ImageFactory.getInstance()
				.createIcon(Resources.getInstance().getResource("images/icons/help.png"));
		mDialog.getContentPane().add(new JLabel(questionMark),
		...
		JButton cancelButton =
				new JButton(mTextTranslator.getText("OptionalDontShowMeAgainDialog.cancel"));
