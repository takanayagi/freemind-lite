/*
 * FreeMind - A Program for creating and viewing Mindmaps Copyright (C) 2000-2013 Joerg Mueller,
...
public abstract class FreeMindTask extends Thread {
	...
	protected static class ProgressDescription {
		/**
		 * @param pProgressString progress
		 * @param pProgressParameters params
		 */
		...
		while (again) {
			...
			if (mProgressMonitor.isVisible()) {
				...
				if (mProgressDescription == null) {
					progressDescription = new ProgressDescription("FreeMindTask.Default",
							new Object[]{mRounds});
				}
				boolean canceled = mProgressMonitor.showProgress(mRounds, mAmountOfSteps,
						progressDescription.mProgressString,
	...
	/**
	 * Subclasses should process one single action out of the set of its actions and then return.
	 * The method is directly called again by the task controller until it returns false.
	 * 
