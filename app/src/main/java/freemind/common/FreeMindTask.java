...
public abstract class FreeMindTask extends Thread {
    ...
    while (again) {
      ...
      if (mProgressMonitor.isVisible()) {
        ...
        if (mProgressDescription == null) {
          progressDescription =
              new ProgressDescription("FreeMindTask.Default", new Object[] {mRounds});
        }
        boolean canceled =
            mProgressMonitor.showProgress(
                mRounds,
                mAmountOfSteps,
  ...
  /**
   * Subclasses should process one single action out of the set of its actions and then return. The
   * method is directly called again by the task controller until it returns false.
   *
