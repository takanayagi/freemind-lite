/*
 * GrabKeyDialog.java - Grabs keys from the keyboard :tabSize=8:indentSize=8:noTabs=false:
...
public class GrabKeyDialog extends JDialog {
  ...
  public static String toString(KeyEvent evt) {
    ...
    return id
        ...
        + ",modifiers=0x"
        + Integer.toString(evt.getModifiersEx(), 16);
  } // }}}
  ...
  // {{{ getShortcut() method
  /**
   * Returns the shortcut, or null if the current shortcut should be removed or the dialog either
   * has been cancelled. Use isOK() to determine if the latter is true.
   */
  public String getShortcut() {
    if (isOK) return shortcut.getText();
    else return null;
  } // }}}
  ...
  private void init(KeyBinding binding, Vector<KeyBinding> allBindings, Buffer debugBuffer) {
    ...
    JPanel content =
        new JPanel(new GridLayout(0, 1, 0, 6)) {
          /**
           * Makes the tab key work in Java 1.4.
           *
           * @since jEdit 3.2pre4
  ...
  private String getSymbolicName(int keyCode) {
    ...
    /*
     * else if(keyCode == KeyEvent.VK_OPEN_BRACKET) return "["; else if(keyCode ==
  ...
  private void updateAssignedTo(String shortcut) {
    ...
    if (kb != null)
      if (kb.isPrefix) text = getText("grab-key.assigned-to.prefix") + " " + shortcut;
      // FIXME: getText("grab-key.assigned-to.prefix")+
      // new String[] { shortcut };
      else text = kb.label;

    if (ok != null) ok.setEnabled(kb == null || !kb.isPrefix);
  ...
  class InputPane extends JTextField {
    ...
    protected void processKeyEvent(KeyEvent _evt) {
      if ((getModifierMask() & _evt.getModifiersEx()) != 0) {
        KeyEvent evt =
            new KeyEvent(
                _evt.getComponent(),
                _evt.getID(),
                _evt.getWhen(),
                ~getModifierMask() & _evt.getModifiersEx(),
                _evt.getKeyCode(),
                _evt.getKeyChar(),
                _evt.getKeyLocation());
        processKeyEvent(evt);
      ...
      if (debugBuffer != null) {
        debugBuffer.insert(
            debugBuffer.getLength(),
            "Event " + GrabKeyDialog.toString(_evt) + (evt == null ? " filtered\n" : " passed\n"));
      }
  ...
  class ActionHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      if (evt.getSource() == ok) {
        ...
        isOK = true;
        dispose();
      } else if (evt.getSource() == cancel) dispose();
    ...
    private boolean canClose() {
      ...
      if (Objects.equals(bindingReset.name, binding.name)) {
        ...
        JOptionPane.showMessageDialog(
            GrabKeyDialog.this, getText("grab-key.duplicate-alt-shortcut"));
        return false;
      }
