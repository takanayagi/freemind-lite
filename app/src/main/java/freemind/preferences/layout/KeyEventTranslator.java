/*
 * KeyEventTranslator.java - Hides some warts of AWT event API :tabSize=8:indentSize=8:noTabs=false:
...
/**
 * In conjunction with the <code>KeyEventWorkaround</code>, hides some warts in the AWT key event
 * API.
 *
 * @author Slava Pestov
...
public class KeyEventTranslator {
  ...
  /**
   * Pass this an event from {@link KeyEventWorkaround#processKeyEvent(java.awt.event.KeyEvent)}.
   *
   * @since jEdit 4.2pre3
   */
  public static Key translateKeyEvent(KeyEvent evt) {
    ...
    switch (evt.getID()) {
      case KeyEvent.KEY_PRESSED:
        ...
        if ((keyCode >= KeyEvent.VK_0 && keyCode <= KeyEvent.VK_9)
          ...
          if (KeyEventWorkaround.ALTERNATIVE_DISPATCHER) return null;
          else {
            returnValue =
                new Key(
                    ...
                    // need this.
                    '\0',
                    Character.toUpperCase((char) keyCode));
          }
        } else {
          if (keyCode == KeyEvent.VK_TAB) {
            ...
            if ((modifiers & ~InputEvent.SHIFT_DOWN_MASK) == 0) returnValue = null;
            else {
      ...
      case KeyEvent.KEY_TYPED:
        ...
        switch (ch) {
          ...
          case '\b':
            return null;
          case ' ':
            if ((modifiers & ~InputEvent.SHIFT_DOWN_MASK) != 0) return null;
        }
    ...
    /*
     * I guess translated events do not have the 'evt' field set so consuming won't work. I
     * don't think this is a problem as nothing uses translation anyway
     */
    Key trans = transMap.get(returnValue);
    if (trans == null) return returnValue;
    else return trans;
  } // }}}
  ...
  /**
   * Changes the mapping between symbolic modifier key names (<code>C</code>, <code>A</code>,
   * <code>M</code>, <code>S</code>) and Java modifier flags.
   *
  ...
  public static String getSymbolicModifierName(int mod) {
    if ((mod & c) != 0) return "control";
    else if ((mod & a) != 0) return "alt";
    else if ((mod & m) != 0) return "meta";
    else if ((mod & s) != 0) return "shift";
    else return "";
  } // }}}
  ...
  public static String modifiersToString(int mods) {
    ...
    if ((mods & InputEvent.ALT_DOWN_MASK) != 0) {
      ...
      else buf.append(GrabKeyDialog.MODIFIER_SEPARATOR);
  ...
  public static String getModifierString(InputEvent evt) {
    ...
    if (evt.isControlDown()) buf.append(getSymbolicModifierName(InputEvent.CTRL_DOWN_MASK));
    if (evt.isAltDown()) buf.append(getSymbolicModifierName(InputEvent.ALT_DOWN_MASK));
    if (evt.isMetaDown()) buf.append(getSymbolicModifierName(InputEvent.META_DOWN_MASK));
    if (evt.isShiftDown()) buf.append(getSymbolicModifierName(InputEvent.SHIFT_DOWN_MASK));
  ...
  public static class Key {
    ...
    public boolean equals(Object o) {
      if (o instanceof Key k) {
        return (modifiers.equals(k.modifiers)) && key == k.key && input == k.input;
      }
