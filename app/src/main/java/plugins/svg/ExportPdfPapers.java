 ...
 * @author Andy
 * 
 *         Class is resposible to provide all papers that could be used for PDF export
 */
class ExportPdfPapers {
	...
	private void addPaper(MediaSizeName name, String displayName) {
		...
		paper.setSize(mSize.getX(MediaSize.MM) * ppmm, mSize.getY(MediaSize.MM) * ppmm);
	 ...
	 * 
	 * @param formats string like A3 or A4 that is one of the values of getPaperNames()
