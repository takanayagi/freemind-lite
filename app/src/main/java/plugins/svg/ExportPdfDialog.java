 ...
 * @author Andy
 * 
 *         Class is responsible for a dialog to input all pdf export parameters like page
...
public class ExportPdfDialog extends JDialog {
	...
	private void initialize() {
		...
		listBox = new JComboBox<String>(formatsCollection);
		listBox.setEditable(false);
		listBox.setPreferredSize(new Dimension(100, 25));

		// Page orientation
		birdButtonLandscape =
				new JRadioButton(controller.getResourceString("ExportPdfDialog.Landscape"));
		birdButtonLandscape.setMnemonic(KeyEvent.VK_L);
		birdButtonLandscape.setSelected(true);

		birdButtonPortrait =
				new JRadioButton(controller.getResourceString("ExportPdfDialog.Portrait"));
		...
		panel1.setBorder(BorderFactory
				.createTitledBorder(controller.getResourceString("ExportPdfDialog.Paper")));
		GridBagLayout gridbag1 = new GridBagLayout();
		panel1.setLayout(gridbag1);

		JPanel panel2 = new JPanel();
		panel2.setBorder(BorderFactory
				.createTitledBorder(controller.getResourceString("ExportPdfDialog.Orientation")));
	...
	private void initDefaults() {
		// page orientation
		String storedOrientation = controller.getProperty(PROP_PDF_PAGE_ORIENTATION);
