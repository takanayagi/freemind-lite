...
public class ExportPdf extends ExportVectorGraphic {
	...
	public void startupMapHook() {
		...
		boolean nodeExport = Tools.safeEquals("node", getResourceString("export_type"));
		...
		while (!selecteds.isEmpty()) {
			MindMapNode selectedNode = selecteds.remove(0);
			String nameExtension = null;
			if (nodeExport) {
				nameExtension = " " + selectedNode.getShortText(getController());
			}
			File chosenFile =
					chooseFile("pdf", getResourceString("export_pdf_text"), nameExtension);
			if (chosenFile == null) {
				return;
			}
		...
		try {
			for (File fileToOpen : documentsToOpen) {
				getController().getFrame().openDocument(Tools.fileToUrl(fileToOpen));
			}
		} catch (Exception e) {
			freemind.main.Resources.getInstance().logException(e);
		}
	...
	public HashMap<TranscodingHints.Key, Float> choosePaper() {
		...
		ExportPdfDialog dialog =
				new ExportPdfDialog(getController().getFrame().getJFrame(), paperNames, controller);
		...
		if (paper != null) {
			...
			if (pageFormat.getOrientation() == PageFormat.PORTRAIT) {
				...
				// portrait
				retValue.put(PDFTranscoder.KEY_HEIGHT,
						Float.valueOf((float) pageFormat.getPaper().getHeight()));
				retValue.put(PDFTranscoder.KEY_WIDTH,
						Float.valueOf((float) pageFormat.getPaper().getWidth()));
			} else {
	...
	public boolean exportAsPdf(boolean nodeExport, MindMapNode selectedNode, File chosenFile)
			throws Exception {
		return exportAsPdf(nodeExport, selectedNode, chosenFile, null);
	}

	public boolean exportAsPdf(boolean nodeExport, MindMapNode selectedNode, File chosenFile,
			HashMap<TranscodingHints.Key, Float> pTranscoderHints) throws Exception {
		...
		/*
		 * according to https://sourceforge.net/tracker/?func=detail&atid=107118&
		 * aid=1921334&group_id=7118
		 * 
		 * Submitted By: Frank Spangenberg (f_spangenberg) Summary: Large mind maps produce invalid
		 * PDF
		 */
		pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_HEIGHT, Float.valueOf(19200f));
		pdfTranscoder.addTranscodingHint(PDFTranscoder.KEY_MAX_WIDTH, Float.valueOf(19200f));
