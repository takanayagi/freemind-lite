...
public class MemoryTest {
  ...
  public void testCreateLanguagesMenu() throws Exception {
    ...
    if (memoryBefore + 800_000 < memoryAfter) {
      fail(
          "Memory Leak SpellChecker.createLanguagesMenu. memory before:"
              + (memoryBefore / 1024)
              + " KB  memory after:"
              + (memoryAfter / 1024)
              + " KB");
  ...
  public void testRegister() throws Exception {
    ...
    if (memoryBefore + 500_000 < memoryAfter) {
      fail(
          "Memory Leak SpellChecker.register. memory before:"
              + (memoryBefore / 1024)
              + " KB  memory after:"
              + (memoryAfter / 1024)
              + " KB");
