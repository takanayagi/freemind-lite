...
public class Utils {
  ...
  /**
   * Translate a GUI string in one of the supported languages. If the value was not find then the
   * key is returned.
   *
  ...
  static JButton getButton(String resource) {
    ...
    if (customProvider != null) {
      ...
      if (!Objects.equals(tooltip, tooltipKey)) {
  ...
  static void setDialogIcon(JDialog dlg) {
    try {
      ...
      // with earlier JVMs. Equivalent to calling setIconImage(image);
      Class<?> cls = Dialog.class;
      java.lang.reflect.Method m = cls.getMethod("setIconImage", Image.class);
      m.invoke(dlg, image);
    } catch (Throwable e1) {
  ...
  /**
   * Create a String where the first letter is written with an uppercase.
   *
  ...
  /**
   * Create a String with inverted case for the first letter. If it is lowercase then it will change
   ...
   *
   * @param word the word that should be checked. It can not be null.
   * @return true if the first character is an uppercase letter
   */
   ...
   *
   * @param word the word that should be checked. It can not be null or empty.
   * @return if all character are an uppercase letter
   */
  ...
  /**
   * Check and replace Unicode variants of quotation marks and hyphens. Unicode characters for
   * general punctuation can be seen here: http://www.unicodemap.org/range/40/General_Punctuation/
   *
   * @param word the word that should be checked. It can not be null.
