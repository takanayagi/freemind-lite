...
public final class CheckerListener implements PopupMenuListener, LanguageChangeListener {
  ...
  public void popupMenuWillBecomeVisible(PopupMenuEvent ev) {
    ...
    if (invoker instanceof JTextComponent jText) {
      ...
      try {
        ...
        // find the first invalid word from current position, use the Tokenizer that it is ever
        // compatible with the red zigzag line
        ...
        boolean needCapitalization =
            tokenizer.isFirstWordInSentence() && Utils.isFirstCapitalized(word);
  ...
  protected int getCursorPosition(JTextComponent jText) throws BadLocationException {
    ...
    if (offs > 0
        && (offs >= doc.getLength() || Character.isWhitespace(doc.getText(offs, 1).charAt(0)))) {
   ...
   * @param needCapitalization if the first letter of the suggestion should be capitalized
   */
  void addSuggestionMenuItem(
   ...
   *
   * @param jText current JTextComponent
   * @param word current word, which can be added
   * @param addSeparator true, add a separator before the menu item
   */
  protected void addMenuItemAddToDictionary(
      JTextComponent jText, String word, boolean addSeparator) {
