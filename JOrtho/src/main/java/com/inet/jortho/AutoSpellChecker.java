...
class AutoSpellChecker implements DocumentListener, LanguageChangeListener {
  ...
  static void disable(JTextComponent text) {
    ...
    for (DocumentListener listener : doc.getDocumentListeners()) {
      if (listener instanceof AutoSpellChecker autoSpell) {
  ...
  /**
   * Refresh the highlighting. This can be useful if the dictionary was modified.
   *
   * @param text the JTextComponent
   */
  ...
  private void checkElements(int offset, int length) {
    ...
    do {
      try {
        // We need to use a ParagraphElement because a CharacterElement produce problems with
        // formating in a word
   ...
   *
   * @param element the checking Element
   */
  private void checkElement(javax.swing.text.Element element) {
    try {
      ...
      int l = jText.getDocument().getLength();
      j = Math.min(j, l);
      if (i >= j) return;
  ...
  /**
   * // * Check the completely text. Because this can consume many times with large Documents that
   * this will do in a thread in the background step by step.
   */
  private void checkAll() {
    if (jText == null) {
      // the needed objects does not exist
      return;
    }
    if (dictionary == null) {
            ...
            () -> {
              ...
              for (int i = 0; i < document.getLength(); ) {
                try {
                  ...
                  SwingUtilities.invokeLater(() -> checkElement(element));
                } catch (Exception ex) {
                  return;
                }
              }
