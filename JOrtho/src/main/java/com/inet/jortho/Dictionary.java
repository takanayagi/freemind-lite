...
final class Dictionary extends DictionaryBase {
  ...
  /**
   * Create a Dictionary from a serialize Dictionary. This is used from the DictionaryFactory.
   *
   * @see #toArray()
  ...
  /**
   * Trims the capacity of this <tt>Dictionary</tt> instance to be the current size. An application
   * can use this operation to minimize the storage of an <tt>Dictionary</tt> instance. The load
   * methods already call it.
   */
  void trimToSize() {
  ...
  /**
   * Add a word to the tree. If it already exists then it has no effect.
   *
  ...
  private void searchCharOrAdd(char c) {
    if (c == LAST_CHAR) throw new RuntimeException("Invalid Character");
    while (idx < size && tree[idx] < c) {
      idx += 3;
    }
    if (idx >= size) throw new RuntimeException("Internal Error");
    if (tree[idx] == c) {
      return;
    }
    insertChar(c);
  ...
  /**
   * Create a new node at end of the array. On the current idx position is writing the pointer. The
   * pointer on the current idx position must be 0 without some word end flags (0x8000 on idx+1)
   *
