...
class Tokenizer {
  ...
  Tokenizer(
      JTextComponent jText, Dictionary dictionary, Locale locale, SpellCheckerOptions options) {
  ...
  Tokenizer(
    ...
    this(
        ...
        dictionary,
        locale,
        Utilities.getParagraphElement(jText, offset).getStartOffset(),
  ...
  /** Get the next misspelling word. If not found then it returns null. */
    ...
    while (true) {
      ...
      while (endWord != BreakIterator.DONE) {
        ...
        if (word.length() > 1 && Character.isLetter(word.charAt(0))) {
          boolean exist =
              bundle.existInDictionary(word, dictionary, options, isFirstWordInSentence);
          ...
          if (!exist && startWord + 1 == endWord) {
            ...
            if (nextChar == '.' || nextChar == '\'') {
              exist =
                  bundle.existInDictionary(
                      word + nextChar, dictionary, options, isFirstWordInSentence);
            }
          }
  ...
  /**
   * Check if the word is a web address. This means an email address or web page address.
   *
   * @param word the word that should be checked. It can not be null and can not include any
   *     whitespace.
  ...
  private boolean isWebAddress(String word) {
    ...
    if (startWord + 3 < sentence.length()
        && sentence.charAt(startWord) == ':'
        && sentence.charAt(startWord + 1) == '/'
        && sentence.charAt(startWord + 2) == '/') {
  ...
  private boolean nextParagraph() {
    if (doc instanceof AbstractDocument) {
      paragraphOffset =
          ((AbstractDocument) doc).getParagraphElement(paragraphOffset).getEndOffset();
  ...
  /**
   * Update the text after a word was replaced. The changes in the text should be only after the
   * current word offset.
   */
  void updatePhrase() {
