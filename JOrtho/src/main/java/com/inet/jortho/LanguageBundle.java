...
public class LanguageBundle extends ResourceBundle {
  ...
  /**
   * Check if the word is in the current dictionary. It verifies the options isCaseSensitive,
   * getIgnoreCapitalization and the first word of sentence
   *
  ...
  boolean existInDictionary(
    ...
    if (!exist && !options.isCaseSensitive()) {
      exist = dictionary.exist(Utils.getInvertedCapitalizion(word));
        ...
        && (isFirstWordInSentence || options.getIgnoreCapitalization())
