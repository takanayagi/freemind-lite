...
abstract class DictionaryBase {
  ...
  /**
   * Check if the word exist in this dictionary.
   *
  ...
  public List<Suggestion> searchSuggestions(String word) {
    ...
    Suggestions suggestions = new Suggestions(Math.min(20, 4 + word.length()));
    idx = 0;
    searchSuggestions(suggestions, word, 0, 0, 0);
   ...
   * difference. The smaller the difference that so similar. This method is calling recursive
   * itself.
   *
  ...
  private void searchSuggestions(
      Suggestions list, CharSequence chars, int charPosition, int lastIdx, int diff) {
  ...
  private void searchSuggestionsLonger(
      Suggestions list, CharSequence chars, int lastIdx, int diff) {
    int tempIdx = idx = lastIdx;
    while (idx < size && tree[idx] < LAST_CHAR) {
      StringBuilder buffer = new StringBuilder();
      buffer.append(chars);
  ...
  /**
   * Search if the character exist in the current node. If found then the variable <code>idx</code>
   * point to the location. If not found then it point on the next character (char value) item in
   * the node.
   *
  ...
  /**
   * Returns an int that describe the dissimilarity of the characters. The value is ever larger 0. A
   * value of means only a small difference.
   *
   * @param a first char
   * @param b second char
