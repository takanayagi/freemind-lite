...
class DictionaryEditDialog extends JDialog {
  ...
  DictionaryEditDialog(JDialog parent) {
    ...
    JButton delete = Utils.getButton("delete");
    content.add(
        delete,
        new GridBagConstraints(
            2,
    ...
    JButton export = Utils.getButton("export");
    content.add(
        export,
        new GridBagConstraints(
            2,
    ...
    JButton importBtn = Utils.getButton("import");
    content.add(
        importBtn,
        new GridBagConstraints(
            2,
    ...
    JButton close = Utils.getButton("close");
    content.add(
        close,
        new GridBagConstraints(
            2,
  ...
  /**
   * A hack for the layout manger to prevent that the dialog is too small to show the title line.
   * The problem occur only if there are small words in the list. With an empty list there are no
   * problems.
   */
  @Override
  ...
  private void loadWordList(
      DefaultListModel<String> model, HashSet<String> wordSet, Iterator<String> wordIterator) {
  ...
  private static File getSuggestedFile() {
    return new File(
        Utils.getResource("userDictionary") + "_" + SpellChecker.getCurrentLocale() + ".txt");
  }
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              DictionaryEditDialog.this, ex.toString(), "Error", JOptionPane.ERROR_MESSAGE);
        }
      }
    }
  }
