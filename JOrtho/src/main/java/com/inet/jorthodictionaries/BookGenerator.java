...
/**
 * How to use
 * <li>Download the latest Wiktionary file "pages_articles.xml". It is typical compressed. The
 *     position changed. We found it last at:
 *
 *     <ul>
...
public abstract class BookGenerator {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (String language : languages) {
      ...
      BookGenerator generator =
          (BookGenerator)
              Class.forName(BookGenerator.class.getName() + "_" + language)
   ...
   *
   * @param file Daten im XML format
   */
   ...
   */
  private void saveStatistics(File dictFile) throws Exception {
    String statistics = "";
   ...
   * @throws Exception
   */
  private void createPackage(String language) throws Exception {
    ZipOutputStream out =
        new ZipOutputStream(
  ...
  private void addFileToZip(ZipOutputStream out, String filename, boolean delete) throws Exception {
    File file = new File(filename);
  ...
  /**
   * Check if the word is valid word. This exclude help pages and some phrases. It should be call
   * ever before addWord(String)
   *
  ...
  /**
   * Check if a word is a valid word of the current language. With function getBook().addWord() you
   * can add additional Flexion of the word. The current word self does not need added.
   *
