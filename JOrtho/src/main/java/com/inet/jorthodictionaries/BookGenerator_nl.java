...
public class BookGenerator_nl extends BookGenerator {
  ...
  boolean isValidLanguage(String word, String wikiText) {
    ...
    addTemplate(
        word, wikiText, "-nlverb-onp-", 7); // http://nl.wiktionary.org/wiki/Sjabloon:-nlverb-onp-
    addTemplate(
        word,
        wikiText,
        "-nlverb-reflex-",
        11); // http://nl.wiktionary.org/wiki/Sjabloon:-nlverb-reflex-

    if (isNlWord) {
      addTemplate(
          word, wikiText, "adjcomp", 6); // http://nl.wiktionary.org/wiki/Sjabloon:adjcomp
      addTemplate(
          word,
          wikiText,
          "adjcompp",
          9); // http://nl.wiktionary.org/wiki/Sjabloon:adjcompp
    }
    ...
    for (int i = 1; i <= count; i++) {
      ...
      for (String word : words) {
        ...
        if (super.isValidWord(word)) {
          addWord(word);
          //                } else {
          //                    if( word.length() > 0 && !word.equals( "-" ) && !word.equals( "[1]"
          // ) && !word.equals( "[2]" )
          //                                    && !word.equals( "1" ) && !word.equals( "2" ) &&
          // !word.equals( "3" ) ) {
