...
public class BookGenerator_de extends BookGenerator {
  ...
  protected boolean isValidWord(String word) {
    ...
    return word.endsWith("(Deklination)")
        || word.endsWith("(Konjugation)")
        || word.equals("Wiktionary:Wunschliste");
  }
  ...
  /**
   * Get the chapter on the current position. If a next chapter line is not find then the completely
   * wikiText is return.
   *
   * @param wikiText
  ...
  private String getChapter(String wikiText, int headerIdx) {
    ...
    Pattern pattern =
        Pattern.compile("^" + marker + "[^=].*[^=]" + marker + "\\s*$", Pattern.MULTILINE);
   ...
   */
  private final boolean searchWordAndAdd(
      String baseWord, String wikiText, String marker, int fromIndex) {
    ...
    if (idx1 > 0) {
      ...
      if (idx2 > 0) {
        ...
        System.out.println(
            "End not found for marker '" + marker + "' for base word '" + baseWord + "'");
      }
    } else {
  ...
  private void addLinkWords(String baseWord, String wikiText, String marker) {
    ...
    while (idx3 > 0) {
      ...
      if (idx4 > 0) {
        if (idx3 + 2
            < idx4) { // leere Werte die vom Template entstehen, aus Performance gleich skippen
  ...
  private final void addFormatedWordPhrase(String baseWord, String key, String phrase) {
    ...
    if (phrase.equals("{{fehlend}}")
        || phrase.equals("---")
        || phrase.equals("--")
        || phrase.equals("{{center")) {
      return;
    }
    ...
    if (idx3 >= 0 && idx4 > 0) {
      ...
      String word3 =
          word1.length() + word2.length() > 0 ? word1 + word2 : phrase.substring(idx3 + 1, idx4);
  ...
  private final boolean addWordPhrase(String phrase) {
    ...
    if (index > 0 && Character.isLetter(phrase.charAt(index - 1))) {
      for (int i = 0; i < 2; i++) {
        addWordPhrase(
            phrase.substring(0, index) + (i == 0 ? "e" : "") + phrase.substring(index + 3));
      }
    } else {
   ...
   * http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_unregelm%C3%A4%C3%9Fig
   */
  ...
  /**
   * Implementation of the template
   * http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_schwach_untrennbar Implementation of the
   * template http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_schwach_untrennbar_reflexiv
   */
  ...
  /**
   * Implementation of the template
   * http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_schwach_trennbar Implementation of the
   * template http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_schwach_trennbar_reflexiv
   */
  ...
  /**
   * Implementation of the template
   * http://de.wiktionary.org/wiki/Vorlage:Deutsch_Verb_schwach_doppelt_trennbar
   */
